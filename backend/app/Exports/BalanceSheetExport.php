...
class BalanceSheetExport extends SafeMoneyExport implements FromArray, WithHeadings, WithStyles
{
    ...
    public function array(): array
    {
        ...
        $assetsStartTotal = DecimalMoney::sum(array_column($this->data['assets'], 'start_balance'));
        $liabilitiesTotal = DecimalMoney::sum(array_column($this->data['liabilities'], 'end_balance'));
        $liabilitiesStartTotal = DecimalMoney::sum(array_column($this->data['liabilities'], 'start_balance'));
        $equityTotal = DecimalMoney::sum(array_column($this->data['equity'], 'end_balance'));
        $equityStartTotal = DecimalMoney::sum(array_column($this->data['equity'], 'start_balance'));

        $rows[] = ['TÀI SẢN', '', '', '', ''];
        foreach ($this->data['assets'] as $asset) {
            $rows[] = [$asset['name'], $asset['code'], '', $asset['end_balance'] ?: '', $asset['start_balance'] ?: ''];
        }
        $rows[] = ['TỔNG CỘNG TÀI SẢN', '270', '', $assetsTotal, $assetsStartTotal];

        $rows[] = ['NGUỒN VỐN', '', '', '', ''];
        $rows[] = ['I. Nợ phải trả', '300', '', $liabilitiesTotal, $liabilitiesStartTotal];
        foreach ($this->data['liabilities'] as $liab) {
            $rows[] = [$liab['name'], $liab['code'], '', $liab['end_balance'] ?: '', $liab['start_balance'] ?: ''];
        }
        $rows[] = ['II. Vốn chủ sở hữu', '400', '', $equityTotal, $equityStartTotal];
        foreach ($this->data['equity'] as $eq) {
            $rows[] = [$eq['name'], $eq['code'], '', $eq['end_balance'] ?: '', $eq['start_balance'] ?: ''];
        }
        $rows[] = ['TỔNG CỘNG NGUỒN VỐN', '440', '', DecimalMoney::add($liabilitiesTotal, $equityTotal), DecimalMoney::add($liabilitiesStartTotal, $equityStartTotal)];
