...
class CashReportService
{
    ...
    public function generate(int $companyId, string $code, array $filters): array
    {
        ...
        return match ($code) {
            ...
            'CA-01' => $this->dailyBalance($movements, $filters, $companyId),
            'CA-02' => $this->cashFlow($movements, $filters),
            'CA-03' => $this->detailedLedger($movements, $filters, $companyId),
        };
    }
    ...
    /** @param Collection<int,array<string,int|string>> $movements */
    private function dailyBalance(Collection $movements, array $filters, int $companyId): array
    {
        [$opening, $period] = $this->openingAndPeriod($movements, $filters, $this->confirmedCashOpeningBalance($companyId, $filters['date_from']));
        $receipts = 0;
        $payments = 0;
        $balance = $opening;
        $daily = [];
    ...
    /** @param Collection<int,array<string,int|string>> $movements */
    private function detailedLedger(Collection $movements, array $filters, int $companyId): array
    {
        [$opening, $period] = $this->openingAndPeriod($movements, $filters, $this->confirmedCashOpeningBalance($companyId, $filters['date_from']));
        $receipts = 0;
        $payments = 0;
    ...
    private function openingAndPeriod(Collection $movements, array $filters, int $opening = 0): array
    {
        $period = [];
    ...
    private function confirmedCashOpeningBalance(int $companyId, string $dateFrom): int
    {
        if (! Schema::hasTable('opening_balance_packages') || ! Schema::hasTable('opening_balance_account_lines')) {
            return 0;
        }

        $rows = DB::table('opening_balance_account_lines as line')
            ->join('opening_balance_packages as package', 'package.id', '=', 'line.package_id')
            ->where('package.company_id', $companyId)
            ->where('package.status', 'confirmed')
            ->whereDate('package.effective_date', '<=', $dateFrom)
            ->where('line.account_code', 'like', '111%')
            ->get(['line.debit_amount', 'line.credit_amount']);
        $balance = DecimalMoney::ZERO;
        foreach ($rows as $row) {
            $balance = DecimalMoney::add($balance, DecimalMoney::subtract($row->debit_amount, $row->credit_amount));
        }

        if (! preg_match('/^-?\d+\.00$/D', $balance)) {
            throw new UnexpectedValueException('Cash opening balance must be a whole VND amount.');
        }

        $value = (int) substr($balance, 0, -3);
