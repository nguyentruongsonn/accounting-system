...
class InventoryIssueService
{
        ...
        return DB::transaction(function () use ($data, $companyId) {
            ...
            foreach ($lines as $line) {
                ...
                $issue->lines()->create([
                    ...
                    'unit' => $line['unit'] ?? null,
                    'warehouse_id' => $line['warehouse_id'] ?? $issue->warehouse_id ?? Warehouse::where('company_id', $companyId)->value('id'),
            ...
            if ($lines !== null) {
                ...
                foreach ($lines as $line) {
                    ...
                    $issue->lines()->create([
                        'item_id' => $line['item_id'],
                        'unit' => $line['unit'] ?? null,
                        'warehouse_id' => $line['warehouse_id'] ?? $issue->warehouse_id ?? Warehouse::where('company_id', $issue->company_id)->value('id'),
                        'warehouse_code' => $line['warehouse_code'] ?? null,
                        'description' => $line['description'] ?? $issue->description,
                        'quantity' => $amounts['quantity'],
                        'unit_price' => $amounts['unit_price'],
    ...
    public function post($id, ?int $companyId = null)
    {
        ...
        return DB::transaction(function () use ($id, $companyId) {
            ...
            $policy = ! $directPosting && config('accounting.enforce_inventory_posting_account_mappings', true)
            ...
            $accountMappings = ! $directPosting && $policy !== null ? $this->accountMappingGate->requireSatisfied($issue, $policy) : null;
            ...
            foreach ($issue->lines as $line) {
                if ($line->amount > 0) {
                    $debitAccount = $accountMappings !== null
                        ...
                        : ($directPosting ? $this->requireDirectAccount($line->debit_account, 'debit_account') : $line->debit_account);
                    $creditAccount = $accountMappings !== null
                        ...
                        : ($directPosting ? $this->requireDirectAccount($line->credit_account, 'credit_account') : $line->credit_account);
                    $glLines[] = [
            ...
            $this->recordSourceAudit($issue, 'inventory_issue.posted', $before, $issue->fresh()->toArray(), ['posting_mode' => $directPosting ? 'direct' : 'strict']);
    ...
    private function assertSufficientStock(InventoryIssue $issue): void
    {
        ...
        foreach ($issue->lines as $index => $line) {
            ...
            if ($warehouseId < 1) {
                $fallbackWarehouse = $line->item?->warehouse_id
                    ?? Warehouse::where('company_id', $issue->company_id)->value('id');
                if ($fallbackWarehouse) {
                    $warehouseId = (int) $fallbackWarehouse;
