...
class PeriodClosingService
{
    ...
    public function executeForCompany(int $companyId, array $data): JournalEntry
    {
        ...
        $periodStart = $periodToClose->start_date->toDateString();
        $periodEnd = $periodToClose->end_date->toDateString();
        $datesMustBelongToSelectedPeriod = [
            'from_date' => $fromDate,
            'to_date' => $toDate,
            'voucher_date' => $data['voucher_date'] ?? $toDate,
            'posting_date' => $data['posting_date'] ?? $toDate,
        ];
        $misalignedDates = array_filter(
            $datesMustBelongToSelectedPeriod,
            static fn (mixed $date): bool => (string) $date < $periodStart || (string) $date > $periodEnd,
        );
        if ($misalignedDates !== []) {
            throw ValidationException::withMessages([
                'period_id' => "Các ngày kết chuyển phải nằm trong chính kỳ kế toán {$periodStart} đến {$periodEnd} đã chọn.",
            ]);
        }
