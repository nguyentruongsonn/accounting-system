...
class InventoryReceiptService
{
    ...
    public function post($id, ?int $companyId = null)
    {
        ...
        return DB::transaction(function () use ($id, $companyId) {
            ...
            $accountMappings = ! $directPosting && $policy !== null ? $this->accountMappingGate->requireSatisfied($receipt, $policy) : null;
            ...
            $this->recordSourceAudit($receipt, 'inventory_receipt.posted', $before, $receipt->fresh()->toArray(), ['posting_mode' => $directPosting ? 'direct' : 'strict']);
