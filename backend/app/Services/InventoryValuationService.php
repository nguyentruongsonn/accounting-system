...
class InventoryValuationService
{
    ...
    public function runCostCalculation(array $params): array
    {
        ...
        $fromDate = ! empty($params['from_date']) ? Carbon::parse($params['from_date'])->toDateString() : now()->startOfMonth()->toDateString();
        $toDate = ! empty($params['to_date']) ? Carbon::parse($params['to_date'])->toDateString() : now()->endOfMonth()->toDateString();
        $warehouseId = $params['warehouse_id'] ?? null;
        $itemId = $params['item_id'] ?? null;

        // Costing rewrites issue values and their journals. Serialize the
        // company-level request and reuse the latest valid result for the
        // exact same scope; invalidated runs deliberately do not match.
        $this->periodGuard->assertRangeOpen($params['company_id'], $fromDate, $toDate, 'tính giá xuất kho');

        return DB::transaction(function () use ($params, $method, $fromDate, $toDate, $warehouseId, $itemId): array {
            DB::table('companies')->where('id', $params['company_id'])->lockForUpdate()->first();

            $existingRun = InventoryValuationRun::query()
            ...
            if ($existingRun !== null) {
                return $this->cachedCostCalculationResult($existingRun);
            }
    ...
    private function cachedCostCalculationResult(InventoryValuationRun $run): array
    {
        return [
            'success' => true,
            'method' => $run->method,
            ...
            'total_cost_amount' => (string) $run->total_cost_amount,
            'items' => [],
