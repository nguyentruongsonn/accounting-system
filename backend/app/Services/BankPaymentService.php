...
class BankPaymentService
{
    ...
    public function create(array $data): BankPayment
    {
        ...
        if (empty($data['bank_account_id'])) {
            if (! empty($data['bank_account_number'])) {
                $data['bank_account_id'] = BankAccount::where('company_id', $companyId)
                    ->where('account_number', $data['bank_account_number'])
                    ->value('id');
            }
            if (empty($data['bank_account_id'])) {
                $data['bank_account_id'] = BankAccount::where('company_id', $companyId)
        ...
        return DB::transaction(function () use ($data, $companyId) {
            ...
            $payment = BankPayment::create([
                ...
                'is_bulk_transfer' => $data['is_bulk_transfer'] ?? false,
    ...
    public function update($id, array $data): BankPayment
    {
        ...
        return DB::transaction(function () use ($id, $data) {
            ...
            $payment->update([
                'voucher_type' => array_key_exists('voucher_type', $data) ? $data['voucher_type'] : $payment->voucher_type,
                'payment_type' => array_key_exists('payment_type', $data) ? $data['payment_type'] : $payment->payment_type,
                'is_bulk_transfer' => array_key_exists('is_bulk_transfer', $data) ? (bool) $data['is_bulk_transfer'] : $payment->is_bulk_transfer,
                'contact_type' => array_key_exists('contact_type', $data) ? $data['contact_type'] : $payment->contact_type,
                'contact_id' => array_key_exists('contact_id', $data) ? $contactId : $payment->contact_id,
                'contact_name' => array_key_exists('contact_name', $data) ? $data['contact_name'] : $payment->contact_name,
                'bank_account_id' => array_key_exists('bank_account_id', $data) ? $data['bank_account_id'] : $payment->bank_account_id,
                'employee_id' => array_key_exists('employee_id', $data) ? $data['employee_id'] : $payment->employee_id,
                'employee_name' => array_key_exists('employee_name', $data) ? $data['employee_name'] : $payment->employee_name,
                'voucher_number' => array_key_exists('voucher_number', $data) ? $data['voucher_number'] : $payment->voucher_number,
                'voucher_date' => array_key_exists('voucher_date', $data) ? $data['voucher_date'] : $payment->voucher_date,
                'posting_date' => array_key_exists('posting_date', $data) ? $data['posting_date'] : $payment->posting_date,
                'payee_name' => array_key_exists('payee_name', $data) ? $data['payee_name'] : $payment->payee_name,
                'payee_address' => array_key_exists('payee_address', $data) ? $data['payee_address'] : $payment->payee_address,
                'payee_bank_account' => array_key_exists('payee_bank_account', $data) ? $data['payee_bank_account'] : $payment->payee_bank_account,
                'payee_bank_name' => array_key_exists('payee_bank_name', $data) ? $data['payee_bank_name'] : $payment->payee_bank_name,
                'payee_branch' => array_key_exists('payee_branch', $data) ? $data['payee_branch'] : $payment->payee_branch,
                'fee_bearer' => array_key_exists('fee_bearer', $data) ? $data['fee_bearer'] : $payment->fee_bearer,
                'description' => array_key_exists('description', $data) ? $data['description'] : (array_key_exists('reason', $data) ? $data['reason'] : $payment->description),
                'attached_docs' => array_key_exists('attached_docs', $data) ? $data['attached_docs'] : $payment->attached_docs,
                'currency' => array_key_exists('currency', $data) ? $data['currency'] : $payment->currency,
                'exchange_rate' => array_key_exists('exchange_rate', $data) ? $data['exchange_rate'] : $payment->exchange_rate,
                'amount' => $totalAmount ?: $payment->amount,
                'referenced_vouchers' => array_key_exists('referenced_vouchers', $data) ? $data['referenced_vouchers'] : $payment->referenced_vouchers,
                'updated_by' => $data['updated_by'] ?? auth()->id(),
            ]);
    ...
    public function post($id)
    {
        return DB::transaction(function () use ($id) {
            ...
            $policy = $directPosting
                ? null
                : (($this->requiresCashBankPolicy() || $this->requiresCashBankAccountMappings()) ? $this->accountingPolicyResolver->requireForVoucher((int) $payment->company_id, ($payment->posting_date ?? $payment->voucher_date ?? now())->toDateString(), SystemVoucherType::BANK_PAYMENT) : null);
            $approval = ($directPosting || ! $this->requiresCashBankApproval()) ? null : $this->approvalGate->requireApproved($payment, auth()->id());
            $accountMappings = ($directPosting || ! $this->requiresCashBankAccountMappings()) ? null : $this->accountMappingGate->requireSatisfied($payment, $policy);

            $glLines = [];
            foreach ($payment->lines as $line) {
                $debitAccount = $accountMappings !== null ? CashBankVoucherAccountMappingPostingGate::accountFor($accountMappings, $payment, 'debit', (string) $line->debit_account) : ($directPosting ? ($line->debit_account ?: '331') : $this->accountOrLegacyFallback($line->debit_account, 'debit_account', '331'));
                $creditAccount = $accountMappings !== null ? CashBankVoucherAccountMappingPostingGate::accountFor($accountMappings, $payment, 'credit', (string) $line->credit_account) : ($directPosting ? ($line->credit_account ?: '1121') : $this->accountOrLegacyFallback($line->credit_account, 'credit_account', '1121'));
                $glLines[] = [
