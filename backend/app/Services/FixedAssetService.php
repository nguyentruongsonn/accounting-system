...
use App\Services\AccountingPostingMode;
use Carbon\Carbon;
use Exception;
...
class FixedAssetService
{
        ...
        return DB::transaction(function () use ($data) {
            ...
            if ($accumulatedDepreciation > $originalCost) {
                throw ValidationException::withMessages([
                    'accumulated_depreciation' => 'Hao mòn lũy kế không được lớn hơn nguyên giá.',
                ]);
            }
            $netValue = $originalCost - $accumulatedDepreciation;
    ...
    public function transferAsset(int $companyId, int $id, array $data): array
    {
        return DB::transaction(function () use ($companyId, $id, $data): array {
            $asset = $this->assetForCompany($companyId, $id);
            $this->periodMutationGuard->assertNewEvent($asset, $data, 'lập chứng từ điều chuyển tài sản cố định');
            ...
            $toDepartment = trim((string) ($data['to_department_code'] ?? ''));
            if ($toDepartment === '') {
                throw ValidationException::withMessages([
                    'to_department_code' => 'Bộ phận nhận điều chuyển là bắt buộc.',
                ]);
            }
            if ($toDepartment === (string) $asset->department_code) {
                throw ValidationException::withMessages([
                    'to_department_code' => 'Bộ phận nhận phải khác bộ phận hiện tại.',
                ]);
            }

            $before = $asset->toArray();
            $asset->department_code = $toDepartment;
            $asset->updated_by = auth()->id();
            $asset->save();
            $after = array_merge($asset->fresh()->toArray(), [
                'transfer' => [
                    'voucher_number' => $data['voucher_number'] ?? null,
                    'voucher_date' => $data['voucher_date'] ?? now()->toDateString(),
                    'reason' => $data['reason'] ?? null,
                    'from_department_code' => $before['department_code'] ?? null,
                    'to_department_code' => $toDepartment,
                ],
            ]);
            $event = $this->recordAudit($asset, 'fixed_asset.transferred', $before, $after, [
                'reason' => $data['reason'] ?? null,
                'voucher_number' => $data['voucher_number'] ?? null,
                'voucher_date' => $data['voucher_date'] ?? now()->toDateString(),
                'from_department_code' => $before['department_code'] ?? null,
            ...
            return [
                ...
                'before' => $before,
                'after' => $after,
                'asset' => $asset->fresh(['supplier', 'journalEntry']),
            ];
        });
    }

    /** @return array<string, mixed> */
    public function inventoryAsset(int $companyId, int $id, array $data): array
    {
        return DB::transaction(function () use ($companyId, $id, $data): array {
            $asset = $this->assetForCompany($companyId, $id);
            $this->periodMutationGuard->assertNewEvent($asset, $data, 'lập biên bản kiểm kê tài sản cố định');

            if ($asset->status === 'disposed') {
                throw new Exception("Tài sản {$asset->asset_code} đã được thanh lý và không thể kiểm kê.");
            }

            $countedQuantity = (int) ($data['counted_quantity'] ?? $asset->quantity ?? 1);
            $countedNetValue = round((float) ($data['counted_net_value'] ?? $asset->net_value), 2);
            if ($countedQuantity < 0 || $countedNetValue < 0) {
                throw ValidationException::withMessages([
                    'counted_net_value' => 'Số lượng và giá trị kiểm kê không được âm.',
                ]);
            }

            $before = $asset->toArray();
            $bookQuantity = (int) ($asset->quantity ?: 1);
            $bookNetValue = round((float) $asset->net_value, 2);
            $variance = [
                'quantity' => $countedQuantity - $bookQuantity,
                'net_value' => round($countedNetValue - $bookNetValue, 2),
            ];
            $requiresFollowUp = $variance['quantity'] !== 0 || abs($variance['net_value']) > 0.01;
            $after = array_merge($before, [
                'inventory' => [
                    'counted_quantity' => $countedQuantity,
                    'counted_net_value' => $countedNetValue,
                    'book_quantity' => $bookQuantity,
                    'book_net_value' => $bookNetValue,
                    'variance' => $variance,
            ...
            $event = $this->recordAudit($asset, 'fixed_asset.inventoried', $before, $after, [
                'reason' => $data['reason'] ?? null,
                'voucher_number' => $data['voucher_number'] ?? null,
                'voucher_date' => $data['voucher_date'] ?? now()->toDateString(),
                'variance' => $variance,
    ...
    public function runMonthlyDepreciation(int $companyId, string $month, ?string $description = null): DepreciationLog
    {
        return DB::transaction(function () use ($companyId, $month, $description) {
            ...
            foreach ($preview['lines'] as $lineData) {
                ...
                $asset->net_value = max(0, floatval($asset->original_cost) - floatval($asset->accumulated_depreciation));
    ...
    {
        if ($this->postingMode->isDirectPostingEnabled()) {
            return;
        }
    ...
    private function recordAudit(Model $model, string $action, array $before, array $after, array $metadata = []): Model
    {
        return $this->auditService->record($model, $action, $before, $after, null, array_merge([
            'domain' => 'fixed_assets',
            'operation' => $action,
        ], $metadata));
    }
