...
class SalesInvoiceService
{
    ...
    public function create(array $data): SalesInvoice
    {
        return DB::transaction(function () use ($data) {
            ...
            $invoice = SalesInvoice::create([
                ...
                'payment_method_display' => array_key_exists('payment_method_display', $data) ? $data['payment_method_display'] : null,
                'bank_account' => array_key_exists('bank_account', $data) ? $data['bank_account'] : null,
                ...
                'invoice_form_number' => array_key_exists('invoice_form_number', $data) ? $data['invoice_form_number'] : null,
                'delivery_voucher_number' => $data['delivery_voucher_number'] ?? null,
                'invoice_date' => $data['invoice_date'] ?? now()->toDateString(),
                'accounting_date' => $accountingDate,
                'due_date' => $data['due_date'] ?? now()->addDays(30)->toDateString(),
                'payment_terms' => array_key_exists('payment_terms', $data) ? $data['payment_terms'] : null,
                'due_days' => array_key_exists('due_days', $data) ? ($data['due_days'] !== null ? (int) $data['due_days'] : null) : null,
                'budget_relation_code' => array_key_exists('budget_relation_code', $data) ? $data['budget_relation_code'] : null,
                'id_card_number' => array_key_exists('id_card_number', $data) ? $data['id_card_number'] : null,
                'passport_number' => array_key_exists('passport_number', $data) ? $data['passport_number'] : null,
                'sub_total' => $subTotal,
    ...
    public function update(int $id, array $data): SalesInvoice
    {
        return DB::transaction(function () use ($id, $data) {
            ...
            $invoice = $this->scopeToActorCompany(SalesInvoice::with('lines.item'))
                ->lockForUpdate()
                ->findOrFail($id);
            $companyId = (int) $invoice->company_id;
            $input = static function (string $key, mixed $fallback) use ($data): mixed {
            ...
            CommercialTenantReferenceGuard::assertSalesInvoice([
                ...
                'employee_id' => $input('employee_id', $invoice->employee_id),
            ], $companyId);
            ...
            $invoice->update([
                ...
                'customer_address' => $input('customer_address', $invoice->customer_address),
                'receiver_name' => $input('receiver_name', $invoice->receiver_name),
                'employee_id' => $input('employee_id', $invoice->employee_id),
                'employee_name' => $input('employee_name', $invoice->employee_name),
                'voucher_type' => $input('voucher_type', $invoice->voucher_type),
                'payment_method' => $paymentMethod,
                'payment_method_display' => $input('payment_method_display', $invoice->payment_method_display),
                'bank_account' => $input('bank_account', $invoice->bank_account),
                'invoice_number' => $input('invoice_number', $invoice->invoice_number),
                'invoice_symbol' => $input('invoice_symbol', $invoice->invoice_symbol),
                'invoice_code' => $input('invoice_code', $invoice->invoice_code),
                'invoice_form_number' => $input('invoice_form_number', $invoice->invoice_form_number),
                'delivery_voucher_number' => $input('delivery_voucher_number', $invoice->delivery_voucher_number),
                'invoice_date' => $input('invoice_date', $invoice->invoice_date),
                'accounting_date' => $input('accounting_date', $invoice->accounting_date),
                'due_date' => $input('due_date', $invoice->due_date),
                'payment_terms' => $input('payment_terms', $invoice->payment_terms),
                'due_days' => $input('due_days', $invoice->due_days) !== null ? (int) $input('due_days', $invoice->due_days) : null,
                'budget_relation_code' => $input('budget_relation_code', $invoice->budget_relation_code),
                'id_card_number' => $input('id_card_number', $invoice->id_card_number),
                'passport_number' => $input('passport_number', $invoice->passport_number),
                'sub_total' => $subTotal,
                'discount_amount' => $discountAmount,
                'tax_amount' => $taxAmount,
                'total_amount' => $totalAmount,
                ...
                'attached_docs' => $input('attached_docs', $invoice->attached_docs),
                'currency' => $input('currency', $invoice->currency),
                'exchange_rate' => $input('exchange_rate', $invoice->getRawOriginal('exchange_rate') ?? 1),
                'functional_currency_code' => $input('functional_currency_code', $invoice->functional_currency_code),
                'functional_total_amount_raw' => $input('functional_total_amount_raw', $invoice->functional_total_amount_raw),
                'functional_total_amount_scale' => $input('functional_total_amount_scale', $invoice->functional_total_amount_scale),
                'original_total_amount_raw' => $input('original_total_amount_raw', $invoice->original_total_amount_raw),
                'original_total_amount_scale' => $input('original_total_amount_scale', $invoice->original_total_amount_scale),
                'is_export_slip' => $isExportSlip,
                'referenced_vouchers' => $input('referenced_vouchers', $invoice->referenced_vouchers),
                'updated_by' => auth()->id(),
            ]);
            ...
            if (array_key_exists('referenced_vouchers', $data)) {
                $invoice->syncReferences($data['referenced_vouchers']);
            }
    ...
    public function post($id)
    {
        return DB::transaction(function () use ($id) {
            ...
            $receivableAccount = $accountMappings === null
                ? ($invoice->lines->first()?->debit_account ?: SalesInvoiceAccountMappingPostingGate::legacySettlementAccount($invoice))
    ...
    public function void($id, string $auditEvent = 'voided')
    {
        return DB::transaction(function () use ($id, $auditEvent) {
            $invoice = $this->scopeToActorCompany(SalesInvoice::with('lines'))->findOrFail($id);
    ...
    /**
     * A sales invoice marked as an export slip is also the stock-out source
     * document. Keep the quantity/value movement in the append-only movement
     * ledger so stock reports do not depend on a separate hand-created issue.
     */
    private function recordInventoryMovements(SalesInvoice $invoice): void
    {
        if (! $invoice->is_export_slip) {
            return;
        }
        ...
        foreach ($invoice->lines as $index => $line) {
            $itemType = strtolower(trim((string) ($line->item?->type ?? '')));
            if ($line->item_id === null || in_array($itemType, ['service', 'dịch vụ'], true)) {
                continue;
            }

            $warehouseId = (int) ($line->warehouse_id ?: $line->item?->warehouse_id ?: 0);
            if ($warehouseId < 1) {
                // Keep the common single-warehouse workflow ergonomic. Once a
                // tenant has more than one warehouse, the line must carry an
                // explicit choice so stock cannot be assigned ambiguously.
                $companyWarehouses = Warehouse::query()
                    ->where('company_id', $invoice->company_id)
                    ->orderBy('id')
                    ->pluck('id');
                $warehouseId = $companyWarehouses->count() === 1 ? (int) $companyWarehouses->first() : 0;
            }
            if ($warehouseId < 1) {
                throw ValidationException::withMessages([
                    "lines.{$index}.warehouse_id" => 'Hóa đơn kiêm phiếu xuất kho phải xác định kho cho từng dòng hàng.',
                ]);
            }

            if (! Warehouse::query()->where('company_id', $invoice->company_id)->whereKey($warehouseId)->exists()) {
                throw ValidationException::withMessages([
                    "lines.{$index}.warehouse_id" => 'Kho xuất không thuộc doanh nghiệp của hóa đơn.',
                ]);
            }
            ...
            if (DecimalMoney::compare($quantity, DecimalMoney::ZERO) <= 0) {
                throw ValidationException::withMessages([
                    "lines.{$index}.quantity" => 'Số lượng xuất kho phải lớn hơn 0.',
                ]);
            }

            $cogsAmount = $this->persistedMoney($line, 'cogs_amount');
            if (DecimalMoney::compare($cogsAmount, DecimalMoney::ZERO) === 0) {
                $cogsAmount = $this->multiplyMoney($quantity, $this->persistedMoney($line, 'cogs_price'));
            }
            ...
            $postingCycle = ((int) (InventoryMovementEvent::query()
                ...
                ->where('movement_type', 'sales_invoice_out')
                ->max('posting_cycle'))) + 1;
            ...
            InventoryMovementEvent::create([
                ...
                'quantity_delta' => DecimalMoney::negate($quantity),
                'amount_delta' => DecimalMoney::negate($cogsAmount),
    ...
    /** Reverse the exact stock-out event cycle when a posted invoice is voided/unposted. */
    private function reverseInventoryMovements(SalesInvoice $invoice): void
    {
        ...
        foreach ($events as $event) {
            $alreadyReversed = InventoryMovementEvent::query()
                ...
                ->where('movement_type', 'sales_invoice_out_reversal')
            ...
            InventoryMovementEvent::create([
                ...
                'movement_type' => 'sales_invoice_out_reversal',
    ...
    private function requiresAccountingPolicy(): bool
    {
        return ! $this->postingMode->isDirectPostingEnabled()
            && (bool) config('accounting.enforce_sales_invoice_posting_policy', true);
    }

    private function requiresAccountMappingPosting(): bool
    {
        return ! $this->postingMode->isDirectPostingEnabled()
            && (bool) config('accounting.enforce_sales_invoice_posting_account_mappings', true);
    }

    private function requiresDimensionPosting(): bool
    {
        return ! $this->postingMode->isDirectPostingEnabled()
            && (bool) config('accounting.enforce_sales_invoice_posting_dimensions', true);
    }
