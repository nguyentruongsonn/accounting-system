...
final class AccountingAuditTrailExplorer
{
    ...
    {
        $companyId = $this->companyId($companyId);
        $query = AuditLog::withoutGlobalScope('company')
            ->where('company_id', $companyId)
            ->with(['user' => fn ($user) => $user->where('company_id', $companyId)->select('id', 'name', 'email')])
            ->orderByDesc('id');
        $actorId = $filters['actor_id'] ?? $filters['actor'] ?? null;
        if ($actorId !== null && $actorId !== '') {
            $query->where('user_id', $actorId);
        }
        if (! empty($filters['module'])) {
            $prefixes = $this->modulePrefixes((string) $filters['module']);
            $query->where(function ($moduleQuery) use ($prefixes): void {
                foreach ($prefixes as $index => $prefix) {
                    $method = $index === 0 ? 'where' : 'orWhere';
                    $pattern = addcslashes($prefix, "\\%_").'.%';
                    $moduleQuery->{$method.'Raw'}("action LIKE ? ESCAPE '\\'", [$pattern]);
                }
            });
        }
    ...
    private function auditSummary(AuditLog $log): array
    {
        return ['id' => $log->id, 'occurred_at' => optional($log->created_at)->toIso8601String(), 'action' => $log->action,
            'module' => $this->moduleFromAction((string) $log->action), 'entity_type' => $log->model_type,
            'entity_id' => (string) $log->model_id, 'actor_id' => $log->user_id,
            'actor_name' => $log->user?->name, 'actor_email' => $log->user?->email,
    ...
    private function moduleFromAction(string $action): string
    {
        ...
        return match (true) {
            str_starts_with($prefix, 'purchase') => 'purchase',
            str_starts_with($prefix, 'sales') => 'sales',
            str_starts_with($prefix, 'cash') => 'cash',
            str_starts_with($prefix, 'bank') => 'bank',
            str_starts_with($prefix, 'inventory') => 'inventory',
            str_starts_with($prefix, 'journal') => 'journal',
            str_starts_with($prefix, 'user') => 'user',
            str_starts_with($prefix, 'reconciliation') => 'reconciliation',
            str_starts_with($prefix, 'period_close') => 'period_close',
            default => $prefix,
        };
    }

    /** @return list<string> */
    private function modulePrefixes(string $module): array
    {
        return match (strtolower($module)) {
            'purchase' => ['purchase', 'purchase_invoice', 'purchase_order', 'purchase_return'],
            'sales' => ['sales', 'sales_invoice', 'sales_order', 'sales_return'],
            'cash' => ['cash', 'cash_receipt', 'cash_payment'],
            'bank' => ['bank', 'bank_receipt', 'bank_payment'],
            'inventory' => ['inventory', 'inventory_receipt', 'inventory_issue'],
            'reconciliation' => ['reconciliation', 'reconciliation_run'],
            'period_close' => ['period_close', 'period_close_signoff_package'],
