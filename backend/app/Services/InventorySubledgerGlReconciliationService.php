...
final class InventorySubledgerGlReconciliationService
{
    ...
    private function calculateControlled(int $companyId, string $cutoff, CarbonImmutable $inputCutoffAt, array $contract, ?AccountingPolicyVersion $policy): array
    {
        ...
        $events = InventoryMovementEvent::query()->where('company_id', $companyId)->whereDate('movement_date', '<=', $cutoff)->where('updated_at', '<=', $inputCutoffAt)->orderBy('id')->get(['id', 'item_id', 'warehouse_id', 'movement_type', 'quantity_delta', 'amount_delta']);
        foreach ($events as $event) {
            $isSalesOut = str_starts_with((string) $event->movement_type, 'sales_invoice_out');
            if (! $this->validWarehouseLineage($companyId, $event->item_id, $event->warehouse_id)) {
                $exceptions[] = $this->exception('inventory_movement_lineage_invalid', 'Inventory movement event is not tenant-bound.', ['inventory_movement_events', 'items', 'warehouses'], ['event_id' => (int) $event->id]);
                continue;
            }
            if ($event->amount_delta === null) {
                $exceptions[] = $this->exception($isSalesOut ? 'inventory_sales_value_unavailable' : 'inventory_transfer_value_unavailable', $isSalesOut ? 'A sales stock-out movement has no carrying value evidence.' : 'A transfer movement has no carrying value; run weighted-average valuation before reconciliation.', ['inventory_movement_events', 'inventory_valuation_runs'], ['event_id' => (int) $event->id]);
                continue;
            }
            ...
            $kind = $isSalesOut ? 'sales_out' : 'transfer';
            $this->addRollforward($rollforward, $event->item_id, $event->warehouse_id, $kind, $this->decimalMoney($event->amount_delta), (string) $event->quantity_delta);
            $isSalesOut ? $salesOutCount++ : $transferCount++;
        }
        ...
        foreach ($rollforwardRows as &$row) {
            ...
            $row['ending_value'] = DecimalMoney::add($row['ending_value'], $row['sales_out_value']);
            ...
            $row['ending_quantity'] = $this->quantityAdd($row['ending_quantity'], $row['sales_out_quantity']);
        ...
        return ['snapshot' => [
            ...
            'amounts' => ['subledger_ending_value' => $endingValue, 'gl_inventory_balance' => $gl, 'difference' => $difference, 'ending_quantity' => $endingQuantity, 'receipt_line_count' => $receiptCount, 'issue_line_count' => $issueCount, 'transfer_event_count' => $transferCount, 'sales_out_event_count' => $salesOutCount, 'gl_control_line_count' => $glLineCount, 'opening_subledger_value' => $opening['value'], 'opening_gl_value' => $openingGl, 'item_warehouse_rollforward' => $rollforwardRows],
            ...
            'statement' => $exceptions === [] ? 'Controlled inventory subledger-to-GL tie-out computed from confirmed opening stock, posted receipt/issue lines, sales stock-out events and valued transfer events.' : 'Controlled inventory reducer found blocking exceptions; no close authority is granted.',
    ...
    private function addRollforward(array &$rollforward, mixed $itemId, mixed $warehouseId, string $kind, string $value, string $quantity): void
    {
        ...
        if (! isset($rollforward[$key])) {
            $rollforward[$key] = [
                ...
                'sales_out_value' => DecimalMoney::ZERO,
                'sales_out_quantity' => '0.0000',
            ];
        }
    ...
    private function sourceJournalLineage(int $companyId, string $cutoff): array
    {
        ...
        $salesRows = SalesInvoice::withoutGlobalScope('company')->where('company_id', $companyId)->where('is_posted', true)->where('is_export_slip', true)
            ->whereDate('invoice_date', '<=', $cutoff)->get(['id', 'journal_entry_id']);
        foreach ($salesRows as $row) $sources[] = ['class' => SalesInvoice::class, 'type' => 'sales_invoice_export', 'id' => (int) $row->id, 'journal_entry_id' => $row->journal_entry_id];
        $valid = 0; $invalid = [];
    ...
    private function sourceCompleteness(array $adapterObservation, array $lineage, bool $hasApprovedContract, bool $mappingAvailable): array
    {
        $schemaAvailable = Schema::hasTable('inventory_receipts')
            ...
            && Schema::hasColumns('sales_invoices', ['company_id', 'id', 'is_posted', 'is_export_slip', 'invoice_date', 'journal_entry_id'])
            && Schema::hasTable('sales_invoice_lines')
            && Schema::hasColumns('sales_invoice_lines', ['sales_invoice_id', 'item_id', 'warehouse_id', 'quantity', 'cogs_amount'])
            && Schema::hasTable('inventory_movement_events')
            && Schema::hasColumns('inventory_movement_events', ['company_id', 'movement_date', 'warehouse_id', 'item_id', 'movement_type', 'posting_cycle', 'quantity_delta', 'amount_delta', 'source_type', 'source_id', 'source_line_id'])
        ...
        return [
            'inventory_movement_source_schema' => $this->condition($schemaAvailable, 'inventory_gl_source_schema_unavailable', 'The inventory receipt/issue, sales export, movement event, line, or posted-journal source-lineage schema is unavailable.', ['schema_available' => $schemaAvailable]),
