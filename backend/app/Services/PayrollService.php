...
class PayrollService
{
    ...
    public function create(int $companyId, array $data): Payroll
    {
        return DB::transaction(function () use ($companyId, $data) {
            $companyId = $this->requireCompanyId($companyId);
            $isDirect = $this->postingMode->isDirectPostingEnabled();
            if ($isDirect && isset($data['lines']) && is_array($data['lines'])) {
                foreach ($data['lines'] as $k => $line) {
                    if (empty($line['debit_account'])) {
                        $data['lines'][$k]['debit_account'] = '6421';
                    }
                    if (empty($line['credit_account'])) {
                        $data['lines'][$k]['credit_account'] = '334';
                    }
                }
            }
            ...
            foreach ($data['lines'] as $index => $line) {
                ...
                $payroll->lines()->create([
                    ...
                    'debit_account' => trim((string) ($line['debit_account'] ?? ($isDirect ? '6421' : ''))),
                    'credit_account' => trim((string) ($line['credit_account'] ?? ($isDirect ? '334' : ''))),
                ]);
            }
    ...
    public function post(int $companyId, int $id): Payroll
    {
        return DB::transaction(function () use ($companyId, $id) {
            ...
            foreach ($payroll->lines as $line) {
                $debitAcc = trim((string) $line->debit_account) ?: ($isDirect ? '6421' : '');
                $creditAcc = trim((string) $line->credit_account) ?: ($isDirect ? '334' : '');
                $glLines[] = [
                    'account_code' => $this->journalEntryService->resolveLeafAccount($companyId, $debitAcc),
                    'description' => 'Chi phí lương '.$payroll->month.' - '.$line->employee_name,
                    'debit_amount' => $line->net_salary,
                    'credit_amount' => 0,
                ];
                $glLines[] = [
                    'account_code' => $this->journalEntryService->resolveLeafAccount($companyId, $creditAcc),
