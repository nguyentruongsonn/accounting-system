...
final class SalesInvoiceCollectionService
{
    ...
    public function collect(User $actor, int $invoiceId, array $data): array
    {
        ...
        return DB::transaction(function () use ($actor, $companyId, $invoiceId, $data): array {
            ...
            // Do not rely only on the allocation service's later guard: a
            // retry must be rejected before it can create a receipt outside
            // the selected open accounting period.
            $this->periodGuard->assertOpen($companyId, $postingDate, 'thu tiền khách hàng');

            $amount = $this->paymentAmount($data['amount_raw'] ?? null, $this->settlementScale($data['amount_scale'] ?? 0));
            $integerAmount = $this->integerAmount($amount);
            $existingReceipt = CashReceipt::withoutGlobalScope('company')
                ->where('company_id', $companyId)
                ->where('voucher_number', $voucherNumber)
                ->lockForUpdate()
                ->first();
            if ($existingReceipt !== null) {
                $existingLine = $existingReceipt->lines()->first();
                $existingAllocation = SettlementAllocation::withoutGlobalScope('company')
                    ->where('company_id', $companyId)
                    ->where('source_document_type', 'cash_receipt')
                    ->where('source_document_id', $existingReceipt->id)
                    ->where('target_document_type', 'sales_invoice')
                    ->where('target_document_id', $invoice->id)
                    ->where('allocation_kind', 'settlement')
                    ->where('allocation_direction', 'reduction')
                    ->where('status', 'posted')
                    ->lockForUpdate()
                    ->first();

                $requestedCurrency = strtoupper((string) ($data['currency'] ?? $invoice->currency ?? 'VND'));
                $samePayload = $existingReceipt->is_posted
                    && (int) $existingReceipt->contact_id === (int) $invoice->customer_id
                    && optional($existingReceipt->voucher_date)->toDateString() === $voucherDate
                    && optional($existingReceipt->posting_date)->toDateString() === $postingDate
                    && strtoupper((string) ($existingReceipt->currency ?? 'VND')) === $requestedCurrency
                    && $existingLine !== null
                    && (int) $existingLine->invoice_id === (int) $invoice->id
                    && (string) $existingLine->debit_account === trim((string) ($data['debit_account'] ?? ''))
                    && (string) $existingLine->credit_account === trim((string) ($data['credit_account'] ?? ''))
                    && (string) $existingLine->amount === $integerAmount
                    && $existingAllocation !== null
                    && (string) $existingAllocation->amount_raw === $integerAmount
                    && (int) $existingAllocation->amount_scale === 0;

                if ($samePayload) {
                    return [
                        'receipt' => $existingReceipt->load(['lines', 'references']),
                ...
                throw ValidationException::withMessages([
                    'voucher_number' => 'Số phiếu thu đã tồn tại với dữ liệu khác; không thể ghi đè chứng từ đã ghi sổ.',
                ]);
            }
            ...
            $receipt = $this->cashReceipts->create([
                ...
                'lines' => [[
                    ...
                    // Customer and supplier ids are independent sequences;
                    // retain the kind for the tenant reference guard.
                    'line_contact_type' => 'customer',
