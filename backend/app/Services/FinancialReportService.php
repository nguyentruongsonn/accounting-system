...
class FinancialReportService
{
    ...
    public function getAccountBalances(int $companyId, ?string $fromDate = null, ?string $toDate = null, bool $excludeClosing = false): array
    {
        ...
        $this->excludeLegacyOpeningJournal($query, $companyId);
        ...
        // Cumulative/as-of reports include the opening package, even while it
        // remains a draft, so internal reports can expose reconciliation gaps.
        // Period-turnover queries (fromDate present) deliberately do not.
        if ($fromDate === null) {
            $this->mergeOpeningBalances($balanceMap, $companyId, $toDate);
        }
    ...
    public function getTrialBalance(int $companyId, ?string $fromDate = null, ?string $toDate = null)
    {
        ...
        if ($fromDate) {
            ...
            $openingBalanceQuery = DB::table('journal_entry_lines as l')
                ->join('journal_entries as e', 'l.journal_entry_id', '=', 'e.id')
                ->where('e.company_id', $companyId)
                ->where('e.status', 'posted')
                ->whereNull('e.deleted_at')
                ->where('e.posting_date', '<', $fromDate);
            $this->excludeLegacyOpeningJournal($openingBalanceQuery, $companyId);
            $openingBalances = $openingBalanceQuery
            ...
            $this->mergeOpeningBalances($openingMap, $companyId, $fromDate);
        }
    ...
    private function mergeOpeningBalances(array &$target, int $companyId, ?string $cutoff, ?string $from = null): void
    {
        ...
        $query = DB::table('opening_balance_account_lines as o')
            ...
            ->whereIn('p.status', ['draft', 'confirmed']);
        if ($cutoff !== null) {
            $query->whereDate('p.effective_date', '<=', $cutoff);
        }
        if ($from !== null) {
            $query->whereDate('p.effective_date', '>=', $from);
        }
        $rows = $query->select(
            'o.account_code',
    ...
    public function getBalanceSheet(int $companyId, ?string $toDate = null, ?string $fromDate = null): array
    {
        ...
        if ($fromDate) {
            // A package effective on the first day of the report is the
            // opening position, not a period movement. Merge only packages
            // effective on that boundary to avoid double counting older data.
            $this->mergeOpeningBalances($startMap, $companyId, $fromDate, $fromDate);
        }
    ...
    public function getGeneralJournal(int $companyId, string $fromDate, string $toDate): array
    {
        ...
        ";
        if ($this->hasOpeningBalancePackage($companyId)) {
            $query = str_replace(
                "AND e.posting_date BETWEEN ? AND ?",
                "AND e.posting_date BETWEEN ? AND ?\n            AND e.voucher_number NOT LIKE 'SDDK-%'\n            AND (e.source_document_type IS NULL OR e.source_document_type <> 'opening_balance')",
                $query,
            );
        }
    ...
    public function getGeneralLedger(int $companyId, string $accountCode, string $fromDate, string $toDate): array
    {
        ...
        $matchingLines = DB::table('journal_entry_lines as l')
            ...
            ->get();
        if ($this->hasOpeningBalancePackage($companyId)) {
            $matchingLines = $matchingLines->reject(fn (object $line): bool => str_starts_with((string) $line->voucher_number, 'SDDK-')
                || (string) $line->source_document_type === 'opening_balance');
        }
    ...
    public function getGeneralLedgerOpeningBalance(int $companyId, string $accountCode, string $fromDate): array
    {
        ...
        // Include a package effective on the first day of the
        // selected period in the ledger's opening row.
        $this->mergeOpeningBalances($balanceMap, $companyId, $fromDate, $fromDate);
        $debit = DecimalMoney::ZERO;
        $credit = DecimalMoney::ZERO;
    ...
    private function hasOpeningBalancePackage(int $companyId): bool
    {
        return Schema::hasTable('opening_balance_packages')
            && DB::table('opening_balance_packages')
    ...
    private function excludeLegacyOpeningJournal($query, int $companyId): void
    {
        if (! $this->hasOpeningBalancePackage($companyId)) {
            return;
        }
        ...
        $query->where(function ($nested): void {
            $nested->where('e.voucher_number', 'not like', 'SDDK-%')
                ->where(function ($source): void {
                    $source->whereNull('e.source_document_type')
                        ->orWhere('e.source_document_type', '!=', 'opening_balance');
                });
        });
    }
