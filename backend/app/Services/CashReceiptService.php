...
class CashReceiptService
{
    ...
    public function create(array $data): CashReceipt
    {
        ...
        return DB::transaction(function () use ($data) {
            ...
            if (empty($voucherNumber)) {
                $voucherNumber = $this->getNextCode((int) $data['company_id']);
            }
    ...
    public function post($id)
    {
        return DB::transaction(function () use ($id) {
            ...
            $policy = $directPosting
                ? null
                : (($this->requiresCashBankPolicy() || $this->requiresCashBankAccountMappings())
                    ? $this->accountingPolicyResolver->requireForVoucher((int) $receipt->company_id, ($receipt->posting_date ?? $receipt->voucher_date ?? now())->toDateString(), SystemVoucherType::CASH_RECEIPT)
                    : null);
            $accountMappings = $directPosting || ! $this->requiresCashBankAccountMappings()
                ? null
                : $this->accountMappingGate->requireSatisfied($receipt, $policy);

            $glLines = [];
            foreach ($receipt->lines as $line) {
                $debitAccount = $accountMappings !== null
                    ? CashBankVoucherAccountMappingPostingGate::accountFor($accountMappings, $receipt, 'debit', (string) $line->debit_account)
                    : ($directPosting ? $this->requireDirectAccount($line->debit_account, 'debit_account') : $this->accountOrLegacyFallback($line->debit_account, 'debit_account', '1111'));
                $creditAccount = $accountMappings !== null
                    ? CashBankVoucherAccountMappingPostingGate::accountFor($accountMappings, $receipt, 'credit', (string) $line->credit_account)
                    : ($directPosting ? $this->requireDirectAccount($line->credit_account, 'credit_account') : $this->accountOrLegacyFallback($line->credit_account, 'credit_account', '131'));
                $glLines[] = [
            ...
            $this->recordSourceAudit($receipt, 'cash_receipt.posted', $before, $receipt->fresh()->toArray(), array_filter(['accounting_policy' => $policy, 'posting_authorization' => $authorization, 'account_mappings' => $accountMappings, 'posting_mode' => $directPosting ? 'direct' : null]));
    ...
    private function requireDirectAccount(?string $account, string $field): string
    {
        $account = trim((string) $account);
        if ($account === '') {
            throw ValidationException::withMessages([
                $field => 'Chế độ ghi sổ trực tiếp yêu cầu tài khoản Nợ/Có đã được lưu trên chứng từ.',
            ]);
        }
    ...
    protected function sanitizeReceiptData(array $data): array
    {
        ...
        if (! empty($data['employee_id'])) {
            ...
            if ($emp) {
                $data['employee_name'] = $data['employee_name'] ?? $emp->name;
            } elseif ($isExactThuKhachHang) {
                // Keep legacy sanitization for an invalid employee reference,
                // but never discard a valid employee selected on the form.
