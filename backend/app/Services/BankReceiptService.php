...
class BankReceiptService
{
    ...
    public function update($id, array $data): BankReceipt
    {
        ...
        return DB::transaction(function () use ($id, $data) {
            ...
            $receipt->update([
                'voucher_type' => array_key_exists('voucher_type', $data) ? $data['voucher_type'] : $receipt->voucher_type,
                'contact_type' => array_key_exists('contact_type', $data) ? $data['contact_type'] : $receipt->contact_type,
                'contact_id' => array_key_exists('contact_id', $data) ? $contactId : $receipt->contact_id,
                'contact_name' => array_key_exists('contact_name', $data) ? $data['contact_name'] : $receipt->contact_name,
                'bank_account_id' => array_key_exists('bank_account_id', $data) ? $data['bank_account_id'] : $receipt->bank_account_id,
                'employee_id' => array_key_exists('employee_id', $data) ? $data['employee_id'] : $receipt->employee_id,
                'employee_name' => array_key_exists('employee_name', $data) ? $data['employee_name'] : $receipt->employee_name,
                'voucher_number' => array_key_exists('voucher_number', $data) ? $data['voucher_number'] : $receipt->voucher_number,
                'voucher_date' => array_key_exists('voucher_date', $data) ? $data['voucher_date'] : $receipt->voucher_date,
                'posting_date' => array_key_exists('posting_date', $data) ? $data['posting_date'] : $receipt->posting_date,
                'payer_name' => array_key_exists('payer_name', $data) ? $data['payer_name'] : $receipt->payer_name,
                'payer_address' => array_key_exists('payer_address', $data) ? $data['payer_address'] : $receipt->payer_address,
                'payer_bank_account' => array_key_exists('payer_bank_account', $data) ? $data['payer_bank_account'] : $receipt->payer_bank_account,
                'description' => array_key_exists('description', $data) ? $data['description'] : (array_key_exists('reason', $data) ? $data['reason'] : $receipt->description),
                'attached_docs' => array_key_exists('attached_docs', $data) ? $data['attached_docs'] : $receipt->attached_docs,
                'currency' => array_key_exists('currency', $data) ? $data['currency'] : $receipt->currency,
                'exchange_rate' => array_key_exists('exchange_rate', $data) ? $data['exchange_rate'] : $receipt->exchange_rate,
                'amount' => $totalAmount ?: $receipt->amount,
                'referenced_vouchers' => array_key_exists('referenced_vouchers', $data) ? $data['referenced_vouchers'] : $receipt->referenced_vouchers,
                'updated_by' => $data['updated_by'] ?? auth()->id(),
            ]);
    ...
    public function post($id)
    {
        return DB::transaction(function () use ($id) {
            ...
            $policy = $directPosting
                ? null
                : (($this->requiresCashBankPolicy() || $this->requiresCashBankAccountMappings()) ? $this->accountingPolicyResolver->requireForVoucher((int) $receipt->company_id, ($receipt->posting_date ?? $receipt->voucher_date ?? now())->toDateString(), SystemVoucherType::BANK_RECEIPT) : null);
            $approval = ($directPosting || ! $this->requiresCashBankApproval()) ? null : $this->approvalGate->requireApproved($receipt, auth()->id());
            $accountMappings = ($directPosting || ! $this->requiresCashBankAccountMappings()) ? null : $this->accountMappingGate->requireSatisfied($receipt, $policy);

            $glLines = [];
            foreach ($receipt->lines as $line) {
                $debitAccount = $accountMappings !== null ? CashBankVoucherAccountMappingPostingGate::accountFor($accountMappings, $receipt, 'debit', (string) $line->debit_account) : ($directPosting ? ($line->debit_account ?: '1121') : $this->accountOrLegacyFallback($line->debit_account, 'debit_account', '1121'));
                $creditAccount = $accountMappings !== null ? CashBankVoucherAccountMappingPostingGate::accountFor($accountMappings, $receipt, 'credit', (string) $line->credit_account) : ($directPosting ? ($line->credit_account ?: '131') : $this->accountOrLegacyFallback($line->credit_account, 'credit_account', '131'));
                $glLines[] = [
