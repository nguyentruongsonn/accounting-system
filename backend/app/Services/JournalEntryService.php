...
class JournalEntryService
{
            ...
            if (! empty($data['lines']) && is_array($data['lines'])) {
                foreach ($data['lines'] as $lineIndex => $line) {
                    ...
                    if (isset($line['debit_account']) && isset($line['credit_account'])) {
                        ...
                        // Debit line
                        $processedLines[] = [
                            'account_code' => $this->resolveLeafAccount($companyId, (string) ($line['debit_account'] ?? '')),
                        ...
                        // Credit line
                        $processedLines[] = [
                            'account_code' => $this->resolveLeafAccount($companyId, (string) ($line['credit_account'] ?? '')),
                            'description' => $lineDesc,
                            'debit_amount' => '0.00',
                        ...
                        $processedLines[] = [
                            'account_code' => $this->resolveLeafAccount($companyId, (string) ($line['account_code'] ?? '')),
                            'description' => $lineDesc,
                            'debit_amount' => $d,
                            'credit_amount' => $c,
                            'contact_type' => $contactType,
    ...
    public function createPosted(array $data): JournalEntry
    {
        return DB::transaction(function () use ($data) {
            ...
            if ($existing !== null && ! $existing->trashed() && $existing->status === 'posted') {
            ...
            if (isset($data['lines']) && is_array($data['lines'])) {
                foreach ($data['lines'] as $lineIndex => $line) {
                    ...
                    if (isset($line['debit_account']) && isset($line['credit_account'])) {
                        ...
                        $processedLines[] = [
                            'account_code' => $this->resolveLeafAccount((int) $entry->company_id, (string) ($line['debit_account'] ?? '')),
                            'description' => $lineDesc,
                            'debit_amount' => $amount,
                            'credit_amount' => '0.00',
                            'contact_type' => $contactType,
                        ...
                        $processedLines[] = [
                            'account_code' => $this->resolveLeafAccount((int) $entry->company_id, (string) ($line['credit_account'] ?? '')),
                            'description' => $lineDesc,
                            'debit_amount' => '0.00',
                            'credit_amount' => $amount,
                            'contact_type' => $contactType,
                        ...
                        $processedLines[] = [
                            'account_code' => $this->resolveLeafAccount((int) $entry->company_id, (string) ($line['account_code'] ?? '')),
                            'description' => $lineDesc,
                            'debit_amount' => $d,
                            'credit_amount' => $c,
                            'contact_type' => $contactType,
    ...
    public function resolveLeafAccount(int $companyId, string $code): string
    {
        $code = trim($code);
        if ($code === '') {
            return $code;
        ...
        $account = ChartOfAccount::withoutGlobalScope('company')
            ->where('company_id', $companyId)
            ->where('code', $code)
            ->first();

        if ($account && $account->is_parent) {
            $leaf = ChartOfAccount::withoutGlobalScope('company')
                ->where('company_id', $companyId)
        ...
        if (! $account) {
            for ($len = strlen($code) - 1; $len >= 3; $len--) {
                $prefix = substr($code, 0, $len);
                $parentLeaf = ChartOfAccount::withoutGlobalScope('company')
                    ->where('company_id', $companyId)
                    ->where('is_active', true)
                    ->where('is_parent', false)
                    ->where('code', $prefix)
                ...
                if ($parentLeaf) {
                    return $parentLeaf->code;
                }
            }

            // Fallback for TT200 codes when company uses TT133
            $tt200To133 = [
                '6424' => '6422',
                '6425' => '6422',
                '6426' => '6422',
            ...
            foreach ($tt200To133 as $tt200Prefix => $tt133Target) {
                if (str_starts_with($code, $tt200Prefix)) {
                    $targetAcc = ChartOfAccount::withoutGlobalScope('company')
                        ->where('company_id', $companyId)
                        ->where('is_active', true)
                        ->where('is_parent', false)
                        ->where('code', $tt133Target)
                        ->first();
                    if ($targetAcc) {
                        return $targetAcc->code;
                    }
                }
            }
        }
