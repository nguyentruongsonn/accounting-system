        ...
        return DB::transaction(function () use ($id) {
            ...
            $policy = $directPosting
                ...
                : (($this->requiresCashBankPolicy() || $this->requiresCashBankAccountMappings())
                    ? $this->accountingPolicyResolver->requireForVoucher((int) $payment->company_id, ($payment->posting_date ?? $payment->voucher_date ?? now())->toDateString(), SystemVoucherType::CASH_PAYMENT)
                    : null);
            $accountMappings = $directPosting || ! $this->requiresCashBankAccountMappings()
                ? null
                : $this->accountMappingGate->requireSatisfied($payment, $policy);

            $glLines = [];
            foreach ($payment->lines as $line) {
                $debitAccount = $accountMappings !== null
                    ? CashBankVoucherAccountMappingPostingGate::accountFor($accountMappings, $payment, 'debit', (string) $line->debit_account)
                    : ($directPosting ? $this->requireDirectAccount($line->debit_account, 'debit_account') : $this->accountOrLegacyFallback($line->debit_account, 'debit_account', '331'));
                $creditAccount = $accountMappings !== null
                    ? CashBankVoucherAccountMappingPostingGate::accountFor($accountMappings, $payment, 'credit', (string) $line->credit_account)
                    : ($directPosting ? $this->requireDirectAccount($line->credit_account, 'credit_account') : $this->accountOrLegacyFallback($line->credit_account, 'credit_account', '1111'));
                $glLines[] = [
            ...
            $this->recordSourceAudit($payment, 'cash_payment.posted', $before, $payment->fresh()->toArray(), array_filter(['accounting_policy' => $policy, 'posting_authorization' => $authorization, 'account_mappings' => $accountMappings, 'posting_mode' => $directPosting ? 'direct' : null]));
