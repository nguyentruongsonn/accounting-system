...
final class PurchaseInvoicePaymentService
{
    ...
    public function pay(User $actor, int $invoiceId, array $data): array
    {
        ...
        return DB::transaction(function () use ($actor, $companyId, $invoiceId, $data): array {
            ...
            // A client retry after a successful response must be idempotent.
            // The voucher number is the caller-provided idempotency key; only
            // replay the original evidence when it is already allocated to
            // this exact invoice for the same amount.
            $existingPayment = CashPayment::withoutGlobalScope('company')
                ->with('lines')
                ->where('company_id', $companyId)
            ...
            if ($existingPayment !== null) {
                $existingAllocation = SettlementAllocation::withoutGlobalScope('company')
                    ->where('company_id', $companyId)
                    ->where('source_document_type', 'cash_payment')
                    ->where('source_document_id', $existingPayment->id)
                ...
                if ($existingAllocation !== null
                    ...
                    return [
                        'payment' => $existingPayment->fresh(['lines', 'references']),
                ...
                throw ValidationException::withMessages([
                    'voucher_number' => 'Số phiếu chi đã tồn tại và không khớp với lần thanh toán này.',
                ]);
            }
            ...
            $payment = $this->cashPayments->create([
                ...
                'lines' => [[
                    ...
                    // The same numeric id may exist in customers and
                    // suppliers; preserve the contact kind at the guard
                    // boundary even though the legacy line schema stores
                    // only the id.
                    'line_contact_type' => 'supplier',
