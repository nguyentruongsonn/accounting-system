...
class PeriodCloseReadinessService
{
    ...
    public function __construct(
        ...
        private readonly PeriodDraftDocumentService $draftDocuments,
    ) {}
    ...
    public function evaluate(int $companyId, int $periodId, ?int $actorId = null): PeriodCloseReadinessSnapshot
    {
        ...
        return DB::transaction(function () use ($companyId, $periodId, $actorId): PeriodCloseReadinessSnapshot {
            ...
            $snapshot = $this->buildSnapshot(
                ...
                $this->draftDocuments->forPeriod($companyId, $period),
            );
    ...
    private function buildSnapshot(
        ...
        $noDraftDocuments = (int) ($draftDocuments['count'] ?? 0) === 0;
        $eligible = $controlledEnforcement && $noBlockingResults && $namedDomainsControlled && $inventoryValuationCurrent && $noDraftDocuments;
        ...
        return [
            ...
            'checks' => [
                $this->check(
                    'RECONCILIATION.COMPLETED_EVIDENCE',
                    'Đã có bằng chứng đối chiếu hoàn tất',
                    $hasCompletedRun ? 'pass' : 'fail',
                    $hasCompletedRun,
                    true,
                    $hasCompletedRun
                ...
                $this->check(
                    'RECONCILIATION.RESULTS_WITHOUT_BLOCKERS',
                    'Kết quả đối chiếu không có lỗi hoặc dữ liệu thiếu',
                    $noBlockingResults ? 'pass' : 'fail',
                    [
                        'result_count' => $hasCompletedRun ? (int) $run->result_count : 0,
                        'failed_result_count' => $hasCompletedRun ? (int) $run->failed_result_count : 0,
                        'not_available_result_count' => $hasCompletedRun ? (int) $run->not_available_result_count : 0,
                    ],
                    [
                        'result_count' => '> 0',
                ...
                $this->check(
                    'RECONCILIATION.ENFORCED_CLOSE_GATE',
                    'Có chốt đóng kỳ được phê duyệt',
                    $controlledEnforcement ? 'pass' : 'fail',
                    ['basis' => $run?->basis, 'enforcement' => $enforcement],
                    ['basis' => 'controlled', 'enforcement' => 'block_close'],
                    [
                ...
                $this->check(
                    'RECONCILIATION.NAMED_DOMAINS_SAME_CUTOFF',
                    'Các phân hệ đối chiếu cùng ngày chốt',
                    $namedDomainsControlled ? 'pass' : 'fail',
                    $namedDomainsControlled,
                    true,
                    $controlledReconciliation,
                ),
                $this->check(
                    'INVENTORY.VALUATION_CURRENT',
                    'Giá vốn hàng tồn kho hiện hành',
                    $inventoryValuationCurrent ? 'pass' : 'fail',
                    $inventoryValuationCurrent,
                    true,
                    $inventoryValuation,
                ),
                $this->check(
                    'DRAFT_DOCUMENTS.NONE_IN_PERIOD',
                    'Không còn chứng từ nháp trong kỳ',
                    $noDraftDocuments ? 'pass' : 'fail',
                    (int) ($draftDocuments['count'] ?? 0),
                    0,
                    $draftDocuments,
                ),
            ],
