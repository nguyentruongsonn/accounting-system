...
final class PurchaseInvoiceDimensionReadinessService
{
    ...
    public function inspect(PurchaseInvoice $invoice): array
    {
        ...
        try {
            ...
            if ($this->postingMode->isDirectPostingEnabled() || ! config('accounting.enforce_purchase_invoice_posting_policy', true)) {
                return [
                    'status' => 'not_required',
                    'can_enforce' => true,
                    'posting_date' => $postingDate,
                    'policy_id' => null,
                    'policy_contract_hash' => null,
                    'required_dimensions' => [],
                    'audit_requirement' => 'Chế độ ghi sổ trực tiếp (direct posting mode) đang kích hoạt.',
                ];
            }
