...
class SalesReturnService extends BaseService
{
    ...
    public function post(int|string $id): SalesReturn
    {
        return DB::transaction(function () use ($id) {
            ...
            if (! $this->postingMode->isDirectPostingEnabled() && (bool) config('accounting.enforce_return_discount_posting_account_mappings', true)) {
                try {
            ...
            $salesReturn->save();
            $this->recordInventoryMovements($salesReturn);
    ...
    /** Record an inward stock movement for a posted physical sales return. */
    private function recordInventoryMovements(SalesReturn $salesReturn): void
    {
        if (! $salesReturn->is_inward && ! $salesReturn->is_import_slip) {
            return;
        }
        ...
        foreach ($salesReturn->lines as $index => $line) {
            // Service returns affect revenue/receivable only, not stock.
            ...
            if ($warehouseId < 1) {
                throw ValidationException::withMessages([
                    "lines.{$index}.warehouse_id" => 'Chứng từ trả lại hàng bán phải xác định kho nhập cho từng dòng hàng.',
                ]);
            }
            if (! Warehouse::query()->where('company_id', $salesReturn->company_id)->whereKey($warehouseId)->exists()) {
                throw ValidationException::withMessages([
                    "lines.{$index}.warehouse_id" => 'Kho nhập không thuộc doanh nghiệp của chứng từ.',
                ]);
            }
            ...
            if (DecimalMoney::compare($quantity, DecimalMoney::ZERO) <= 0) {
                throw ValidationException::withMessages([
                    "lines.{$index}.quantity" => 'Số lượng hàng bán trả lại phải lớn hơn 0.',
                ]);
            }
            $cogsAmount = $this->persistedMoney($line, 'cogs_amount');
            if (DecimalMoney::compare($cogsAmount, DecimalMoney::ZERO) === 0) {
                $cogsAmount = DecimalMoney::multiply($quantity, $this->persistedMoney($line, 'cogs_price'));
            }
            ...
            $postingCycle = ((int) (InventoryMovementEvent::query()
                ...
                ->where('movement_type', 'sales_return_in')
                ->max('posting_cycle'))) + 1;

            InventoryMovementEvent::create([
                'company_id' => $salesReturn->company_id,
                'movement_date' => ($salesReturn->accounting_date ?? $salesReturn->voucher_date)->toDateString(),
                'warehouse_id' => $warehouseId,
                'item_id' => $line->item_id,
                'movement_type' => 'sales_return_in',
                'posting_cycle' => $postingCycle,
                'quantity_delta' => $quantity,
                'amount_delta' => $cogsAmount,
            ...
            $this->valuationRunInvalidator->invalidateForInventoryMovement(
                (int) $salesReturn->company_id,
                ($salesReturn->accounting_date ?? $salesReturn->voucher_date)->toDateString(),
                'sales_return_posted',
                $warehouseId,
                (int) $line->item_id,
            );
    ...
    /** Reverse every posted inward-return event exactly once per posting cycle. */
    private function reverseInventoryMovements(SalesReturn $salesReturn): void
    {
        ...
        foreach ($events as $event) {
            $alreadyReversed = InventoryMovementEvent::query()
                ...
                ->where('movement_type', 'sales_return_in_reversal')
            ...
            InventoryMovementEvent::create([
                ...
                'movement_type' => 'sales_return_in_reversal',
    ...
    public function unpost(int|string $id): SalesReturn
    {
        return DB::transaction(function () use ($id) {
            ...
            $this->reverseInventoryMovements($salesReturn);
