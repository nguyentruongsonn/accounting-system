...
class PurchaseInvoiceController extends Controller
{
    public function __construct(
        ...
        private readonly PurchaseExpenseAllocationService $expenseAllocations,
    ) {}
    ...
    public function expenseAllocationCandidates(Request $request): JsonResponse
    {
        $companyId = TenantContext::companyId($request);
        $input = $request->validate([
            'source_invoice_id' => ['nullable', 'integer', 'min:1'],
        ...
        $result = $this->expenseAllocations->candidates(
            $companyId,
            isset($input['source_invoice_id']) ? (int) $input['source_invoice_id'] : null,
