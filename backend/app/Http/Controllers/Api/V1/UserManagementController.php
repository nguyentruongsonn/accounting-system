...
final class UserManagementController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $user = DB::transaction(function () use ($data, $company) {
            ...
            $this->auditService->recordMutation($user, 'user.created', [], $this->auditSnapshot($user), $data['reason'] ?? null, [
                'role' => $data['role'],
            ]);
    ...
    public function update(Request $request, int $id)
    {
        ...
        $user = DB::transaction(function () use ($company, $id, $data) {
            ...
            $this->auditService->recordMutation($user, 'user.updated', $before, $this->auditSnapshot($user->fresh()), $data['reason'] ?? null, [
                'changed_fields' => array_values(array_keys(collect($data)->only(['name', 'email', 'password', 'role', 'is_active'])->all())),
    ...
    public function destroy(Request $request, int $id)
    {
        ...
        $request->validate(['reason' => ['sometimes', 'nullable', 'string', 'max:2000']]);
        $company = TenantContext::companyId($request);
        ...
        DB::transaction(function () use ($user, $reason): void {
            ...
            $this->auditService->recordMutation($user, 'user.disabled', $before, $this->auditSnapshot($user->fresh()), $reason, [
                'access_revoked' => true,
            ]);
        });
