...
class ReconciliationController extends Controller
{
    ...
    public function closeReadiness(Request $request, int $periodId): JsonResponse
    {
        ...
        $signoffRequired = (bool) config('accounting.enforce_period_close_signoff', true);
        $accountMappingsRequired = (bool) config('accounting.enforce_period_close_account_mappings', true);

        return response()->json([
            'data' => [
                'schema' => 'period-close-readiness.v1',
                'mode' => ! $signoffRequired && ! $accountMappingsRequired
                    ? 'internal_two_role'
                    : 'controlled',
                'period' => [
                    'id' => $period->id,
                ...
                'close_gate' => [
                    'enforcement' => ! $signoffRequired && ! $accountMappingsRequired ? 'internal' : 'controlled',
                    'status' => ! $signoffRequired && ! $accountMappingsRequired ? 'not_required' : 'evidence_required',
                    'close_permitted_by_this_endpoint' => false,
                    'signoff_required' => $signoffRequired,
                    'account_mappings_required' => $accountMappingsRequired,
                    'reason' => ! $signoffRequired && ! $accountMappingsRequired
                        ? 'Internal two-role mode: readiness is diagnostic evidence; the admin-only close endpoint remains the final authority.'
                        : 'Controlled mode requires the configured owner-approved close evidence before period close.',
                ],
