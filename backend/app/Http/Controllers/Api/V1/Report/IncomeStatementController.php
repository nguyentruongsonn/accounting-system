...
class IncomeStatementController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $result = $this->service->getIncomeStatement($companyId, $fromDate, $toDate);
        $this->auditReportIssuance($request, $this->reportAudit, $companyId, 'income_statement', $this->reportDelivery($request), [
            'from_date' => $fromDate,
            'to_date' => $toDate,
        ...
        if ($request->query('export') === 'excel') {
            return Excel::download(
                new IncomeStatementExport($result),
                'income_statement.xlsx'
            );
        }

        if ($request->query('export') === 'pdf') {
            $pdf = Pdf::loadView('reports.pdf.income_statement', ['data' => $result]);
