...
class FixedAssetController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        try {
            $companyId = TenantContext::companyId($request);
            $lifecycleAction = $request->input('lifecycle_action');
            if ($lifecycleAction === 'transfer') {
                $data = $request->validate([
                    'lifecycle_action' => ['required', 'in:transfer'],
                    'voucher_number' => ['required', 'string', 'max:50'],
                    'voucher_date' => ['required', 'date_format:Y-m-d'],
                    'to_department_code' => ['required', 'string', 'max:50'],
                    'reason' => ['nullable', 'string', 'max:2000'],
                ]);

                return response()->json(['data' => $this->service->transferAsset($companyId, (int) $id, $data)]);
            }
            if ($lifecycleAction === 'inventory') {
                $data = $request->validate([
                    'lifecycle_action' => ['required', 'in:inventory'],
                    'voucher_number' => ['required', 'string', 'max:50'],
                    'voucher_date' => ['required', 'date_format:Y-m-d'],
                    'counted_quantity' => ['required', 'integer', 'min:0'],
                    'counted_net_value' => ['required', 'numeric', 'min:0'],
                    'reason' => ['nullable', 'string', 'max:2000'],
                ]);

                return response()->json(['data' => $this->service->inventoryAsset($companyId, (int) $id, $data)]);
            }
