...
return [
    ...
    // Test/local-only two-user workflow: the accountant may post from the
    // voucher's persisted accounts without policy/mapping evidence approval.
    // The service layer also blocks this mode in production.
    'direct_posting_mode' => $isProduction ? false : env('ACCOUNTING_DIRECT_POSTING_MODE', false),
    ...
    // The internal two-role profile closes from server readiness evidence and
    // the admin-only close permission. Signoff packages remain available as a
