...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('inventory_receipts', function (Blueprint $table) {
            ...
            $table->string('deliverer_name')->nullable()->after('contact_name'); // Người giao hàng
            $table->date('posting_date')->nullable()->after('voucher_date'); // Ngày hạch toán
        ...
        Schema::table('inventory_issues', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('contact_id')->nullable()->after('contact_type');
            $table->string('contact_name')->nullable()->after('contact_id');
            $table->string('receiver_name')->nullable()->after('contact_name'); // Người nhận hàng
            $table->date('posting_date')->nullable()->after('voucher_date'); // Ngày hạch toán
            $table->string('attached_docs')->nullable()->after('description');
            $table->string('currency', 3)->default('VND')->after('attached_docs');
            $table->decimal('exchange_rate', 15, 2)->default(1)->after('currency');
    ...
    {
        Schema::table('inventory_receipts', function (Blueprint $table) {
            $table->dropColumn(['contact_type', 'contact_id', 'contact_name', 'deliverer_name', 'posting_date', 'attached_docs', 'currency', 'exchange_rate']);
        });

        Schema::table('inventory_issues', function (Blueprint $table) {
            $table->dropColumn(['contact_type', 'contact_id', 'contact_name', 'receiver_name', 'posting_date', 'attached_docs', 'currency', 'exchange_rate']);
        });
    }
};
