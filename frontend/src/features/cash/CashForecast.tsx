...
import { Table, Button, Input, Select, InputNumber, Tag, Space, DatePicker, Tooltip } from 'antd';
...
export const CashForecast: React.FC = React.memo(() => {
    ...
    const loadForecasts = useCallback(async (rethrow = false) => {
        setLoading(true);
        setLoadError(false);
        try {
                    ...
                    actions={(
                        <div className="ui-page-toolbar__action-group flex items-center gap-2">
                            <Button 
                                ...
                                title="Làm mới (F5)" 
                                onClick={() => void runManualDataLoad(() => loadForecasts(true), { success: 'Tải lại dự báo dòng tiền thành công.', failure: 'Không thể tải lại dự báo dòng tiền.' })}
                                loading={loading}
                            />
        ...
        >
            ...
            {loadError && (
                <div className="mb-4 misa-p-12" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                        <div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Không thể tải dự báo dòng tiền</div>
                        <div style={{ color: '#B91C1C', fontSize: 12 }}>Các dòng đang hiển thị được giữ nguyên; máy chủ chưa trả về dữ liệu mới hợp lệ.</div>
                    </div>
                    <Button size="small" onClick={() => void loadForecasts()}>Thử lại</Button>
                </div>
            )}
            <DataTableSurface>
                <Table 
