...
export function useCashReportQuery(
  ...
  const selectedDateFrom = selection?.filters.date_from;
  const selectedDateTo = selection?.filters.date_to;
  const selectedStatus = selection?.filters.status;
  const selectedSearch = selection?.filters.search;
  const selectedCashAccount = selection?.filters.cash_account;
  const appliedFilters = useMemo(
    () => {
      if (selectedDateFrom === undefined || selectedDateTo === undefined || selectedStatus === undefined) {
        return null;
      }
    ...
    [selectedCashAccount, selectedDateFrom, selectedDateTo, selectedSearch, selectedStatus],
  );
