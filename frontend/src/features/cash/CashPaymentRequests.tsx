...
import { Button, DatePicker, Form, Input, InputNumber, Space, Table } from 'antd';
...
export const CashPaymentRequests: React.FC = React.memo(() => {
    ...
    const loadRequests = useCallback(async (rethrow = false) => {
        setLoading(true);
        setLoadError(false);
        try {
    ...
    return <PageShell title={<PageHeader eyebrow="Tiền mặt" title="Đề nghị chi tiền — quy trình nháp nội bộ" description="Lập và theo dõi các đề nghị chi tiền trong kỳ." />}>
        <PageToolbar
            ...
            actions={<><button type="button" className="misa-btn-tool" title="Làm mới" onClick={() => void runManualDataLoad(() => loadRequests(true), { success: 'Tải lại đề nghị chi tiền thành công.', failure: 'Không thể tải lại đề nghị chi tiền.' })} disabled={loading}><ReloadOutlined /></button><Button type="primary" icon={<PlusOutlined />} className="misa-btn-primary-green misa-btn-action-h32-b600" onClick={openCreate}>Lập đề nghị chi tiền</Button></>}
        />
        {loadError && (
            <div className="mb-4 misa-p-12" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                    <div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Không thể tải đề nghị chi tiền</div>
                    <div style={{ color: '#B91C1C', fontSize: 12 }}>Các dòng đang hiển thị được giữ nguyên; hãy thử lại khi máy chủ sẵn sàng.</div>
                </div>
                <Button size="small" onClick={() => void loadRequests()}>Thử lại</Button>
            </div>
        )}
