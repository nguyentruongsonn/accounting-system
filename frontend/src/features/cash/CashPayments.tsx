...
import { Table, Button, ConfigProvider, Form, Input, InputNumber, Select, DatePicker, Switch, Dropdown, Tag } from 'antd';
...
export const CashPayments: React.FC<CashPaymentsProps> = React.memo(({ modalOnly = false, open: controlledOpen, onOpenChange }) => {
    ...
    const [activePaymentIsPosted, setActivePaymentIsPosted] = useState(false);
    ...
    const mutation = useMutation({
        ...
        onSuccess: (data: any) => {
            ...
            const persistedIsPosted = persistedPayment.is_posted === true || persistedPayment.status === 'posted';
            setEditingPaymentId(Number(persistedPayment.id));
    ...
    const postMutation = useMutation({
        ...
        onMutate: async (id: number) => {
            const context = optimisticTogglePostStatus(queryClient, ['cash-payments'], id, true);
    ...
    const unpostMutation = useMutation({
        ...
        onMutate: async (id: number) => {
            const context = optimisticTogglePostStatus(queryClient, ['cash-payments'], id, false);
            setActivePaymentIsPosted(false);
            return context;
        },
        onSuccess: (response: any) => {
    ...
    const deleteMutation = useMutation({
        ...
        onMutate: async (id: number) => {
            return instantRemoveVoucher(queryClient, ['cash-payments'], id);
        },
        onSuccess: (response: any) => {
    ...
    const handleViewPayment = async (record: any, targetMode: 'view' | 'edit' = 'view') => {
        ...
        try {
            ...
            setActivePaymentIsPosted(payment.is_posted === true || payment.status === 'posted');
        } catch (e) {
            setEditingPaymentId(null);
            setActivePaymentIsPosted(false);
    ...
    useEffect(() => {
        ...
        if (listed) {
            clearSourceQuery();
            void handlersRef.current.handleViewPayment(listed, 'view');
            return;
        }
        ...
        api.get('/cash/payments/' + sourceId)
            .then(({ data }) => {
                ...
                clearSourceQuery();
                void handlersRef.current.handleViewPayment(record, 'view');
            })
            .catch((error) => {
                if (!active) return;
                clearSourceQuery();
        ...
        return () => { active = false; };
    }, [isLoading, isPaymentsError, modalOnly, payments, searchParams, setSearchParams]);
                            ...
                            <div className="misa-footer-right">
                                {modalMode === 'view' ? (
                                    <>
                                        ...
                                        {editingPaymentId && activePaymentIsPosted ? (
                                            <Button
                ...
                <div className="ui-page-toolbar__action-group">
                    <Button 
                        ...
                        onClick={() => void runManualDataLoad(
                            () => refetchPayments(),
                            { success: 'Tải lại danh sách phiếu chi thành công.', failure: 'Không thể tải lại danh sách phiếu chi.' },
                        )}
