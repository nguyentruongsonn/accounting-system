...
export const CashAdvanceSettlements: React.FC = React.memo(() => {
    ...
    const load = useCallback(async (rethrow = false) => {
        ...
        setLoadError(null);
        try {
            const response = await api.get('/cash/advance-settlements');
            setRecords(listPayload(response.data).filter((v: any) => Number.isInteger(Number(v?.id))) as AdvanceSettlement[]);
            return true;
        } catch (error) {
            setLoadError('Máy chủ không trả về danh sách quyết toán hợp lệ. Các dòng đang hiển thị được giữ nguyên.');
            console.error(error);
    ...
    return <PageShell title={<PageHeader eyebrow="Tiền mặt" title="Quyết toán tạm ứng" description="Lập và theo dõi quyết toán các khoản tạm ứng cho nhân viên." />}>
        <PageToolbar
            ...
            actions={<><button type="button" className="misa-btn-tool" title="Làm mới" onClick={() => void runManualDataLoad(() => load(true), { success: 'Tải lại quyết toán tạm ứng thành công.', failure: 'Không thể tải lại quyết toán tạm ứng.' })} disabled={loading}><ReloadOutlined /></button><Button type="primary" icon={<PlusOutlined />} className="misa-btn-primary-green misa-btn-action-h32-b600" onClick={openCreate}>Lập quyết toán tạm ứng</Button></>}
        />
        {loadError && (
            <div className="misa-p-12 misa-mb-12" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                    <div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Không thể tải quyết toán tạm ứng</div>
                    <div style={{ color: '#B91C1C', fontSize: 12 }}>{loadError}</div>
                </div>
                <Button size="small" onClick={() => void load()}>Thử lại</Button>
            </div>
        )}
