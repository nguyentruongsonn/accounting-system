...
import { Table, Button, Input, Select, Dropdown } from 'antd';
...
import { sumPostedCashTransactionAmounts } from './cashTransactionTotals';
...
function sumCashAmounts(rows: any[], loading: boolean, error: boolean): number | null {
    return sumPostedCashTransactionAmounts(rows, loading, error);
}
...
export const CashTransactions: React.FC<CashTransactionsProps> = React.memo(() => {
    ...
    React.useEffect(() => {
        if (isReceiptsError) {
            message.error('Không thể tải giao dịch tiền mặt (phiếu thu). Hãy đăng nhập lại nếu phiên đã hết hạn hoặc bấm Nạp lại dữ liệu để thử lại.');
        }
        if (isPaymentsError) {
            message.error('Không thể tải giao dịch tiền mặt (phiếu chi). Hãy đăng nhập lại nếu phiên đã hết hạn hoặc bấm Nạp lại dữ liệu để thử lại.');
        }
    ...
    const togglePostMutation = useMutation({
        ...
        onMutate: async ({ id, type, isPosted }) => {
            const queryKey = type === 'receipt' ? ['cash-receipts'] : ['cash-payments'];
            const targetIsPosted = !isPosted;
            const context = optimisticTogglePostStatus(queryClient, queryKey, id, targetIsPosted);

            // Cập nhật ngay tức thì selectedRow nếu đang trỏ vào chứng từ này
            setSelectedRow((prev: any) => {
                if (prev && (prev.id === id || String(prev.id) === String(id))) {
                    return {
                        ...prev,
                        is_posted: targetIsPosted,
                        status: targetIsPosted ? 'posted' : 'draft',
                    };
                }
                return prev;
            });
        ...
        onError: (err: any, variables, context) => {
            ...
            rollbackVoucherCache(queryClient, queryKey, context);
            // Rollback selectedRow
            setSelectedRow((prev: any) => {
                if (prev && (prev.id === variables.id || String(prev.id) === String(variables.id))) {
                    return {
                        ...prev,
                        is_posted: variables.isPosted,
                        status: variables.isPosted ? 'posted' : 'draft',
                    };
                }
                return prev;
            });
    ...
    const deleteMutation = useMutation({
        ...
        onMutate: async ({ id, type }) => {
            const queryKey = type === 'receipt' ? ['cash-receipts'] : ['cash-payments'];
            const context = instantRemoveVoucher(queryClient, queryKey, id);
            setSelectedRow((prev: any) => {
                if (prev && (prev.id === id || String(prev.id) === String(id))) {
                    return null;
                }
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            width: 110,
            align: 'center' as const,
            render: (_isPosted: boolean, record: any) => (
                    ...
                    <>
                        <Button
                            ...
                            onClick={() => void runManualDataLoad(
                                ...
                                { success: 'Đã làm mới danh sách dữ liệu!', failure: 'Không thể tải lại dữ liệu tiền mặt.' },
                            )}
