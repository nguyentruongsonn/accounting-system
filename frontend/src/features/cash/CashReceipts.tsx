...
import { Table, Button, ConfigProvider, Form, Input, InputNumber, Select, DatePicker, Switch, Dropdown, Space, Tag } from 'antd';
...
export const CashReceipts: React.FC<CashReceiptsProps> = React.memo(({ modalOnly = false, open: controlledOpen, onOpenChange }) => {
    ...
    useEffect(() => {
        if (isReceiptsError) {
            message.error('Không thể tải danh sách phiếu thu. Hãy thử tải lại.');
        }
    }, [isReceiptsError]);
    ...
    const mutation = useMutation({
        ...
        onSuccess: (data: any) => {
            ...
            const wasEditing = Boolean(editingReceiptId);
            const persistedIsPosted = persistedReceipt.is_posted === true || persistedReceipt.status === 'posted';
            setEditingReceiptId(Number(persistedReceipt.id));
    ...
    const postMutation = useMutation({
        ...
        onMutate: async (id: number) => {
            const context = optimisticTogglePostStatus(queryClient, ['cash-receipts'], id, true);
    ...
    const unpostMutation = useMutation({
        ...
        onMutate: async (id: number) => {
            const context = optimisticTogglePostStatus(queryClient, ['cash-receipts'], id, false);
            setActiveReceiptIsPosted(false);
            return context;
        },
        onSuccess: (response: any) => {
    ...
    const deleteMutation = useMutation({
        ...
        onMutate: async (id: number) => {
            return instantRemoveVoucher(queryClient, ['cash-receipts'], id);
        },
        onSuccess: (response: any) => {
            if (typeof response?.data?.message !== 'string') {
    ...
    const handleViewReceipt = async (record: any, targetMode: 'view' | 'edit' = 'view') => {
        ...
        try {
            ...
            setActiveReceiptIsPosted(receipt.is_posted === true || receipt.status === 'posted');
        } catch (e) {
            setEditingReceiptId(null);
            setActiveReceiptIsPosted(false);
    ...
    const handleEditReceipt = async (record: ReceiptRecord) => {
        ...
        const decision = getVoucherActionDecision('edit', {
            ...
            status: record.status ?? (record.is_posted ? 'posted' : 'draft'),
    ...
    useEffect(() => {
        ...
        if (listed) {
            clearSourceQuery();
            void handlersRef.current.handleViewReceipt(listed, 'view');
            return;
        }
        ...
        api.get('/cash/receipts/' + sourceId)
            .then(({ data }) => {
                ...
                clearSourceQuery();
                void handlersRef.current.handleViewReceipt(record, 'view');
            })
            .catch((error) => {
                if (!active) return;
                clearSourceQuery();
        ...
        return () => { active = false; };
    }, [isLoading, isReceiptsError, modalOnly, receipts, searchParams, setSearchParams]);
                            ...
                            <div className="misa-footer-right">
                                {modalMode === 'view' ? (
                                    <>
                                        ...
                                        {editingReceiptId && activeReceiptIsPosted ? (
                                            <Button
                ...
                <div className="ui-page-toolbar__action-group">
                    ...
                    <Button
                        ...
                        onClick={() => void runManualDataLoad(
                            ...
                            { success: 'Tải lại dữ liệu tiền mặt thành công.', failure: 'Không thể tải lại dữ liệu tiền mặt.' },
                        )}
