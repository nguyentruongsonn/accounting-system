...
export const SalesDiscounts: React.FC = () => {
    ...
    const postMutation = useMutation({
        ...
        onMutate: async (id: number) => {
            return optimisticTogglePostStatus(queryClient, ['sales-discounts'], id, true);
        },
        onSuccess: (response) => {
    ...
    const unpostMutation = useMutation({
        ...
        onMutate: async (id: number) => {
            return optimisticTogglePostStatus(queryClient, ['sales-discounts'], id, false);
        },
        onSuccess: (response) => {
            if (persistedActionId(response) === undefined || persistedActionId(response) === null) {
    ...
    const columns: ColumnsType<SalesDiscountRecord> = [
        ...
        },
        {
            ...
            align: 'center',
            width: 120,
            render: (_: unknown, record: SalesDiscountRecord) => <VoucherStatusBadge status={record} />
        },
        {
            title: 'Chức năng',
            key: 'action',
                ...
                actions={(
                    ...
                    <button 
                        ...
                        onClick={() => void runManualDataLoad(
                            () => queryClient.invalidateQueries({ queryKey: ['sales-discounts'] }),
                            { success: 'Tải lại danh sách giảm giá hàng bán thành công.', failure: 'Không thể tải lại danh sách giảm giá hàng bán.' },
                        )}
                    >
                        <ReloadOutlined />
                    </button>
