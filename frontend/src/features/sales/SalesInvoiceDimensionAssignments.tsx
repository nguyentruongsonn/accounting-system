import { Button, Descriptions, Drawer, Form, Select, Spin, Tag } from 'antd';
...
export default function SalesInvoiceDimensionAssignments({ invoice, open, onClose }: Props) {
  ...
  return (
    <Drawer title="Chiều hạch toán" size={560} open={open} onClose={onClose} destroyOnHidden
      ...
      {(contextQuery.isError || assignmentsQuery.isError) && <div role="alert" className="mb-4 misa-p-12" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8 }}><div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Không thể xác minh context hoặc evidence</div><div style={{ color: '#B91C1C', fontSize: 12, marginTop: 2 }}>Chứng từ chỉ xem khi máy chủ chưa trả đủ context. Không lưu hay gửi ghi sổ từ đây.</div></div>}
      {!contextQuery.isLoading && !contextQuery.isError && <>
        ...
        {readinessReason && <div role="alert" className="mb-4 misa-p-12" style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8 }}><div style={{ fontWeight: 600, color: '#92400E', fontSize: 13 }}>Blocker do máy chủ đánh giá</div><div style={{ color: '#B45309', fontSize: 12, marginTop: 2 }}>{readinessReason}</div></div>}
        {context?.status === 'not_required' && <div className="mb-4 misa-p-12" style={{ background: '#EBF5FF', border: '1px solid #BFDBFE', borderRadius: 8 }}><div style={{ fontWeight: 600, color: 'var(--ui-primary, #0064E0)', fontSize: 13 }}>Policy hiệu lực không yêu cầu chiều hạch toán</div><div style={{ color: '#1C1E21', fontSize: 12, marginTop: 2 }}>Không có lựa chọn nào cần lưu cho ngày hạch toán này.</div></div>}
        {contextUnavailable && <div role="alert" className="mb-4 misa-p-12" style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8 }}><div style={{ fontWeight: 600, color: '#92400E', fontSize: 13 }}>Chưa thể chọn chiều hạch toán</div><div style={{ color: '#B45309', fontSize: 12, marginTop: 2 }}>{context?.reason || 'Máy chủ chưa có policy hoặc dimension/value hiệu lực phù hợp.'}</div></div>}
        {saveForbidden && <div role="alert" className="mb-4 misa-p-12" style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8 }}><div style={{ fontWeight: 600, color: '#92400E', fontSize: 13 }}>Bạn chỉ có quyền xem</div><div style={{ color: '#B45309', fontSize: 12, marginTop: 2 }}>Máy chủ không cấp quyền cập nhật hóa đơn bán; giao diện không cố bypass quyền này.</div></div>}
