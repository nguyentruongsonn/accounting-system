...
export const SalesReports: React.FC<SalesReportsProps> = ({ active = true, embedded = false }) => {
  ...
  const reportToolbar = <PageToolbar
    ...
    actions={<Space wrap>
      <Button icon={<ReloadOutlined />} onClick={() => void runManualDataLoad(() => reportQuery.refetch(), { success: 'Tải lại báo cáo bán hàng thành công.', failure: 'Không thể tải lại báo cáo bán hàng.' })} loading={reportQuery.isFetching}>Tải lại</Button>
    ...
    >
      ...
      <DataTableSurface>
        ...
        {customersQuery.isError && (
          <div role="alert" className="mb-3 misa-p-12" style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
