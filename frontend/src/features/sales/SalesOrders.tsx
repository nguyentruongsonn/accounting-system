...
import { ConfigProvider, Table, Button, Form, Input, InputNumber, Select, DatePicker } from 'antd';
...
export const SalesOrders: React.FC = () => {
    ...
    const columns: ColumnsType<SalesOrderRecord> = [
        ...
        },
        {
            ...
            width: 120,
            align: 'center',
            render: (s: string) => <VoucherStatusBadge status={s} />,
        },
        {
            title: 'Giao hàng',
        ...
        <PageShell>
            ...
            <div className="misa-action-bar">
                ...
                <div className="misa-action-bar-right">
                    ...
                    <Button className="misa-btn-tool" icon={<ReloadOutlined />} onClick={() => void runManualDataLoad(() => refetch(), { success: 'Tải lại danh sách đơn đặt hàng thành công.', failure: 'Không thể tải lại danh sách đơn đặt hàng.' })} title="Nạp lại">
                        Nạp
                    </Button>
                </div>
            </div>
            ...
            <div className="misa-grid-wrapper">
                {isOrdersError ? (
                    <div role="alert" className="misa-p-12 mb-4" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
            ...
            >
                {hasCatalogueError && (
                    <div role="alert" className="misa-mb-8 misa-p-12" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                            <div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Không thể tải danh mục cho đơn đặt hàng</div>
                            <div style={{ color: '#B91C1C', fontSize: 12, marginTop: 2 }}>Không có dữ liệu thay thế; hãy tải lại danh mục khách hàng, nhân viên và hàng hóa trước khi lưu.</div>
                        </div>
                        <Button size="small" onClick={retryCatalogues}>Thử lại danh mục đơn hàng</Button>
                    </div>
                )}
