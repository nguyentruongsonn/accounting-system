...
import { Button, Card, Descriptions, Drawer, Empty, Form, Input, List, Select, Space, Spin, Table, Tag, Typography } from 'antd';
...
const ApArReconciliationInputBoundary: React.FC = () => {
  ...
  if (!canView) return <PageShell title={<PageHeader eyebrow="Báo cáo" title="Bằng chứng cutoff AP/AR" description="Capture-only / fail-closed." />}><div role="alert" className="misa-p-12 mb-4" style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8 }}><div style={{ fontWeight: 600, color: '#92400E', fontSize: 13 }}>Bạn không có quyền xem bằng chứng AP/AR</div><div style={{ color: '#B45309', fontSize: 12, marginTop: 4 }}>{`Cần quyền ${VIEW_PERMISSION}. Giao diện không tải, suy đoán số dư hoặc suy đoán quyền từ tenant hiện tại.`}</div></div></PageShell>;
  ...
  return <PageShell title={<PageHeader eyebrow="Báo cáo" title="Đối chiếu công nợ AP/AR" description="Kết quả do máy chủ tính từ chứng từ đã ghi sổ và cấu hình được admin duyệt." />} toolbar={<PageToolbar actions={<Button icon={<ReloadOutlined />} onClick={() => void runManualDataLoad(() => runs.refetch(), { success: 'Tải lại danh sách đối chiếu công nợ thành công.', failure: 'Không thể tải lại danh sách đối chiếu công nợ.' })} loading={runs.isFetching}>Tải lại danh sách</Button>} />}>
    <Card title={<Space><SafetyCertificateOutlined /> Đối chiếu công nợ AP/AR</Space>} extra={<Tag color="blue">Server-controlled</Tag>}>
      <div className="misa-p-12 misa-mb-12" style={{ background: '#EBF5FF', border: '1px solid #BFDBFE', borderRadius: 8, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
        <LockOutlined style={{ color: 'var(--ui-primary, #0064E0)', fontSize: 16, marginTop: 2 }} />
        <div>
          <div style={{ fontWeight: 600, color: 'var(--ui-primary, #0064E0)', fontSize: 13 }}>Kết quả chỉ có giá trị khi cấu hình đối chiếu đã được admin duyệt</div>
          <div style={{ color: '#1C1E21', fontSize: 12, marginTop: 2 }}>Kế toán có thể yêu cầu chạy theo ngày cutoff. Máy chủ kiểm tra tenant, chứng từ, phân bổ, số dư đầu kỳ và tài khoản kiểm soát; lệch số liệu sẽ bị chặn khóa kỳ.</div>
        </div>
      </div>
      ...
      {runs.isError && <div role="alert" className="misa-p-12 mb-4" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8 }}>
        <div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Không thể tải bằng chứng cutoff AP/AR</div>
        <div style={{ color: '#B91C1C', fontSize: 12, marginTop: 2 }}>Không có dữ liệu dự phòng. Kiểm tra quyền xem, tenant và phản hồi máy chủ.</div>
      </div>}
    ...
    <Drawer title="Chi tiết bằng chứng cutoff AP/AR" open={selectedUuid !== null} onClose={() => setSelectedUuid(null)} size={760} destroyOnHidden>
      ...
      {evidence.isError && <div role="alert" className="misa-p-12 mb-4" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8 }}>
        <div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Không thể tải bằng chứng</div>
        <div style={{ color: '#B91C1C', fontSize: 12, marginTop: 2 }}>Evidence không tồn tại, không thuộc tenant hiện tại hoặc quyền truy cập đã bị máy chủ từ chối.</div>
      </div>}
      {evidence.data && <>
        <div role="alert" className="misa-mb-12 misa-p-12" style={{
          background: evidence.data.amounts_calculated ? (evidence.data.close_authority ? '#EBF5FF' : '#FEF2F2') : '#FFFBEB',
          border: `1px solid ${evidence.data.amounts_calculated ? (evidence.data.close_authority ? '#BFDBFE' : '#FECACA') : '#FDE68A'}`,
          borderRadius: 8
        }}>
          <div style={{ fontWeight: 600, fontSize: 13, color: evidence.data.amounts_calculated ? (evidence.data.close_authority ? 'var(--ui-primary, #0064E0)' : '#991B1B') : '#92400E' }}>
            {evidence.data.amounts_calculated ? (evidence.data.close_authority ? 'Đã đối chiếu khớp' : 'Đối chiếu bị chặn') : 'Chưa có kết quả tính số dư'}
          </div>
          {evidence.data.limitation && <div style={{ fontSize: 12, marginTop: 2, color: evidence.data.amounts_calculated ? (evidence.data.close_authority ? '#1C1E21' : '#B91C1C') : '#B45309' }}>{evidence.data.limitation}</div>}
        </div>
