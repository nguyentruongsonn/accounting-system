...
import { Table, Form, DatePicker, Button, Space, Typography, Select } from 'antd';
...
export default function GeneralLedger({ embedded = false }: GeneralLedgerProps) {
    ...
    const handleSearch = useCallback(async (values: any, silent = false) => {
    ...
    return (
        <PageShell embedded={embedded} className="reports-general-ledger-page">
            ...
            <PageToolbar
                ...
                actions={<Space>
                    <Button icon={<ReloadOutlined />} onClick={() => void runManualDataLoad(() => fetchAccounts(), { success: 'Tải lại danh mục thành công.', failure: 'Không thể tải lại danh mục.' })}>Tải lại danh mục</Button>
            ...
            {accountsLoadError ? (
                <div role="alert" className="misa-p-12 mb-4" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        <div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Không thể tải danh mục tài khoản</div>
                        <div style={{ color: '#B91C1C', fontSize: 12 }}>Không thể chọn tài khoản khi máy chủ chưa trả về danh mục hợp lệ.</div>
                    </div>
                    <Button size="small" onClick={() => void fetchAccounts()}>Thử lại</Button>
                </div>
            ) : null}
