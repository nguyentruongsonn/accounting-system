...
const TrialBalanceReport: React.FC<TrialBalanceReportProps> = ({ embedded = false }) => {
    ...
    const totalOpeningDebit = addDecimalMoney(...postingAccounts.map((account: any) => account.opening_debit));
    const totalOpeningCredit = addDecimalMoney(...postingAccounts.map((account: any) => account.opening_credit));
        ...
        />}>
            <PageToolbar
                filters={<Space>
                    ...
                    />
                    <Button icon={<ReloadOutlined />} onClick={() => void runManualDataLoad(() => refetch(), { success: 'Tải lại bảng cân đối tài khoản thành công.', failure: 'Không thể tải lại bảng cân đối tài khoản.' })}>Tải lại</Button>
                </Space>}
                actions={<Space>
                    ...
                    <Table
                        ...
                        summary={() => (
                            <Table.Summary.Row className="misa-table-summary-row">
                                ...
                                <Table.Summary.Cell index={2} align="right">{formatDecimalMoney(totalOpeningDebit)}</Table.Summary.Cell>
                                <Table.Summary.Cell index={3} align="right">{formatDecimalMoney(totalOpeningCredit)}</Table.Summary.Cell>
            ...
            {compareDecimalMoney(totalOpeningDebit, totalOpeningCredit) !== 0 && (
                <div className="mt-2 text-red-500 font-bold">
                    Cảnh báo: Tổng số dư đầu kỳ Nợ và Có không cân bằng! Lệch: {formatDecimalMoney(absDecimalMoney(subtractDecimalMoney(totalOpeningDebit, totalOpeningCredit)))}
                </div>
            )}
