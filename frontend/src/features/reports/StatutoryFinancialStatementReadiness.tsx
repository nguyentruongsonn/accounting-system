...
import { Button, Card, Descriptions, Empty, Form, Input, InputNumber, List, Space, Spin, Tag, Typography } from 'antd';
...
const StatutoryFinancialStatementReadiness: React.FC = () => {
  ...
  if (!canView) {
    return <PageShell title={<PageHeader eyebrow="Báo cáo" title="Đánh giá sẵn sàng lập BCTC" description="Chỉ đọc / fail-closed; không thực thi lập hoặc phát hành báo cáo." />}><div role="alert" className="misa-p-12 mb-4" style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8 }}><div style={{ fontWeight: 600, color: '#92400E', fontSize: 13 }}>Bạn không có quyền xem readiness BCTC</div><div style={{ color: '#B45309', fontSize: 12, marginTop: 4 }}>{`Cần quyền ${VIEW_PERMISSION}. Giao diện không tải hoặc tự suy đoán catalogue/chứng cứ khi chưa được máy chủ cho phép.`}</div></div></PageShell>;
  }
  ...
  return <PageShell title={<PageHeader eyebrow="Báo cáo" title="Đánh giá sẵn sàng lập BCTC" description="Chỉ đọc catalogue và evidence do máy chủ công bố; không có thao tác thực thi." />} toolbar={<PageToolbar actions={<Button icon={<ReloadOutlined />} disabled={!requiredInput(input)} onClick={() => void runManualDataLoad(() => readiness.refetch(), { success: 'Tải lại bằng chứng readiness thành công.', failure: 'Không thể tải lại bằng chứng readiness.' })}>Tải lại bằng chứng</Button>} />}>
    <Card title={<Space><SafetyCertificateOutlined /> Đánh giá sẵn sàng lập BCTC</Space>} extra={<Tag color="orange">Chỉ đọc / fail-closed</Tag>}>
      <div className="misa-mb-12 misa-p-12" style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8, display: 'flex', gap: 10, alignItems: 'flex-start' }}><LockOutlined style={{ color: '#D97706', fontSize: 16, marginTop: 2 }} /><div><div style={{ fontWeight: 600, color: '#92400E', fontSize: 13 }}>Không phải màn hình lập hoặc phát hành BCTC</div><div style={{ color: '#B45309', fontSize: 12, marginTop: 2 }}>Màn hình chỉ đọc catalogue và chứng cứ do máy chủ công bố. Không có số liệu, lưu chuyển tiền tệ, thuyết minh, kết quả thuế, ký số, tải xuống hoặc nộp hồ sơ tại đây.</div></div></div>
      ...
      {readiness.isError && <div role="alert" className="misa-p-12 mb-4" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8 }}><div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Không thể xác minh readiness BCTC</div><div style={{ color: '#B91C1C', fontSize: 12, marginTop: 2 }}>Không có dữ liệu dự phòng. Kiểm tra ID năm tài chính, mã biểu mẫu, ngày cutoff, quyền reports.view và phản hồi máy chủ.</div></div>}
    ...
    {result && <>
      <Card className="misa-mt-16" title={<Space><FileProtectOutlined /> Phạm vi đánh giá từ máy chủ</Space>} extra={<Tag color="red">Không cho thực thi</Tag>}>
        ...
        <div role="alert" className="misa-mt-12 misa-p-12" style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8 }}><div style={{ fontWeight: 600, color: '#92400E', fontSize: 13 }}>Backend chưa cấp quyền chạy BCTC</div><div style={{ color: '#B45309', fontSize: 12, marginTop: 2 }}>{result.meta.disclaimer}</div></div>
