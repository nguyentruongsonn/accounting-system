  ...
  open ? <div className="misa-modal-overlay" role="dialog" aria-modal="true" aria-label="Thông tin người dùng">
    ...
    <ModalFrame>
      <form onSubmit={onSubmit} className="misa-form-stack" aria-label="Thông tin người dùng">
        ...
        {error && <div role="alert" className="misa-form-error-text" style={{ color: '#ff4d4f', marginBottom: 12 }}>{error}</div>}
        ...
        <Form.Item label="Lý do thay đổi">
          <Input.TextArea aria-label="Lý do thay đổi" rows={3} maxLength={2000} value={values.reason} onChange={event => onChange({ ...values, reason: event.target.value })} placeholder="Ghi rõ lý do cấp, sửa hoặc thu hồi quyền" />
        </Form.Item>
