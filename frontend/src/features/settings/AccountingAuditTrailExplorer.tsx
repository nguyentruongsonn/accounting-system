...
const AccountingAuditTrailExplorer: React.FC = () => {
  ...
  if (!canView) return <PageShell title={<PageHeader eyebrow="Thiết lập" title="Truy vết kiểm toán kế toán" description="Tra cứu chứng cứ nghiệp vụ theo tenant; chỉ đọc." />}><Alert type="warning" showIcon title="Bạn không có quyền xem audit trail" description={`Cần quyền ${VIEW_PERMISSION}. Hệ thống không tải hoặc suy đoán dữ liệu khi chưa có quyền.`} /></PageShell>;
  ...
  return <PageShell title={<PageHeader eyebrow="Thiết lập" title="Truy vết kiểm toán kế toán" description="Tra cứu chứng cứ nghiệp vụ theo tenant; chỉ đọc." />} toolbar={<PageToolbar actions={<Button icon={<ReloadOutlined />} onClick={() => void audits.refetch()}>Tải lại</Button>} />}>
    ...
    {audits.isError ? <Alert type="error" showIcon title="Không thể tải audit trail" description="Không có dữ liệu thay thế. Hãy kiểm tra quyền, bộ lọc và kết nối máy chủ." action={<Button size="small" onClick={() => void audits.refetch()}>Thử lại audit trail</Button>} /> : <DataTableSurface><Table rowKey="id" columns={columns} dataSource={audits.data?.data ?? []} loading={audits.isLoading} locale={{ emptyText: 'Không có dữ liệu trong phạm vi đã chọn.' }} pagination={{ current: page, pageSize: audits.data?.meta.per_page ?? 25, total: audits.data?.meta.total ?? 0, showSizeChanger: false, onChange: setPage }} /></DataTableSurface>}
    <Drawer title="Trace chứng cứ kế toán" open={traceTarget !== null} onClose={() => setTraceTarget(null)} size={760} destroyOnHidden>
      ...
      {trace.isError && <Alert type="error" showIcon title="Không thể tải trace" description="Trace không tồn tại, không thuộc tenant hiện tại hoặc máy chủ từ chối yêu cầu." action={<Button size="small" onClick={() => void trace.refetch()}>Thử lại trace</Button>} />}
      {trace.data && <>
