...
const toMoneyMinorUnits = (value: OpeningBalanceDecimal): bigint | null => {
  const parsed = parseDecimal(value);
  if (!parsed) return null;
  if (parsed.scale <= 2) return (parsed.negative ? -parsed.digits : parsed.digits) * 10n ** BigInt(2 - parsed.scale);

  const divisor = 10n ** BigInt(parsed.scale - 2);
  const quotient = parsed.digits / divisor;
  const rounded = quotient + (parsed.digits % divisor * 2n >= divisor ? 1n : 0n);
  return parsed.negative ? -rounded : rounded;
};
...
export function subtractOpeningBalanceValue(left: OpeningBalanceDecimal, right: OpeningBalanceDecimal): string {
  const leftMinorUnits = toMoneyMinorUnits(left);
  const rightMinorUnits = toMoneyMinorUnits(right);
  if (leftMinorUnits === null || rightMinorUnits === null) return '0.00';
  return formatMoneyMinorUnits(leftMinorUnits - rightMinorUnits);
}

export function openingBalanceDifference(totalDebit: OpeningBalanceDecimal, totalCredit: OpeningBalanceDecimal): { side: 'debit' | 'credit' | null; amount: string } {
  const debitMinorUnits = toMoneyMinorUnits(totalDebit);
  const creditMinorUnits = toMoneyMinorUnits(totalCredit);
  if (debitMinorUnits === null || creditMinorUnits === null || debitMinorUnits === creditMinorUnits) {
    return { side: null, amount: '0.00' };
  }

  return debitMinorUnits > creditMinorUnits
    ? { side: 'credit', amount: formatMoneyMinorUnits(debitMinorUnits - creditMinorUnits) }
    : { side: 'debit', amount: formatMoneyMinorUnits(creditMinorUnits - debitMinorUnits) };
}

export function hasZeroOpeningBalance(debit: OpeningBalanceDecimal, credit: OpeningBalanceDecimal): boolean {
  const debitMinorUnits = toMoneyMinorUnits(debit);
  const creditMinorUnits = toMoneyMinorUnits(credit);
  return debitMinorUnits === null || creditMinorUnits === null || (debitMinorUnits === 0n && creditMinorUnits === 0n);
}
