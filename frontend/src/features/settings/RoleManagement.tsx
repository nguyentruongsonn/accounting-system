...
const emptyDraft: UserDraft = { name: '', email: '', role: '', is_active: true, password: '', password_confirmation: '', reason: '' };
...
const RoleManagement: React.FC = () => {
  ...
  const remove = () => {
    ...
    void (async () => {
      ...
      try {
        const config = draft.reason ? { data: { reason: draft.reason } } : undefined;
        await api.delete(`/users/${editing}`, config);
        setEditing(null); setDraft({ ...emptyDraft }); await loadUsers();
      }
          ...
          <div data-testid="user-directory-table" aria-busy={loading ? 'true' : 'false'}>
            <Table<ManagedUser>
              ...
              columns={[
                ...
                { title: 'Trạng thái', dataIndex: 'is_active', key: 'is_active', align: 'center' as const, width: 140, render: (active: boolean) => (
                ...
                { title: 'Chức năng', key: 'actions', align: 'center' as const, width: 120, render: (_value: unknown, user: ManagedUser) => <MisaButton aria-label={`Sửa ${user.name}`} onClick={() => edit(user)}>Sửa</MisaButton> },
              ]}
            />
          </div>
        </DataTableSurface>
