...
import { Button, Descriptions, Drawer, Form, Input, Popconfirm, Select, Space, Table, Typography } from 'antd';
...
const NoticeBox: React.FC<{
  ...
  className?: string;
}> = ({ type = 'info', message, description, action, className = '' }) => {
  ...
  return (
    <div
      ...
      className={`misa-p-12 mb-4 ${className}`}
      style={{
        background: bg,
        border,
        borderRadius: 8,
    ...
    >
      <div>
        {message && <div style={{ fontWeight: 600, color: titleColor, fontSize: 13 }}>{message}</div>}
...
const ApprovedAccountMappings: React.FC = () => {
  ...
  const columns = useMemo(() => [
    ...
    { title: 'Trạng thái', dataIndex: 'status', key: 'status', width: 130, align: 'center' as const, render: (value: MappingStatus) => (
      <span className={`misa-apple-pill ${value === 'approved' ? 'misa-apple-pill-green' : 'misa-apple-pill-orange'}`}>
        <span className="misa-apple-pill-dot" />
        {value === 'approved' ? 'Đã phê duyệt' : 'Dự thảo'}
      </span>
    ) },
    {
      title: 'Chức năng', key: 'action', width: 185, render: (_: unknown, mapping: ApprovedAccountMapping) => (
        <Space size="small">
  ...
  const policyColumns = useMemo(() => [
    ...
    { title: 'Trạng thái', dataIndex: 'status', key: 'status', width: 130, align: 'center' as const, render: (value: MappingStatus) => (
      <span className={`misa-apple-pill ${value === 'approved' ? 'misa-apple-pill-green' : 'misa-apple-pill-orange'}`}>
        <span className="misa-apple-pill-dot" />
        {value === 'approved' ? 'Đã duyệt' : 'Dự thảo'}
      </span>
    ) },
    {
      title: 'Chức năng', key: 'actions', width: 220, render: (_: unknown, policy: AccountingPolicy) => policy.status === 'draft'
  ...
  if (!canView) {
    return <PageShell title={<PageHeader eyebrow="Thiết lập" title="Mapping tài khoản được phê duyệt" description="Chứng cứ mapping theo policy và ngày hiệu lực; không tự sinh danh mục." />}><div role="alert" className="misa-p-12 mb-4" style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8 }}><div style={{ fontWeight: 600, color: '#92400E', fontSize: 13 }}>Bạn không có quyền xem mapping tài khoản</div><div style={{ color: '#B45309', fontSize: 12, marginTop: 4 }}>{`Cần quyền ${permission.view}. Hệ thống không tải hay suy đoán dữ liệu khi chưa có quyền.`}</div></div></PageShell>;
  }

  return (
    <PageShell title={<PageHeader eyebrow="Thiết lập" title="Cấu hình tài khoản hạch toán" description="Thiết lập nâng cao; không cần mở trang này để ghi sổ ở chế độ trực tiếp." />} toolbar={<PageToolbar filters={<Space><span>Trạng thái:</span><Select value={status} onChange={setStatus} style={{ width: 160 }} options={[{ value: 'all', label: 'Tất cả' }, { value: 'draft', label: 'Dự thảo' }, { value: 'approved', label: 'Đã phê duyệt' }]} /></Space>} actions={<Space><Button icon={<ReloadOutlined />} onClick={() => void runManualDataLoad(() => mappingsQuery.refetch(), { success: 'Tải lại mapping tài khoản thành công.', failure: 'Không thể tải lại mapping tài khoản.' })}>Tải lại</Button><Button icon={<SettingOutlined />} onClick={() => setPolicyDrawerOpen(true)}>Chính sách hạch toán</Button>{canCreate && <Button type="primary" className="misa-btn-primary" icon={<PlusOutlined />} onClick={openCreate}>Tạo dự thảo</Button>}</Space>} />}>
      {mappingsQuery.isError ? <NoticeBox type="error" message="Không thể tải mapping tài khoản" description="Không hiển thị dữ liệu thay thế. Hãy kiểm tra quyền hoặc kết nối máy chủ." action={<Button size="small" onClick={() => void runManualDataLoad(() => mappingsQuery.refetch(), { success: 'Tải lại mapping tài khoản thành công.', failure: 'Không thể tải lại mapping tài khoản.' })}>Thử lại mapping</Button>} /> : <DataTableSurface className="approved-mappings-table-surface"><Table rowKey="id" columns={columns} dataSource={mappingsQuery.data ?? []} loading={mappingsQuery.isLoading} pagination={{ pageSize: 50 }} /></DataTableSurface>}
      ...
      <Modal title={editing ? 'Sửa dự thảo mapping' : 'Tạo dự thảo mapping'} open={formOpen} onCancel={() => { setFormOpen(false); setEditing(null); }} onOk={() => form.submit()} okText={editing ? 'Lưu dự thảo' : 'Tạo dự thảo'} confirmLoading={saveMutation.isPending} width={760} destroyOnHidden>
        <ModalFrame>
        <div className="mb-4 misa-p-12" style={{ background: '#EBF5FF', border: '1px solid #BFDBFE', borderRadius: 8 }}>
          <div style={{ fontWeight: 600, color: 'var(--ui-primary, #0064E0)', fontSize: 13 }}>Thiết lập nâng cao</div>
          <div style={{ color: '#1C1E21', fontSize: 12, marginTop: 2 }}>Chỉ dùng khi doanh nghiệp bật kiểm soát policy/mapping; không cần thao tác ở đây để ghi sổ trực tiếp.</div>
        </div>
        {policiesQuery.isError && <NoticeBox type="error" message="Không tải được policy đã duyệt" description="Không thể chọn policy khi máy chủ chưa trả về dữ liệu; không tự tạo policy thay thế." action={<Button size="small" onClick={() => void runManualDataLoad(() => policiesQuery.refetch(), { success: 'Tải lại policy thành công.', failure: 'Không thể tải lại policy.' })}>Thử lại policy</Button>} />}
        {accountsQuery.isError && <NoticeBox type="error" message="Không tải được danh mục tài khoản" description="Không thể chọn tài khoản khi danh mục chưa được xác minh; không nhập mã thay thế." action={<Button size="small" onClick={() => void runManualDataLoad(() => accountsQuery.refetch(), { success: 'Tải lại danh mục tài khoản thành công.', failure: 'Không thể tải lại danh mục tài khoản.' })}>Thử lại danh mục tài khoản</Button>} />}
          ...
          <Form.Item label="Điều kiện context" extra="Khai báo từng trường mà backend dùng để phân biệt mapping. Giá trị số (ví dụ item_id, warehouse_id) được giữ đúng kiểu; không cần sửa JSON.">
            <Form.List name="context_entries">
              {(fields, { add, remove }) => <Space direction="vertical" className="w-full" size="small">
                {selectedMappingKey === 'period_close.result' ? <div className="mb-2 misa-p-12" style={{ background: '#EBF5FF', border: '1px solid #BFDBFE', borderRadius: 8 }}><div style={{ fontWeight: 600, color: 'var(--ui-primary, #0064E0)', fontSize: 13 }}>Kết chuyển cuối kỳ dùng context trống</div><div style={{ color: '#1C1E21', fontSize: 12, marginTop: 2 }}>Hai vai trò kết quả kinh doanh được phân giải trực tiếp theo policy và ngày hiệu lực; không cần nhập điều kiện JSON.</div></div> : fields.map((field) => <Space key={field.key} align="start" className="w-full" wrap>
      ...
      <Drawer title="Danh sách chính sách hạch toán" open={policyDrawerOpen} onClose={() => setPolicyDrawerOpen(false)} size={920} extra={canCreate ? <Button type="primary" icon={<PlusOutlined />} onClick={openPolicyCreate}>Tạo chính sách</Button> : undefined}>
        <div className="mb-4 misa-p-12" style={{ background: '#EBF5FF', border: '1px solid #BFDBFE', borderRadius: 8 }}>
          <div style={{ fontWeight: 600, color: 'var(--ui-primary, #0064E0)', fontSize: 13 }}>Quy trình hai người</div>
          <div style={{ color: '#1C1E21', fontSize: 12, marginTop: 2 }}>Kế toán lập dự thảo bằng các trường nghiệp vụ. Admin khác người lập kiểm tra và duyệt; bản đã duyệt không sửa trực tiếp.</div>
        </div>
        {policyListQuery.isError
          ? <NoticeBox type="error" message="Không thể tải chính sách hạch toán" description="Hệ thống không dùng dữ liệu giả thay thế. Hãy kiểm tra quyền hoặc kết nối máy chủ." action={<Button size="small" onClick={() => void runManualDataLoad(() => policyListQuery.refetch(), { success: 'Tải lại chính sách hạch toán thành công.', failure: 'Không thể tải lại chính sách hạch toán.' })}>Thử lại chính sách</Button>} />
      ...
      <Modal title={editingPolicy ? 'Sửa dự thảo chính sách' : 'Tạo chính sách hạch toán'} open={policyFormOpen} onCancel={() => { setPolicyFormOpen(false); setEditingPolicy(null); }} onOk={() => policyForm.submit()} okText={editingPolicy ? 'Lưu dự thảo' : 'Tạo dự thảo'} confirmLoading={savePolicyMutation.isPending} width={820} destroyOnHidden>
        <ModalFrame>
          {profilesQuery.isError && <NoticeBox type="error" message="Không tải được năm tài chính" description="Không thể chọn kỳ áp dụng khi máy chủ chưa trả về profile; không tự chọn năm thay thế." action={<Button size="small" onClick={() => void runManualDataLoad(() => profilesQuery.refetch(), { success: 'Tải lại năm tài chính thành công.', failure: 'Không thể tải lại năm tài chính.' })}>Thử lại năm tài chính</Button>} />}
          {accountsQuery.isError && <NoticeBox type="error" message="Không tải được danh mục tài khoản" description="Không thể chọn tài khoản nguồn khi danh mục chưa được xác minh; không nhập mã thay thế." action={<Button size="small" onClick={() => void runManualDataLoad(() => accountsQuery.refetch(), { success: 'Tải lại danh mục tài khoản thành công.', failure: 'Không thể tải lại danh mục tài khoản.' })}>Thử lại danh mục tài khoản</Button>} />}
      ...
      {detail && <>
          {detail.is_immutable ? <div className="mb-4 misa-p-12" style={{ background: '#EBF5FF', border: '1px solid #BFDBFE', borderRadius: 8 }}><div style={{ fontWeight: 600, color: 'var(--ui-primary, #0064E0)', fontSize: 13 }}>Bản ghi đã phê duyệt và bất biến</div><div style={{ color: '#1C1E21', fontSize: 12, marginTop: 2 }}>{detail.limitation}</div></div> : <Typography.Text className="mb-4 block" type="secondary">Bản ghi đang là dự thảo. {detail.limitation}</Typography.Text>}
          ...
          {detail.status === 'draft' && cannotSelfApprove(detail) && <div role="alert" className="mt-4 misa-p-12" style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8 }}><div style={{ fontWeight: 600, color: '#92400E', fontSize: 13 }}>Không thể tự phê duyệt</div><div style={{ color: '#B45309', fontSize: 12, marginTop: 2 }}>Dự thảo do chính bạn lập; một người có quyền phê duyệt khác phải thực hiện thao tác này.</div></div>}
        </>}
      </Drawer>
    </PageShell>
  );
