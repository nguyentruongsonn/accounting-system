...
const issueVoucherTypes = [
    { value: '1. Xuất kho mua hàng', label: '1. Xuất kho mua hàng' },
    { value: '2. Xuất kho từ sản xuất', label: '2. Xuất kho từ sản xuất' },
    { value: '3. Xuất kho khác', label: '3. Xuất kho khác' },
    { value: '4. Xuất kho hàng trả lại', label: '4. Xuất kho hàng trả lại' },
];

export const InventoryIssues: React.FC<{ embedded?: boolean; openDraftId?: number | null; onDraftOpened?: () => void }> = ({ embedded = false, openDraftId = null, onDraftOpened }) => {
    ...
    const formLines: InventoryIssueLine[] = Form.useWatch('lines', form) || [];
    ...
    const effectiveVoucherType = voucherType ?? issueVoucherTypes[0].value;
    ...
    const { data: customers = [], isError: isCustomersError, refetch: refetchCustomers } = useQuery({
        ...
        queryFn: async () => {
            ...
            return parseInventoryContactCatalogue(data, 'customer');
        },
    });
    ...
    const hasCatalogueError = isCustomersError || isItemsError || isAccountsError || isWarehousesError;
    ...
    const currentLines: InventoryIssueLine[] = formLines.length > 0 ? formLines : (form.getFieldValue('lines') || []);
    ...
    const mutation = useMutation({
        mutationFn: async (payloadWithFlags: any) => {
            ...
            const payload = {
                ...
                voucher_type: values.voucher_type ?? effectiveVoucherType,
            ...
            const res = editingId
                ? await api.put(`/inventory/issues/${editingId}`, payload)
                : await api.post('/inventory/issues', payload);
            requirePersistedInventoryDocument(res.data);
            return { res, andNew, andPrint, editingId };
        },
        onSuccess: (data: any) => {
            message.success(data.editingId ? 'Đã cập nhật phiếu xuất kho nháp.' : 'Tạo Phiếu xuất kho thành công!');
            const saved = data?.res?.data?.data ?? data?.res?.data;
            if (saved && saved.id) {
                instantUpsertVoucher(queryClient, ['inventory-issues'], saved);
            }
    ...
    const postMutation = useMutation({
        mutationFn: async (id: number) => {
            const response = await api.post(`/inventory/issues/${id}/post`);
            requirePersistedInventoryDocument(response.data, true);
            return response;
        },
        onMutate: async (id: number) => {
            return optimisticTogglePostStatus(queryClient, ['inventory-issues'], id, true, { status: 'posted' });
        },
        onError: (err: any, _id, context) => {
            rollbackVoucherCache(queryClient, ['inventory-issues'], context);
    ...
    const unpostMutation = useMutation({
        mutationFn: async (id: number) => {
            const response = await api.post(`/inventory/issues/${id}/unpost`);
            return response;
        },
        onMutate: async (id: number) => {
            return optimisticTogglePostStatus(queryClient, ['inventory-issues'], id, false, { status: 'draft' });
        },
        onError: (err: any, _id, context) => {
            rollbackVoucherCache(queryClient, ['inventory-issues'], context);
    ...
    const duplicateMutation = useMutation({
        mutationFn: async (id: number) => {
            const response = await api.post(`/inventory/issues/${id}/duplicate`);
            return response;
        },
        onSuccess: () => {
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id: number) => {
            return api.delete(`/inventory/issues/${id}`);
        },
        onMutate: async (id: number) => {
            return instantRemoveVoucher(queryClient, ['inventory-issues'], id);
        },
        onError: (err: any, _id, context) => {
            rollbackVoucherCache(queryClient, ['inventory-issues'], context);
    ...
    const handleOpenModal = React.useCallback(() => {
        ...
        form.setFieldsValue({
            lines: [],
            voucher_type: issueVoucherTypes[0].value,
            currency: 'VND',
            exchange_rate: 1,
            voucher_date: dayjs(),
            posting_date: dayjs(),
        });
        api.get('/inventory/issues/next-code').then(({ data }) => {
    ...
    const handleEditModal = (record: InventoryIssueRecord) => {
        setIsViewMode(false);
        ...
        const mappedType = issueVoucherTypes.find(t => t.value === record.voucher_type)?.value
            || (record.voucher_type?.includes('ban') ? issueVoucherTypes[0].value : issueVoucherTypes[0].value);
        form.setFieldsValue({
    ...
    const handleViewModal = async (record: InventoryIssueRecord) => {
        try {
            const { data } = await api.get(`/inventory/issues/${record.id}`);
            const persisted = requirePersistedInventoryDocument(data);
            if (persisted.id !== record.id) throw new Error('Máy chủ không trả về đúng phiếu xuất kho được yêu cầu.');
            handleEditModal((data?.data ?? data) as InventoryIssueRecord);
            setIsViewMode(true);
        } catch (error) {
            message.error(getInventoryMutationError(error, 'Không thể mở chi tiết phiếu xuất kho.'));
        }
    };

    useEffect(() => {
        if (isModalVisible && !form.getFieldValue('voucher_type')) {
            form.setFieldValue('voucher_type', issueVoucherTypes[0].value);
        }
    }, [form, isModalVisible]);

    useEffect(() => {
        const handleCreate = () => handleOpenModal();
        window.addEventListener('open-inventory-issue', handleCreate);
        window.addEventListener('open-inventory-receipt', handleCreate);
        return () => {
            window.removeEventListener('open-inventory-issue', handleCreate);
    ...
    useEffect(() => {
        ...
        window.addEventListener('search-inventory-issue', handleSearch);
        return () => window.removeEventListener('search-inventory-issue', handleSearch);
    }, []);

    const filteredIssues = useMemo(() => {
        const list = issues.map((v: any) => ({
            ...v,
    ...
    useEffect(() => {
        ...
        let active = true;
        api.get(`/inventory/issues/${openDraftId}`)
            .then(({ data }) => {
                const persisted = requirePersistedInventoryDocument(data);
                if (!active || persisted.is_posted === true || persisted.status === 'posted') return;
                const record = (data?.data ?? data) as InventoryIssueRecord;
                setEditingId(record.id);
                form.setFieldsValue({
                    voucher_type: record.voucher_type ?? issueVoucherTypes[0].value,
                    voucher_number: record.voucher_number,
                    voucher_date: record.voucher_date ? dayjs(record.voucher_date) : undefined,
                    posting_date: record.posting_date ? dayjs(record.posting_date) : undefined,
                    description: record.description,
    ...
    useVoucherShortcuts({
        ...
        onClose: () => {
            if (!isCustomerModalVisible && !isItemModalVisible) {
                setIsModalVisible(false);
            }
        },
    ...
    const handleItemChange = (index: number, itemId: number) => {
        ...
        if (item) {
            ...
            curLines[index] = {
                ...
                description: curLines[index]?.description || `Xuất kho ${item.name}`,
    ...
    const columns: ColumnsType<InventoryIssueRecord> = [
        ...
        },
        {
            ...
            width: 120,
            align: 'center',
            render: (_: boolean, record: InventoryIssueRecord) => (
        ...
        },
        {
            ...
            width: 140,
            align: 'center',
            render: (_: any, record: InventoryIssueRecord) => {
        ...
        <PageShell
            ...
            title={(
                <PageHeader
                    ...
                    description="Quản lý và lập phiếu xuất kho theo chứng từ thực tế."
                />
            )}
            toolbar={!embedded && (
            <PageToolbar
                ...
                <Table
                    ...
                    locale={{ emptyText: (
                        <div className="misa-empty-box">
                            ...
                            <div className="misa-empty-text">Chưa có chứng từ xuất kho nào</div>
                        </div>
                    )}}
                />
            </DataTableSurface>
                    ...
                    <div className="misa-modal-header-wrapper">
                        <div className="misa-modal-header-left">
                            <span className="misa-voucher-header-title">
                                Phiếu xuất kho {voucherNumber ? `: ${voucherNumber}` : ''}
                            </span>
                            {isViewMode && (
                            ...
                            <div className="misa-input-group">
                                <Select
                                    value={voucherType ?? issueVoucherTypes[0].value}
                                    disabled={isViewMode}
                                    onChange={(value) => form.setFieldValue('voucher_type', value)}
                                    variant="borderless"
                                    className="misa-input-w230 misa-text-semibold"
                                    popupMatchSelectWidth={false}
                                    options={issueVoucherTypes.map(t => ({ value: t.value, label: t.label }))}
                                />
                            </div>
                        </div>
            ...
            >
                <ModalFrame>
                    {hasCatalogueError && (
                        <div role="alert" className="misa-p-12 misa-mb-12" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <div>
                                <div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Không thể tải danh mục cho phiếu xuất kho</div>
                                <div style={{ color: '#B91C1C', fontSize: 12 }}>Hệ thống không thể tải đầy đủ danh mục hàng hóa, khách hàng, kho hoặc tài khoản để lập chứng từ.</div>
                            </div>
                            <Button size="small" onClick={retryCatalogues}>Thử lại danh mục phiếu xuất kho</Button>
                        </div>
                    )}
                    <Form form={form} preserve={true} layout="vertical" size="small" className="misa-form-flex-col" initialValues={{ voucher_type: issueVoucherTypes[0].value }}>
                        <Form.Item name="voucher_type" hidden initialValue={issueVoucherTypes[0].value}>
                            <Input />
                        </Form.Item>
                        {/* Top Master Card */}
                        <MisaMasterCard>
                                ...
                                <MisaMasterCard.FormGrid>
                                            ...
                                            <MultiColumnContactSelect
                                                ...
                                                onChange={(val, item) => {
                                                    form.setFieldsValue({
                                                        ...
                                                        description: `Xuất kho cho ${item?.name || ''}`
                                                    });
                                                }}
                                    ...
                                    <div className="misa-col-8">
                                        ...
                                        <Form.Item name="contact_name" noStyle>
                                            <Input className="misa-input" placeholder="Tên đối tượng / khách hàng" disabled={isViewMode} />
                                        </Form.Item>
                                    </div>
                                    ...
                                    <div className="misa-col-4">
                                        ...
                                        <Form.Item name="receiver_name" noStyle>
                                            <Input className="misa-input" placeholder="Họ và tên người nhận" disabled={isViewMode} />
                                        </Form.Item>
                                    </div>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                        ...
                                        {!isViewMode && !editingId && (
                                            <Button
                                                ...
                                                onClick={async () => {
                                                    try {
                                                        const { data } = await api.get('/inventory/issues/next-code');
                                                        const nextCode = data?.data?.code || data?.code || data?.data;
                                                        if (nextCode) form.setFieldsValue({ voucher_number: nextCode });
                                                    } catch {}
                                                }}
                                        ...
                                        <div className="misa-detail-flex-col">
                                                    ...
                                                    <table className="misa-voucher-table">
                                                        <thead>
                                                            <tr>
                                                                ...
                                                                <th className="misa-input-w120 misa-text-right">Đơn giá xuất</th>
                                                    ...
                                                    <table className="misa-voucher-table">
                                                        <thead>
                                                            <tr>
                                                                ...
                                                                <th className="misa-input-w160">Đơn đặt hàng</th>
                                                                <th className="misa-text-center"></th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <td>
                                                                            <Form.Item name={[field.name, 'order_number']} noStyle>
                                                                                <Input variant="borderless" className="misa-table-input" placeholder="Đơn đặt hàng..." disabled={isViewMode} />
                                                                            </Form.Item>
                                                                        </td>
                                                                        <td className="misa-text-center">
                                                                            <button
                                            ...
                                            <MisaGridActionFooter
                                                ...
                                                onDeleteAll={() => {
                                                    ...
                                                    message.success('Đã xóa toàn bộ dòng xuất kho');
                                                }}
            ...
            <QuickAddContactModal
                ...
                onSuccess={(newSupp) => {
                    ...
                    form.setFieldsValue({
                        ...
                        receiver_name: newSupp.contact_person || newSupp.name
                    });
                }}
            />
                ...
                onSuccess={(newItem: any) => {
                    ...
                    if (activeRowIndex !== null && newItem?.id) {
                        ...
                        curLines[activeRowIndex] = {
                            ...
                            unit_price: price,
                            amount: price * quantity,
                            description: `Xuất kho ${newItem.name}`,
                        };
                        form.setFieldsValue({ lines: [...curLines] });
                    }
