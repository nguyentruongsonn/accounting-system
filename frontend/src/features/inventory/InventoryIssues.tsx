...
export const InventoryIssues: React.FC<{ embedded?: boolean; openDraftId?: number | null; onDraftOpened?: () => void }> = ({ embedded = false, openDraftId = null, onDraftOpened }) => {
    ...
    const handleOpenModal = React.useCallback(() => {
        ...
        setIsViewMode(false);
        form.resetFields();
        form.setFieldsValue({ lines: [], voucher_type: issueVoucherTypes[0].value });
    ...
    useEffect(() => {
        ...
        return () => window.removeEventListener('refresh-inventory-issue', handleRefresh);
    }, [queryClient]);
                ...
                actions={(
                    <div className="misa-toolbar-right flex items-center gap-2">
                        <Button 
                            ...
                            title="Làm mới (F5)" 
                            icon={<ReloadOutlined />}
                            onClick={() => queryClient.invalidateQueries({ queryKey: ['inventory-issues'] })}
                        />
                        <Button 
                            className="misa-btn-tool" 
                            title="In danh sách" 
            ...
            <DataTableSurface className="misa-voucher-surface">
                {isIssuesError ? (
                    <Alert
                        type="error"
                        showIcon
                        message="Không thể tải danh sách phiếu xuất kho"
                        action={<Button size="small" onClick={() => void refetchIssues()}>Thử lại danh sách phiếu xuất kho</Button>}
                    />
                ) : null}
                <Table 
                    columns={columns} 
                ...
                <ModalFrame>
                    {hasCatalogueError && (
                        <Alert
                            type="error"
                            showIcon
                            title="Không thể tải danh mục cho phiếu xuất kho"
                            description="Hệ thống không thể tải đầy đủ danh mục hàng hóa, khách hàng, kho hoặc tài khoản để lập chứng từ."
                            action={<Button size="small" onClick={retryCatalogues}>Thử lại danh mục phiếu xuất kho</Button>}
                            className="misa-mb-12"
                        />
                    )}
                    <Form form={form} layout="vertical" size="small" className="misa-form-flex-col" initialValues={{ voucher_type: issueVoucherTypes[0].value }}>
                                                                ...
                                                                >
                                                                    ...
                                                                    {activeGridTab === 'items' ? (
                                                                        ...
                                                                        <>
                                                                            ...
                                                                            <td><input className="misa-table-input" placeholder="Đơn đặt hàng..." disabled={isViewMode} /></td>
                                                                        </>
                                                                    )}
                            ...
                            if (Array.isArray(v.lines) && v.lines.length > 0) {
                                v.lines.forEach((l: any) => {
                                    lineCandidates.push({
                                        ...
                                        debit_account: l.debit_account || '632',
                                        credit_account: l.credit_account || '1561',
                                        unit: l.unit,
                                        quantity: l.quantity,
