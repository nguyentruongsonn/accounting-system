...
const StockReportContent: React.FC<{ embedded?: boolean }> = ({ embedded = false }) => {
    ...
    const reportActions = (
        <div className="stock-report-actions flex flex-wrap items-center justify-end gap-2" data-ui="stock-report-actions">
            ...
            <Button
                icon={<ReloadOutlined />}
                onClick={() => void runManualDataLoad(() => refetch(), { success: 'Tải lại báo cáo tồn kho thành công.', failure: 'Không thể tải lại báo cáo tồn kho.' })}
                className="misa-btn-tool"
                title="Làm mới (F5)"
                loading={isLoading}
            />
    ...
    return (
        <PageShell embedded={embedded} title={<PageHeader
            ...
            />}>
            {!embedded && <PageToolbar leading={catalogueUnavailable ? <div role="alert" className="misa-p-8" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 6, maxWidth: 500 }}><div style={{ fontWeight: 600, color: '#991B1B', fontSize: 12 }}>Không thể tải danh mục lọc tồn kho</div><div style={{ color: '#B91C1C', fontSize: 11 }}>Không có dữ liệu thay thế; yêu cầu báo cáo bị khóa cho đến khi máy chủ trả đúng danh mục kho/hàng hóa.</div></div> : undefined} filters={(
            ...
            {embedded && (
                <PageToolbar filters={(
                    <>
                    {catalogueUnavailable && <div role="alert" className="misa-p-8 mb-3" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 6, maxWidth: 500 }}><div style={{ fontWeight: 600, color: '#991B1B', fontSize: 12 }}>Không thể tải danh mục lọc tồn kho</div><div style={{ color: '#B91C1C', fontSize: 11 }}>Không có dữ liệu thay thế; yêu cầu báo cáo bị khóa cho đến khi máy chủ trả đúng danh mục kho/hàng hóa.</div></div>}
                    <Form<StockReportFormValues> form={form} layout="inline" onFinish={handleSearch}>
