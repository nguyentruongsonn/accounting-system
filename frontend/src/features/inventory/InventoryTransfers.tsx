...
const InventoryTransfers: React.FC<InventoryTransfersProps> = ({ embedded = false }) => {
      ...
      actions={
        <div className="misa-toolbar-right flex items-center gap-2">
          <Button
            ...
            title="Làm mới (F5)"
            onClick={() => void runManualDataLoad(() => transfers.refetch(), { success: 'Tải lại phiếu điều chuyển thành công.', failure: 'Không thể tải lại phiếu điều chuyển.' })}
          />
          <Button
  ...
  return (
    ...
    >
      {transfers.error && <div role="alert" className="misa-p-12 misa-mb-16" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
      ...
      {catalogueError && <div role="alert" className="misa-p-12 misa-mb-16" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <div style={{ color: '#B91C1C', fontSize: 12 }}>Không dùng danh sách rỗng thay thế; hãy tải lại danh mục từ máy chủ trước khi lập hoặc lưu phiếu.</div>
        </div>
        <Button size="small" onClick={retryCatalogues}>Thử lại danh mục điều chuyển</Button>
      </div>}
      <DataTableSurface>
        <Table<Transfer>
          rowKey="id"
      ...
      >
        <ModalFrame>
          {catalogueError && <div role="alert" className="mb-4 misa-p-12" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Chưa thể xác thực danh mục điều chuyển</div>
              <div style={{ color: '#B91C1C', fontSize: 12 }}>Kho và hàng hóa phải được tải thành công từ máy chủ trước khi cất chứng từ.</div>
            </div>
            <Button size="small" onClick={retryCatalogues}>Thử lại danh mục điều chuyển</Button>
          </div>}
