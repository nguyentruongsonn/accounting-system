...
import { Button, DatePicker, Form, Input, InputNumber, Popconfirm, Select, Space, Table } from 'antd';
...
const InventoryStockCounts: React.FC<InventoryStockCountsProps> = ({ embedded = false, onOpenAdjustmentDraft }) => {
      ...
      actions={
        <div className="misa-toolbar-right flex items-center gap-2">
          <Button
            ...
            title="Làm mới (F5)"
            onClick={() => void runManualDataLoad(() => counts.refetch(), { success: 'Tải lại biên bản kiểm kê thành công.', failure: 'Không thể tải lại biên bản kiểm kê.' })}
          />
          <Button
  ...
  return (
    ...
    >
      {counts.error && <div role="alert" className="misa-p-12 misa-mb-16" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
      ...
      {catalogueError && <div role="alert" className="misa-p-12 misa-mb-16" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Không tải được danh mục kho hoặc hàng hóa</div>
          <div style={{ color: '#B91C1C', fontSize: 12 }}>Không dùng danh sách rỗng thay thế; hãy tải lại danh mục từ máy chủ trước khi lập biên bản.</div>
        </div>
        <Button size="small" onClick={retryCatalogues}>Thử lại danh mục kiểm kê</Button>
      </div>}
      <DataTableSurface>
        <Table<StockCount>
          rowKey="id"
      ...
      >
        <ModalFrame>
          {catalogueError && <div role="alert" className="mb-4 misa-p-12" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Chưa thể xác thực danh mục kiểm kê</div>
              <div style={{ color: '#B91C1C', fontSize: 12 }}>Kho và hàng hóa phải được tải thành công từ máy chủ trước khi cất biên bản.</div>
            </div>
            <Button size="small" onClick={retryCatalogues}>Thử lại danh mục kiểm kê</Button>
          </div>}
      ...
      >
        <ModalFrame>
          ...
          {draftResult && (
            <div className="misa-p-12 misa-mb-12" style={{ background: '#EBF5FF', border: '1px solid #BFDBFE', borderRadius: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                <div>
                  <div style={{ fontWeight: 600, color: '#0064E0', fontSize: 13 }}>Đã tạo chứng từ điều chỉnh ở trạng thái nháp</div>
                  <div style={{ color: '#1C1E21', fontSize: 12, marginTop: 2 }}>Mở từng phiếu để bổ sung đơn giá và tài khoản theo mapping đã phê duyệt, sau đó dùng thao tác Ghi sổ của phiếu kho.</div>
                </div>
                <Space wrap>
