...
import { Button, Select, Table, Tag, DatePicker, Tooltip } from 'antd';
                ...
                actions={(
                    <div className="misa-toolbar-right flex items-center gap-2">
                        <Button
                            ...
                            title="Làm mới (F5)"
                            onClick={() => void runManualDataLoad(() => valuationRunQuery.refetch(), { success: 'Tải lại trạng thái tính giá thành công.', failure: 'Không thể tải lại trạng thái tính giá.' })}
                        />
                        <Button
                            type="primary"
            ...
            <DataTableSurface>
                {summaryInfo && (
                    <div className="misa-mb-12 flex items-center gap-3">
                        <span className="text-sm font-semibold text-[#0064E0] bg-[#EBF5FF] px-2.5 py-1 rounded border border-[#BFDBFE]">
                ...
                {currentValuationRun?.status === 'invalidated' && (
                    <div className="misa-p-10 misa-mb-12" style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 6 }}>
                        <div style={{ fontWeight: 600, color: '#92400E', fontSize: 12 }}>Cần tính lại vì có phát sinh kho thay đổi</div>
                    </div>
                )}
                <Table
                    columns={columns}
