...
import { Button, Dropdown, Empty, Form, Input, Select, Switch, Table, Tag, Tooltip } from 'antd';
...
const ChartOfAccounts: React.FC = () => {
    ...
    useEffect(() => { if (deferredSearch) setExpandedRowKeys(parentRowKeys); }, [deferredSearch, parentRowKeys]);
    ...
    return (
        <PageShell
            ...
            toolbar={<PageToolbar className="coa-workbench__header" filters={(
                ...
                <>
                    <Tooltip title="Làm mới"><Button aria-label="Làm mới" icon={<ReloadOutlined spin={isFetching && !isLoading} />} onClick={() => void runManualDataLoad(() => refetch(), { success: 'Tải lại hệ thống tài khoản thành công.', failure: 'Không thể tải lại hệ thống tài khoản.' })} /></Tooltip>
        ...
        >
            {isError && (
                <div className="coa-workbench__alert misa-p-12 misa-mb-12" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
            ...
            <Modal title="Chuyển tài khoản hạch toán" open={Boolean(transferAccount)}
                ...
                <ModalFrame>
                    ...
                    {previewMatches && transferPreview && (
                        <div className="misa-p-12 misa-mb-12" style={{ background: '#EBF5FF', border: '1px solid #BFDBFE', borderRadius: 8, color: '#0064E0', fontSize: 13, fontWeight: 500 }}>
                            <span>{`${transferPreview.count} tham chiếu sẽ được chuyển. Máy chủ kiểm tra lại trước khi lưu.`}</span>
                        </div>
                    )}
