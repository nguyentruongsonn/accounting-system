...
import { Empty, Table, Button, Space, Input, Form } from 'antd';
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (status: string) => (
                <span className={`misa-apple-pill ${status === 'active' ? 'misa-apple-pill-green' : 'misa-apple-pill-red'}`}>
        ...
        >
            {loadError && (
                <div className="misa-p-12 misa-mb-12" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                        <div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Không thể tải danh sách nhân viên</div>
                        <div style={{ color: '#B91C1C', fontSize: 12 }}>Các dòng đang hiển thị được giữ nguyên; máy chủ chưa trả về dữ liệu mới hợp lệ.</div>
                    </div>
                    <Button size="small" onClick={() => void fetchData()}>Thử lại</Button>
                </div>
            )}
            <DataTableSurface>
                <Table
