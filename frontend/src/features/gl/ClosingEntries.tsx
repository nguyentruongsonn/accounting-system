...
function parseClosingPreview(value: unknown): ClosingPreview {
    ...
    candidate.suggested_lines.forEach((line, index) => {
        ...
        const debitAccount = typeof row.debit_account === 'number' || typeof row.debit_account === 'string'
            ? String(row.debit_account).trim()
            : '';
        const creditAccount = typeof row.credit_account === 'number' || typeof row.credit_account === 'string'
            ? String(row.credit_account).trim()
            : '';
        ...
        row.credit_account = creditAccount;
    });
...
export const ClosingEntries: React.FC<{ embedded?: boolean }> = ({ embedded = false }) => {
    ...
    useEffect(() => {
        // Warning removed for small business workflow since we use automatic standard mappings
    ...
    return (
        <PageShell embedded={embedded} title={<PageHeader eyebrow="Sổ cái" title="Kết chuyển cuối kỳ" description="Xem trước các dòng kết chuyển do máy chủ cung cấp trước khi thực hiện." />}>
            ...
            {/* Config & Action Box */}
            <div className="misa-filter-box-lg mb-4 p-4 bg-white border border-slate-200 rounded-md">
                <div className="grid grid-cols-12 gap-4 items-end">
                    ...
                    <div className="col-span-3 flex gap-2">
                        <Button
                            icon={<ReloadOutlined />}
                            onClick={() => void runManualDataLoad(() => refetch(), { success: 'Tải lại bản xem kết chuyển thành công.', failure: 'Không thể tải lại bản xem kết chuyển.' })}
                            loading={isLoading}
                        >
                            Xem trước
                        </Button>
                        <Popconfirm
                            ...
                            description="Hệ thống sẽ tự động tạo bút toán kết chuyển doanh thu, chi phí và xác định kết quả kinh doanh cuối kỳ. Bạn có chắc chắn muốn thực hiện?"
