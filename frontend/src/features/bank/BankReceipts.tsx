...
import { Button, ConfigProvider, Form, Input, InputNumber, Select, DatePicker, Switch, Tag, Dropdown } from 'antd';
                ...
                <Form form={form} layout="vertical" onFinish={() => handleSaveForm(false, false)} size="small" className="misa-form-flex-col">
                    {(bankAccountsError || bankAccounts.length === 0) && (
                        <div className="misa-p-12 misa-mb-12" style={{ background: bankAccountsError ? '#FEF2F2' : '#FFFBEB', border: `1px solid ${bankAccountsError ? '#FECACA' : '#FDE68A'}`, borderRadius: 8 }}>
                            ...
                            <div style={{ color: bankAccountsError ? '#B91C1C' : '#B45309', fontSize: 12 }}>Chứng từ thu tiền gửi chỉ được lập khi máy chủ cung cấp tài khoản thuộc đơn vị hiện tại. Hãy tải lại hoặc khai báo tài khoản trước khi lưu.</div>
                        </div>
                    )}
                    {/* Master Card */}
                    <MisaMasterCard>
