...
import { Table, Button, Input, Select, Dropdown, Form, InputNumber, DatePicker } from 'antd';
...
export const BankTransactions: React.FC<BankTransactionsProps> = React.memo(() => {
    ...
    const togglePostMutation = useMutation({
        ...
        onMutate: async ({ id, type, isPosted }) => {
            const queryKey = type === 'receipt' ? ['bank-receipts'] : ['bank-payments'];
            const nextPosted = !isPosted;
            if (selectedRow?.id === id) {
                setSelectedRow((prev: any) => prev ? { ...prev, is_posted: nextPosted, status: nextPosted ? 'posted' : 'draft' } : null);
            }
            const context = optimisticTogglePostStatus(queryClient, queryKey, id, nextPosted, { status: nextPosted ? 'posted' : 'draft' });
            return {
                queryKey,
                context,
        ...
        onError: (err: any, _vars, ctx) => {
            if (ctx?.queryKey && ctx?.context) {
                rollbackVoucherCache(queryClient, ctx.queryKey, ctx.context);
            }
            if (ctx?.prevSelectedRow !== undefined) {
                setSelectedRow(ctx.prevSelectedRow);
            }
    ...
    const updateMutation = useMutation({
        ...
        onSuccess: (response: any, variables) => {
            ...
            const queryKey = variables.type === 'receipt' ? ['bank-receipts'] : ['bank-payments'];
            instantUpsertVoucher(queryClient, queryKey, evidence);
            if (selectedRow?.id === evidence.id) {
                setSelectedRow((prev: any) => prev ? { ...prev, ...evidence } : null);
            }
        ...
        <PageShell title={<PageHeader eyebrow="Ngân hàng" title="Giao dịch tiền gửi ngân hàng" description="Theo dõi báo Có, báo Nợ và số dư theo dữ liệu máy chủ." />}>
            ...
            <DataTableSurface className="bank-transactions-table-surface">
                {(isReceiptsError || isPaymentsError) && (
                    <div className="mb-3 misa-p-12" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div>
                            <div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Không thể tải chứng từ tiền gửi</div>
                            <div style={{ color: '#B91C1C', fontSize: 12 }}>Không hiển thị số liệu tổng hợp như thể danh sách bị rỗng. Hãy thử tải lại nguồn bị lỗi.</div>
                        </div>
