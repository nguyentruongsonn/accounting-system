...
const BankStatementReconciliation: React.FC = () => {
  ...
  if (!canView) return <PageShell title={<PageHeader eyebrow="Ngân hàng" title="Đối chiếu sao kê ngân hàng" description="Kiểm tra chứng cứ đối chiếu theo tenant và maker-checker." />}><div role="alert" className="misa-p-12 misa-mb-12" style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8 }}><div style={{ fontWeight: 600, color: '#92400E', fontSize: 13 }}>Bạn không có quyền xem đối chiếu sao kê</div><div style={{ color: '#B45309', fontSize: 12 }}>{`Cần quyền ${VIEW}. Màn hình không tải hoặc suy đoán dữ liệu nếu chưa được máy chủ cấp quyền.`}</div></div></PageShell>;
  ...
  return <PageShell title={<PageHeader eyebrow="Ngân hàng" title="Đối chiếu sao kê ngân hàng" description="Chứng cứ đối chiếu theo tenant và maker-checker." />}>
    ...
    <Modal title="Quyết định đề xuất ghép" open={decision !== null} onCancel={() => setDecision(null)} footer={null} destroyOnHidden><div className="misa-p-12 misa-mb-16" style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8 }}><div style={{ fontWeight: 600, color: '#92400E', fontSize: 13 }}>Maker-checker</div><div style={{ color: '#B45309', fontSize: 12 }}>Bạn không thể kiểm tra đề xuất do chính mình tạo. Máy chủ kiểm tra lại điều kiện này.</div></div><Form form={decisionForm} layout="vertical" onFinish={(values) => decide.mutate(values)}><Form.Item name="decision" label="Quyết định" rules={[{ required: true }]}><Select options={[['confirmed', 'Xác nhận'], ['rejected', 'Từ chối'], ['reversed', 'Đảo quyết định']].map(([value, label]) => ({ value, label }))} /></Form.Item><Form.Item name="reason" label="Lý do" rules={[{ required: true }]}><Input.TextArea maxLength={2000} /></Form.Item><Button type="primary" htmlType="submit" loading={decide.isPending}>Lưu quyết định</Button></Form></Modal>
  </PageShell>;
};
