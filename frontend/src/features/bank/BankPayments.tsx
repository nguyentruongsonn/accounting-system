...
import { Button, ConfigProvider, Form, Input, InputNumber, Select, DatePicker, Switch, Checkbox, Tag, Dropdown, Radio } from 'antd';
    ...
    const mutation = useMutation({
        mutationFn: async (values: any) => {
            ...
            return api.post('/bank/payments', {
                ...
                is_bulk_transfer: isBulkTransfer,
                contact_type: 'supplier',
                contact_id: values.contact_id,
                contact_name: suppliers?.find((s: any) => s.id === values.contact_id)?.name || values.contact_name,
                employee_id: values.employee_id,
                employee_name: employees?.find((e: any) => e.id === values.employee_id || e.code === values.employee_id)?.name,
                payee_name: values.receiver_name || values.contact_name,
                payee_address: values.address,
                payee_bank_account: values.recipient_account,
                payee_bank_name: values.recipient_bank,
            ...
            >
                <Form form={form} layout="vertical" onFinish={() => handleSaveForm(false, false)} size="small" className="misa-form-flex-col">
                    {(bankAccountsError || bankAccounts.length === 0) && (
                        <div className="misa-p-12 misa-mb-12" style={{ background: bankAccountsError ? '#FEF2F2' : '#FFFBEB', border: `1px solid ${bankAccountsError ? '#FECACA' : '#FDE68A'}`, borderRadius: 8 }}>
                            <div style={{ fontWeight: 600, color: bankAccountsError ? '#991B1B' : '#92400E', fontSize: 13 }}>{bankAccountsError ? 'Không tải được tài khoản ngân hàng' : 'Đơn vị chưa có tài khoản ngân hàng'}</div>
                            <div style={{ color: bankAccountsError ? '#B91C1C' : '#B45309', fontSize: 12 }}>Ủy nhiệm chi chỉ được lập khi máy chủ cung cấp tài khoản thuộc đơn vị hiện tại. Hãy tải lại hoặc khai báo tài khoản trước khi lưu.</div>
                        </div>
                    )}
                    ...
                    <MisaMasterCard>
                        <MisaMasterCard.Left>
                            <div className="misa-list-toolbar misa-gap-8">
                                <Checkbox checked={isBulkTransfer} onChange={(event) => setIsBulkTransfer(event.target.checked)}>Là UNC chuyển tiền theo lô</Checkbox>
