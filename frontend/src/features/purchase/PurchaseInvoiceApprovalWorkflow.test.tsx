...
const invoice = { id: 31, voucher_number: 'PN-000031', is_posted: false };

function approvalData(overrides: Partial<{ is_posted: boolean; requests: unknown[] }> = {}) {
  return {
    invoice_id: invoice.id,
    is_posted: false,
    snapshot_hash: 'current-purchase-snapshot-hash-123456',
...
function request(overrides: Record<string, unknown> = {}) {
  return {
    ...
    evidence: { snapshot_hash: 'request-snapshot-abcdef', is_current: true, reference_note: 'Đã đối chiếu hóa đơn NCC.' },
...
function renderWorkflow(invoiceOverride = invoice) {
  ...
  return render(<QueryClientProvider client={client}><PurchaseInvoiceApprovalWorkflow invoice={invoiceOverride} open onClose={vi.fn()} /></QueryClientProvider>);
}
...
describe('PurchaseInvoiceApprovalWorkflow', () => {
  it('loads and renders both current and stale server evidence without offering decision controls', { timeout: 15_000 }, async () => {
    useAuthStore.setState({ user: { id: 10, name: 'Maker', email: 'maker@example.test', permissions: ['purchase.invoices.view', 'purchase.invoices.update'] } });
    mockedApi.get.mockResolvedValue({ data: { data: approvalData({ requests: [request(), request({ id: 60, evidence: { snapshot_hash: 'old-purchase-snapshot-abcdef', is_current: false, reference_note: null } })] }) } });

    renderWorkflow();

    expect(await screen.findByText('Yêu cầu #61')).toBeInTheDocument();
  ...
  it('submits only a draft approval request and surfaces an API failure without false success', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Ghi chú đối chiếu chứng từ nguồn/i), { target: { value: 'Đã đối chiếu hóa đơn nhà cung cấp.' } });
    fireEvent.click(screen.getByRole('button', { name: /Gửi yêu cầu phê duyệt$/ }));

    await waitFor(() => expect(mockedApi.post).toHaveBeenCalledWith('/purchase/invoices/31/approval-requests', { reference_note: 'Đã đối chiếu hóa đơn nhà cung cấp.' }));
  ...
  it('keeps a posted document read-only and does not submit any request', async () => {
    useAuthStore.setState({ user: { id: 10, name: 'Maker', email: 'maker@example.test', permissions: ['purchase.invoices.view', 'purchase.invoices.update'] } });
    mockedApi.get.mockResolvedValue({ data: { data: approvalData({ is_posted: true }) } });

    renderWorkflow({ ...invoice, is_posted: true });

    expect(await screen.findByText('Chứng từ đã ghi sổ')).toBeInTheDocument();
