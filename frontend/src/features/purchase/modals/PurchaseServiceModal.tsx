...
import { Form, Input, InputNumber, DatePicker, Radio, Checkbox, Table, Button, Space, Popconfirm } from 'antd';
...
import type { PurchaseExpenseAllocationSelection } from './purchaseExpenseAllocation';
...
export const PurchaseServiceModal: React.FC<PurchaseServiceModalProps> = ({
    ...
    const [targetPurchaseVouchers, setTargetPurchaseVouchers] = useState<PurchaseExpenseAllocationSelection[]>([]);
    ...
    useEffect(() => {
        if (open) {
            ...
            if (editRecord) {
                form.setFieldsValue({
                    ...editRecord,
                    voucher_number: editRecord.voucher_number || editRecord.invoice_number,
                    ...
                    tax_code: editRecord.tax_code || editRecord.supplier?.tax_code,
                    invoice_address: editRecord.invoice_address || editRecord.supplier_address,
                    ...
                    contact_name: editRecord.contact_name,
                    receiver_address: editRecord.receiver_address,
                    deliverer_name: editRecord.deliverer_name || editRecord.receiver_name,
                    receiver_name: editRecord.receiver_name || editRecord.deliverer_name,
                });
                setPaymentStatus((editRecord.payment_status === 'Paid' || editRecord.payment_status === 'paid' || (editRecord.payment_method && editRecord.payment_method !== 'unpaid')) ? 'paid' : 'unpaid');
                setPaymentMethod(editRecord.payment_method && editRecord.payment_method !== 'unpaid' ? editRecord.payment_method : 'cash');
                ...
                if (editRecord.invoice_option) {
                    ...
                    else setInvoiceHandling('no_invoice');
                ...
                if (editRecord.currency) setCurrency(editRecord.currency);
                if (editRecord.exchange_rate) setExchangeRate(editRecord.exchange_rate);
                if (Array.isArray(editRecord.referenced_vouchers)) {
                    setReferenceVouchers(editRecord.referenced_vouchers);
                }
                setTargetPurchaseVouchers(Array.isArray(editRecord.expense_allocations) ? editRecord.expense_allocations : []);
    ...
    const handlePaymentTermChange = (term: string) => {
        let days = 30;
        if (term === 'ĐKTT30') days = 30;
        else if (term === 'ĐKTT45') days = 45;
        else if (term === 'ĐKTT60') days = 60;
        else if (term === 'ĐKTT90') days = 90;
        else if (term === 'COD') days = 0;
        form.setFieldsValue({
            due_days: days,
    ...
    const handleSupplierChange = (supplierId: number) => {
        ...
        if (supplier) {
            ...
            const currentReason = form.getFieldValue('spend_reason');
            const currentDeliverer = form.getFieldValue('deliverer_name');
            const currentReceiver = form.getFieldValue('receiver_name');
            ...
            if (!currentDeliverer) newFields.deliverer_name = supplier.contact_name || supplier.name;
            if (!currentReceiver) newFields.receiver_name = supplier.contact_name || supplier.name;
            if (!currentDesc || currentDesc.startsWith('Mua dịch vụ của')) {
                newFields.description = `Mua dịch vụ của ${supplier.name}`;
            }
            if (!currentReason || currentReason.startsWith('Chi tiền mua dịch vụ của')) {
                newFields.spend_reason = `Chi tiền mua dịch vụ của ${supplier.name}`;
            }
            newFields.bank_payment_content = `Thanh toán tiền mua dịch vụ cho ${supplier.name}`;
    ...
    const handleSave = async (andNew = false) => {
        try {
            ...
            const employeeObj = employeeList.find((e: any) => e.id === values.employee_id);
            const employeeName = employeeObj?.name || values.employee_name;
            ...
            const payload = {
                ...
                contact_name: resolveNullableString('contact_name', values, form, editRecord?.contact_name),
                invoice_address: resolveNullableString('invoice_address', values, form, editRecord?.invoice_address || editRecord?.supplier_address),
                ...
                receiver_address: resolveNullableString('receiver_address', values, form, editRecord?.receiver_address),
                ...
                expense_allocations: targetPurchaseVouchers.map((allocation) => ({
                    ...
                    allocation_method: allocation.allocation_method || 'manual',
                })),
                lines: lines.map(l => ({
            ...
            queryClient.invalidateQueries({ queryKey: ['purchase-expense-allocation-candidates'] });
    ...
    const accountingColumns = [
        ...
        },
        {
            ...
            dataIndex: 'cost_object_code',
            key: 'cost_object_code',
            width: 130,
            render: (val: string, r: ServiceLine) => (
                <Input
                    value={val}
                    onChange={e => updateLine(r.key, 'cost_object_code', e.target.value)}
                    placeholder="Mã ĐT THCP"
                    className="misa-input"
                />
            )
        },
        ...
        <>
                        ...
                        <MisaMasterCard.Right>
                            ...
                            {paymentStatus === 'unpaid' && (
                                <>
                                    ...
                                    <MisaMasterCard.MetaRow label="Số ngày được nợ">
                        ...
                        {gridActiveTab === 'expense_allocation' && (
                            <div className="misa-page-layout-gray">
                                ...
                                {targetPurchaseVouchers.length === 0 ? (
                                    ...
                                    <Table<PurchaseExpenseAllocationSelection>
                                        rowKey="target_purchase_invoice_line_id"
                                        ...
                                        columns={[
                                            ...
                                            { title: 'Giá trị còn lại', dataIndex: 'remaining_allocatable_amount', key: 'remaining_allocatable_amount', render: (v: number | string) => `${new Intl.NumberFormat('vi-VN').format(Number(v) || 0)} ₫` },
            ...
            />
             ...
             <SelectExpenseVoucherModal
                ...
                sourceInvoiceId={editRecord?.id}
                sourceExpenseTotal={subTotal}
                existingSelections={targetPurchaseVouchers}
                onSelect={(selected) => {
            ...
            <AllocateExpenseModal
                ...
                items={targetPurchaseVouchers.map((allocation) => ({
                    ...allocation,
                    amount: Number(allocation.remaining_allocatable_amount) || 0,
                }))}
                onAllocate={(allocatedItems: any[]) => {
                    setTargetPurchaseVouchers(allocatedItems.map((allocation) => ({
                        target_purchase_invoice_id: allocation.target_purchase_invoice_id,
                        target_purchase_invoice_line_id: allocation.target_purchase_invoice_line_id,
                        allocated_amount: String(allocation.allocated_expense ?? 0),
                        allocation_method: allocation.allocation_method || 'value',
                        voucher_number: allocation.voucher_number,
                        voucher_date: allocation.voucher_date,
                        supplier_name: allocation.supplier_name,
                        item_code: allocation.item_code,
                        item_name: allocation.item_name,
                        remaining_allocatable_amount: allocation.remaining_allocatable_amount,
                    })));
                    message.success('Đã tính phân bổ chi phí theo các dòng hàng đã chọn. Bấm Lưu để ghi nhận.');
