...
import { Form, Input, Select, DatePicker, Radio, Table, InputNumber, Tabs, Button, Space, Switch } from 'antd';
...
export interface PurchaseReturnModalProps {
    ...
    initialRecord?: PurchaseReturnRecord | null;
    isViewMode?: boolean;
    onSuccess?: () => void;
}

function parsePurchaseReturnCollection<T>(payload: unknown, resource: string): T[] {
...
export const PurchaseReturnModal: React.FC<PurchaseReturnModalProps> = ({
    ...
    const [isOutward, setIsOutward] = useState<boolean>(true);
    ...
    const {
        ...
        queryFn: async () => {
            ...
            const rows = parsePurchaseReturnCollection<any>(data, 'accounts');
    ...
    const { voucherNumber } = useAutoVoucherNumber({
        prefix: 'TLMH',
        endpoint: '/purchase/returns/next-code',
    ...
    const {
        ...
        queryFn: async () => {
            ...
            return parsePurchaseReturnCollection<SupplierOption>(data, 'suppliers');
        },
        enabled: open
    });
    ...
    const {
        ...
        queryFn: async () => {
            ...
            return parsePurchaseReturnCollection<EmployeeOption>(data, 'employees');
        },
        enabled: open
    });
    ...
    const {
        ...
        queryFn: async () => {
            ...
            return parsePurchaseReturnCollection<InventoryItemOption>(data, 'inventory items');
        },
        enabled: open
    });
    ...
    } = useQuery<WarehouseOption[]>({
        queryKey: ['warehouses'],
        queryFn: async () => {
            const { data } = await api.get('/master/warehouses');
            return parsePurchaseReturnCollection<WarehouseOption>(data, 'warehouses');
        },
        enabled: open
    });

    const defaultLineFactory = useCallback((): PurchaseReturnLine => {
        return {
            key: `line-${Date.now()}`,
    ...
    const { lines, setLines, addLine, removeLine, updateLine } = useVoucherLines<PurchaseReturnLine>({
        initialLines: [],
    ...
    useEffect(() => {
        ...
        if (recordId && initialRecord) {
            form.setFieldsValue({
                ...
                is_outward: initialRecord.is_outward ?? true,
            });
            setPaymentMethod(initialRecord.payment_method || 'reduce_payable');
            setIsOutward(initialRecord.is_outward ?? true);
            ...
            form.setFieldsValue({
                ...
                is_outward: true,
                description: 'Xuất trả lại hàng cho nhà cung cấp'
            });
    ...
    const saveMutation = useMutation({
        mutationFn: async (payload: unknown) => {
            if (recordId) {
                return api.put(`/purchase/returns/${recordId}`, payload);
            }
            return api.post('/purchase/returns', payload);
        },
        onSuccess: (response) => {
            const persistedReturn = response?.data?.data ?? response?.data;
            if (!persistedReturn || persistedReturn.id === undefined || persistedReturn.id === null) {
                message.error('Máy chủ không trả về chứng từ trả lại đã lưu; không thể báo thành công.');
                return;
            }
            message.success(recordId ? 'Cập nhật chứng từ trả lại thành công!' : 'Tạo mới chứng từ trả lại hàng mua thành công!');
    ...
    const handleSupplierChange = (supplierId: any, supItem?: any) => {
        ...
        if (sup) {
            form.setFieldsValue({
                ...
                description: `Xuất trả lại hàng mua cho ${sup.name}`
            });
        }
    };
    ...
    const handleQuantityPriceChange = (index: number, field: 'quantity' | 'unit_price', val: number | null) => {
        ...
        const discountAmt = amt * (discountRate / 100);
    ...
    const accountingColumns = [
        ...
        },
        {
            ...
            dataIndex: 'item_id',
            width: 140,
            render: (val: number, _: PurchaseReturnLine, idx: number) => (
                <AdaptiveSelect
                    showSearch
                    size="small"
                    value={val}
        ...
        },
        {
            ...
            dataIndex: 'item_name',
            width: 200,
            render: (val: string, _: PurchaseReturnLine, idx: number) => (
                <Input
                    size="small"
                    value={val}
                    disabled={isViewMode}
        ...
        },
        {
            ...
            render: (val: number, _: PurchaseReturnLine, idx: number) => (
                <AdaptiveSelect
                    ...
                    optionLabelProp="label"
                    filterOption={(input, option: any) => {
                        const code = String(option?.warehouseCode || option?.label || '').toLowerCase();
                        const name = String(option?.warehouseName || '').toLowerCase();
                        const q = input.toLowerCase();
                        return code.includes(q) || name.includes(q);
                    }}
                    onChange={(whId: any) => updateLine(idx, { warehouse_id: whId })}
    ...
    return (
                ...
                <div className="misa-modal-header-wrapper">
                    <div className="misa-flex-center misa-gap-12">
                        <span className="misa-voucher-header-title">
                            {recordId ? (isInternalViewMode ? `Xem chứng từ trả lại hàng mua: ${initialRecord?.voucher_number}` : `Sửa chứng từ trả lại hàng mua: ${initialRecord?.voucher_number}`) : 'Chứng từ trả lại hàng mua (Giảm trừ công nợ / Xuất kho)'}
                        </span>
                        {isInternalViewMode && (
        ...
        >
            ...
            {(isSuppliersError || isEmployeesError || isItemsError || isWarehousesError) && (
            ...
            <Form form={form} layout="vertical">
                {/* Header controls: Payment method & Outward options */}
                <div className="misa-config-bar">
                    <div className="misa-config-bar-left">
                        ...
                        >
                            ...
                            <Radio value="cash"><span className="misa-text-semibold">Thu tiền mặt ngay</span></Radio>
                        </Radio.Group>

                        <div className="misa-flex-center misa-gap-12 misa-ml-16">
                            <Radio.Group
                                value={isOutward ? 'outward' : 'no_outward'}
                                onChange={e => setIsOutward(e.target.value === 'outward')}
                                disabled={isViewMode}
                            >
                                <Radio value="outward"><span className="misa-text-semibold">Kiêm phiếu xuất kho</span></Radio>
                                <Radio value="no_outward"><span className="misa-text-semibold">Không kiêm xuất kho</span></Radio>
                            </Radio.Group>
                        </div>
                    </div>
                            ...
                            <div className="misa-form-grid">
                                ...
                                <div className="misa-col-4">
                                    ...
                                    <Form.Item name="deliverer_name" noStyle>
                                        <Input disabled={isViewMode} className="misa-input" placeholder="Người nhận hàng phía NCC" />
                                    </Form.Item>
                                </div>
                                ...
                                <div className="misa-col-10">
                                    ...
                                    <Form.Item name="description" noStyle>
                                        <Input disabled={isViewMode} className="misa-input" placeholder="Lý do xuất trả lại hàng cho nhà cung cấp..." />
                                    </Form.Item>
                                </div>
                ...
                <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 6, padding: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                        <Tabs
                            ...
                            items={[
                                ...
                                { key: 'statistic', label: '3. Thống kê (Đơn mua / Hợp đồng)' }
                            ]}
                        />
                    </div>
                    ...
                    <MisaTableSummaryBar
                        metrics={[
                            ...
                            { label: 'Tổng tiền hàng', value: totals.subTotal, format: 'currency', isCurrency: true },
                ...
                onSelect={(refs: any[]) => {
                    if (refs && refs.length > 0) {
                        ...
                        form.setFieldsValue({
                            ...
                            description: `Trả lại hàng mua theo ${ref.voucher_number}`,
