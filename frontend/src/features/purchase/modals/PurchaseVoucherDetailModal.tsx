...
import { Form, Input, InputNumber, DatePicker, Radio, Checkbox, Button, Space } from 'antd';
...
import { resolveNullableString, resolveNullableNumber } from './purchaseModalFieldUtils';
...
function mapReceivedExpenseVouchers(editRecord: any): any[] {
    if (!Array.isArray(editRecord?.received_expense_allocations)) return [];

    return editRecord.received_expense_allocations.map((allocation: any) => ({
        id: allocation.id,
        source_invoice_id: allocation.source_invoice_id,
        accounting_date: allocation.source_accounting_date || allocation.effective_date || '',
        voucher_date: allocation.source_voucher_date || allocation.effective_date || '',
        voucher_no: allocation.source_voucher_number || '—',
        supplier_name: allocation.source_supplier_name || '—',
        description: allocation.source_description || '—',
        total_expense: Number(allocation.source_total_expense) || 0,
        allocated_amount: Number(allocation.allocated_amount) || 0,
        this_allocation: Number(allocation.allocated_amount) || 0,
    }));
}
...
const NoticeBox: React.FC<{
    ...
    className?: string;
    showIcon?: boolean;
}> = ({ type = 'info', message, title, description, action, className = '' }) => {
    const isError = type === 'error';
    const isWarning = type === 'warning';
    const bg = isError ? '#FEF2F2' : isWarning ? '#FFFBEB' : '#EBF5FF';
    const border = isError ? '1px solid #FECACA' : isWarning ? '1px solid #FDE68A' : '1px solid #BFDBFE';
    const titleColor = isError ? '#991B1B' : isWarning ? '#92400E' : 'var(--ui-primary, #0064E0)';
    const descColor = isError ? '#B91C1C' : isWarning ? '#B45309' : '#1C1E21';
    const heading = title ?? message;

    return (
        <div
            role={isError || isWarning ? 'alert' : undefined}
            className={`misa-p-12 ${className}`}
            style={{
                background: bg,
                border,
                borderRadius: 8,
        ...
        >
            <div>
                {heading && <div style={{ fontWeight: 600, color: titleColor, fontSize: 13 }}>{heading}</div>}
                {description && <div style={{ color: descColor, fontSize: 12, marginTop: 2 }}>{description}</div>}
            </div>
...
export const PurchaseVoucherDetailModal: React.FC<PurchaseVoucherDetailModalProps> = ({
    ...
    useEffect(() => {
        if (open) {
            ...
            if (editRecord) {
                if (editRecord.voucher_type) {
                    setVoucherType(editRecord.voucher_type);
                    if (editRecord.voucher_type.startsWith('1') || editRecord.voucher_type.startsWith('3')) {
                        setMasterActiveTab('inward');
                    } else {
                        setMasterActiveTab('debt_voucher');
                    }
                }
                const pStatus = (editRecord.payment_status || (editRecord.payment_method === 'unpaid' ? 'unpaid' : 'paid')).toLowerCase();
                setPaymentStatus(pStatus === 'paid' ? 'paid' : 'unpaid');
                if (editRecord.payment_method && editRecord.payment_method !== 'unpaid') {
                    setPaymentMethod(editRecord.payment_method);
                }
                if (editRecord.invoice_option) {
                    if (editRecord.invoice_option === 'Nhận kèm hóa đơn') setInvoiceHandling('with_invoice');
                    else if (editRecord.invoice_option === 'Không kèm hóa đơn') setInvoiceHandling('without_invoice_now');
                    else if (editRecord.invoice_option === 'Không có hóa đơn') setInvoiceHandling('no_invoice');
                } else if (editRecord.is_include_invoice !== undefined) {
                    setInvoiceHandling(editRecord.is_include_invoice ? 'with_invoice' : 'without_invoice_now');
                }
                if (Array.isArray(editRecord.referenced_vouchers)) {
                    setReferenceVouchers(editRecord.referenced_vouchers);
                }
                setExpenseVouchers(mapReceivedExpenseVouchers(editRecord));
                ...
                form.setFieldsValue({
                    ...
                    deliverer_name: editRecord.deliverer_name,
                    receiver_name: editRecord.receiver_name || editRecord.deliverer_name,
                    ...
                    attached_docs: editRecord.attached_docs,
                    spend_reason: editRecord.spend_reason,
                    payment_slip_number: editRecord.payment_slip_number,
                    invoice_option: editRecord.invoice_option,
                    ...
                    lines: (Array.isArray(editRecord.items)
                        ...
                        cost_item_code: it.cost_item_code,
                        cost_object_code: it.cost_object_code,
    ...
    const handlePaymentMethodChange = (newMethod: 'cash' | 'bank' | 'cheque_transfer' | 'cheque_cash') => {
        ...
        const curLines = form.getFieldValue('lines') || [];
        form.setFieldsValue({ lines: curLines.map((line: any) => ({ ...line })) });
        if (newMethod === 'cash') setMasterActiveTab('cash_receipt');
    };

    // Handle payment term change and keep due date in sync with the voucher date
    ...
    // Handle due days change and keep due date in sync with the voucher date
    ...
    // Auto fill supplier
    const handleSupplierChange = (supplierId: number, secondArg?: unknown) => {
        const shouldKeepDesc = secondArg === true;
        const supplier = supplierList.find((item: any) => item.id === supplierId);
        if (supplier) {
            form.setFieldsValue({
                ...
                ...(shouldKeepDesc ? {} : { description: `Mua hàng của ${supplier.name}` })
            });
        }
    };
    ...
    const totalStockValue = isStockInward
        ? ((Number(totals.subTotal) || 0) - (Number(totals.totalDiscount) || 0) + (Number(totals.totalPurchaseExpense) || 0))
        : 0;
    ...
    const handleSave = useCallback(async (closeAfterSave = true) => {
        try {
            ...
            const invoiceOptionValue = invoiceHandling === 'with_invoice'
                ? 'Nhận kèm hóa đơn'
                : (invoiceHandling === 'without_invoice_now' ? 'Không kèm hóa đơn' : 'Không có hóa đơn');

            const payload = {
                supplier_id: values.supplier_id,
                supplier_name: values.supplier_name,
                supplier_address: values.supplier_address,
                invoice_address: resolveNullableString('invoice_address', values, form, editRecord?.invoice_address),
                deliverer_name: resolveNullableString('deliverer_name', values, form, editRecord?.deliverer_name),
                receiver_name: resolveNullableString('receiver_name', values, form, editRecord?.receiver_name),
                tax_code: resolveNullableString('tax_code', values, form, editRecord?.tax_code),
                employee_id: values.employee_id,
                employee_name: values.employee_name || employeeList.find((e: any) => e.id === values.employee_id)?.name,
                attached_docs: resolveNullableString('attached_docs', values, form, editRecord?.attached_docs),
                currency: values.currency || 'VND',
                exchange_rate: values.exchange_rate || 1,
                invoice_number: values.voucher_number || values.invoice_number || editRecord?.voucher_number || editRecord?.invoice_number,
                voucher_number: values.voucher_number || values.invoice_number || editRecord?.voucher_number || editRecord?.invoice_number,
                voucher_type: values.voucher_type || voucherType || editRecord?.voucher_type,
                invoice_date: values.voucher_date?.format('YYYY-MM-DD') || values.invoice_date?.format('YYYY-MM-DD'),
                accounting_date: values.accounting_date?.format('YYYY-MM-DD'),
                due_date: values.due_date?.format('YYYY-MM-DD'),
                payment_status: paymentStatus === 'unpaid' ? 'Unpaid' : 'Paid',
                payment_term_code: resolveNullableString('payment_term_code', values, form, editRecord?.payment_term_code),
                due_days: resolveNullableNumber('due_days', values, form, editRecord?.due_days),
                spend_reason: resolveNullableString('spend_reason', values, form, editRecord?.spend_reason),
                payment_slip_number: resolveNullableString('payment_slip_number', values, form, editRecord?.payment_slip_number),
                purchase_expense: Number(totals.totalPurchaseExpense) || 0,
                total_stock_value: totalStockValue,
                description: values.description,
                payment_method: paymentStatus === 'unpaid' ? 'unpaid' : paymentMethod,
                is_include_invoice: invoiceHandling === 'with_invoice',
                invoice_option: invoiceOptionValue,
                invoice_symbol: values.invoice_symbol,
                invoice_code: values.invoice_number || values.invoice_code || form.getFieldValue('invoice_number') || editRecord?.invoice_code,
                invoice_form: resolveNullableString('invoice_form', values, form, editRecord?.invoice_form),
                referenced_vouchers: referenceVouchers,
                lines: (values.lines || form.getFieldValue('lines') || editRecord?.lines || []).map((l: any, idx: number) => {
                    const fallbackLine = editRecord?.lines?.[idx] || editRecord?.items?.[idx];
                    const purchaseExp = Number(l.purchase_expense ?? fallbackLine?.purchase_expense) || 0;
                    const rawStockVal = l.stock_value ?? fallbackLine?.stock_value;
                    const calculatedStockVal = isStockInward
                        ? (rawStockVal !== undefined && rawStockVal !== null ? Number(rawStockVal) : ((Number(l.amount ?? fallbackLine?.amount) || 0) - (Number(l.discount_amount ?? fallbackLine?.discount_amount) || 0) + purchaseExp))
                        : 0;

                    return {
                        item_id: l.item_id ?? fallbackLine?.item_id,
                        service_code: l.service_code ?? fallbackLine?.service_code,
                        description: l.item_name || l.description || fallbackLine?.description || values.description,
                        warehouse: l.warehouse ?? fallbackLine?.warehouse ?? fallbackLine?.warehouse_code,
                        warehouse_code: l.warehouse ?? fallbackLine?.warehouse ?? fallbackLine?.warehouse_code,
                        warehouse_id: warehouseList.find((warehouse: any) =>
                            (warehouse.code || warehouse.warehouse_code) === (l.warehouse ?? fallbackLine?.warehouse)
                        )?.id,
                        debit_account: l.debit_account ?? fallbackLine?.debit_account,
                        credit_account: l.credit_account ?? fallbackLine?.credit_account,
                        unit: l.unit ?? fallbackLine?.unit,
                        quantity: Number(l.quantity ?? fallbackLine?.quantity) || 0,
                        unit_price: Number(l.unit_price ?? fallbackLine?.unit_price) || 0,
                        amount: Number(l.amount ?? fallbackLine?.amount) || 0,
                        discount_rate: Number(l.discount_rate ?? fallbackLine?.discount_rate) || 0,
                        discount_amount: Number(l.discount_amount ?? fallbackLine?.discount_amount) || 0,
                        tax_rate: l.tax_rate ?? fallbackLine?.tax_rate,
                        tax_amount: Number(l.tax_amount ?? fallbackLine?.tax_amount) || 0,
                        tax_account: l.tax_account ?? fallbackLine?.tax_account,
                        import_tax_rate: Number(l.import_tax_rate ?? fallbackLine?.import_tax_rate) || 0,
                        import_tax_amount: Number(l.import_tax_amount ?? fallbackLine?.import_tax_amount) || 0,
                        purchase_expense: purchaseExp,
                        stock_value: calculatedStockVal,
                        invoice_symbol: values.invoice_symbol,
                        ...
                        cost_item_code: l.cost_item_code ?? fallbackLine?.cost_item_code,
                        cost_object_code: l.cost_object_code ?? fallbackLine?.cost_object_code,
                        order_id: l.order_id ?? fallbackLine?.order_id,
                        contract_id: l.contract_id ?? fallbackLine?.contract_id,
                    };
                }) || []
            };
    ...
    }, [form, paymentStatus, paymentMethod, isStockInward, voucherType, totals.totalPurchaseExpense, totalStockValue, invoiceHandling, onSuccess, onCancel, employeeList, queryClient, referenceVouchers, warehouseList, editRecord]);
                            ...
                            {gridActiveTab === 'expenses' && (
                                <div className="misa-flex-center misa-gap-8">
                                    <span className="apple-muted-text">
                                        Phân bổ được quản lý trên chứng từ mua dịch vụ. Chỉnh sửa phân bổ trên chứng từ mua dịch vụ.
                                    </span>
                                </div>
                            )}
                                ...
                                <table className="misa-grid-table misa-w-full">
                                    <thead>
                                        <tr>
                                            ...
                                            <th className="misa-col-w-160 misa-text-right">Số đã phân bổ</th>
                                            <th className="misa-col-w-36 misa-text-center"></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {expenseVouchers.length === 0 ? (
                                            <tr>
                                                <td colSpan={10} className="misa-empty-table-cell">
                                                    <div>Chưa có chứng từ chi phí được liên kết.</div>
                                                    <div className="apple-muted-text misa-mt-4">
                                                        Để phân bổ chi phí từ chứng từ dịch vụ, mở chứng từ chi phí và chọn các dòng hàng mua đích.
                                                    </div>
                                                </td>
                                            </tr>
                                        ) : (
                                            expenseVouchers.map((ev, idx) => (
                                                <tr key={ev.id}>
                                                    ...
                                                    <td className="misa-text-right misa-text-bold misa-text-blue">
                                                        {new Intl.NumberFormat('vi-VN').format(ev.this_allocation || 0)} ₫
                                                    </td>
                                                    <td className="misa-text-center apple-muted-text">—</td>
                                                </tr>
                                            ))
                                        )}
                                        {expenseVouchers.length > 0 && (
