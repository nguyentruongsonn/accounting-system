...
export const PurchaseDiscounts: React.FC = () => {
    ...
    const postMutation = useMutation({
        ...
        onMutate: async (id: number) => {
            return optimisticTogglePostStatus(queryClient, ['purchase-discounts'], id, true);
        },
        onSuccess: (response) => {
        ...
        onError: (err: unknown, _id: number, context) => {
            rollbackVoucherCache(queryClient, ['purchase-discounts'], context);
    ...
    const unpostMutation = useMutation({
        ...
        onMutate: async (id: number) => {
            return optimisticTogglePostStatus(queryClient, ['purchase-discounts'], id, false);
        },
        onSuccess: (response) => {
            if (persistedActionId(response) === undefined || persistedActionId(response) === null) {
            ...
            render: (_: unknown, record: PurchaseDiscountRecord) => {
                ...
                return (
                    <VoucherActionCell
                        ...
                        onPrimaryAction={() => handleOpenViewRecord(record)}
                        menuItems={menuItems}
                    />
                );
            }
                        ...
                        <div className="ui-page-toolbar__action-group flex items-center gap-2">
                            <Button
                                ...
                                onClick={() => void runManualDataLoad(
                                    () => refetchList(),
                                    { success: 'Tải lại danh sách giảm giá hàng mua thành công.', failure: 'Không thể tải lại danh sách giảm giá hàng mua.' },
                                )}
        ...
        >
            ...
            {isListError && (
                ...
                >
                    ...
                    <Button size="small" onClick={() => void runManualDataLoad(() => refetchList(), { success: 'Tải lại danh sách giảm giá hàng mua thành công.', failure: 'Không thể tải lại danh sách giảm giá hàng mua.' })}>Thử lại</Button>
                </div>
            )}
