...
export default function PurchaseInvoiceDimensionAssignments({ invoice, open, onClose }: Props) {
  ...
  return (
    ...
    >
      ...
      {(contextQuery.isError || assignmentsQuery.isError) && (
        <div role="alert" className="mb-4 misa-p-12" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8 }}>
          <div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Không thể xác minh context hoặc evidence</div>
          <div style={{ color: '#B91C1C', fontSize: 12, marginTop: 2 }}>Chứng từ ở chế độ chỉ xem. Không lưu hay gửi ghi sổ khi máy chủ chưa trả đầy đủ context.</div>
        </div>
      )}
        ...
        <>
          ...
          {context?.status === 'not_required' && (
            <div className="mb-4 misa-p-12" style={{ background: '#EBF5FF', border: '1px solid #BFDBFE', borderRadius: 8 }}>
              <div style={{ fontWeight: 600, color: 'var(--ui-primary, #0064E0)', fontSize: 13 }}>Policy hiệu lực không yêu cầu chiều hạch toán</div>
              <div style={{ color: '#1C1E21', fontSize: 12, marginTop: 2 }}>Không có lựa chọn nào cần lưu cho ngày hạch toán này.</div>
            </div>
          )}
          {contextUnavailable && (
            <div role="alert" className="mb-4 misa-p-12" style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8 }}>
              <div style={{ fontWeight: 600, color: '#92400E', fontSize: 13 }}>Chưa thể chọn chiều hạch toán</div>
              <div style={{ color: '#B45309', fontSize: 12, marginTop: 2 }}>{context?.reason || 'Máy chủ chưa có policy hoặc dimension/value hiệu lực phù hợp.'}</div>
            </div>
          )}
          {saveForbidden && (
            <div role="alert" className="mb-4 misa-p-12" style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8 }}>
              <div style={{ fontWeight: 600, color: '#92400E', fontSize: 13 }}>Bạn chỉ có quyền xem</div>
              <div style={{ color: '#B45309', fontSize: 12, marginTop: 2 }}>Máy chủ không cấp quyền cập nhật chứng từ mua hàng; giao diện không cố bypass quyền này.</div>
            </div>
          )}
