...
export default function PurchaseInvoiceApprovalWorkflow({ invoice, open, onClose }: Props) {
  ...
  return <Drawer title="Phê duyệt chứng từ mua hàng" size={650} open={open} onClose={onClose} destroyOnHidden>
    ...
    {canView && !requestQuery.isLoading && !requestQuery.isError && <>
      ...
      {Boolean(invoice?.is_posted ?? requestQuery.data?.is_posted) && <div role="alert" className="mb-4 misa-p-12" style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8 }}><div style={{ fontWeight: 600, color: '#92400E', fontSize: 13 }}>Chứng từ đã ghi sổ</div><div style={{ color: '#B45309', fontSize: 12, marginTop: 2 }}>Không thể tạo yêu cầu mới cho chứng từ đã ghi sổ. Dùng quy trình điều chỉnh/đảo được kiểm soát nếu cần.</div></div>}
