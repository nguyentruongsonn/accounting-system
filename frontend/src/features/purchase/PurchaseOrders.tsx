...
import { Table, Button, Input, Select, DatePicker, Popconfirm, Form, InputNumber } from 'antd';
...
export const PurchaseOrders: React.FC = () => {
    ...
    const columns = [
        ...
        { 
            ...
            align: 'center' as const,
            render: (s: string) => {
                let tone = 'misa-apple-pill-orange';
                let label = 'Chưa thực hiện';
                if (s === 'processing') {
                    tone = 'misa-apple-pill-blue';
                    label = 'Đang thực hiện';
                } else if (s === 'completed') {
                    tone = 'misa-apple-pill-green';
                    label = 'Hoàn thành';
                } else if (s === 'cancelled') {
                    tone = 'misa-apple-pill-gray';
                    label = 'Hủy bỏ';
                }
                return (
                    <span className={`misa-apple-pill ${tone}`}>
        ...
        },
        {
            ...
            render: (_: any, r: any) => (
                <VoucherActionCell
                    ...
                    onPrimaryAction={() => handleEditOrder(r)}
                    menuItems={getActionMenu(r).items}
                />
            )
        }
    ];
                        ...
                        <div className="ui-page-toolbar__action-group flex items-center gap-2">
                            <Button
                                ...
                                onClick={() => void runManualDataLoad(
                                    () => refetchOrders(),
                                    { success: 'Tải lại danh sách đơn mua hàng thành công.', failure: 'Không thể tải lại danh sách đơn mua hàng.' },
                                )}
                            />
                            <Button
                                icon={<PrinterOutlined />}
                                                ...
                                                <DatePicker
                                                    ...
                                                    onChange={(d) => {
                                                        ...
                                                        form.setFieldValue(
                                                            'due_days',
                                                            d && orderDate
                                                                ? Math.max(0, dayjs(d).startOf('day').diff(dayjs(orderDate).startOf('day'), 'day'))
                                                                : null,
                                                        );
                                                    }}
                                                />
