...
import FixedAssetTransfers from './FixedAssetTransfers';
import FixedAssetInventory from './FixedAssetInventory';
...
const TABS = [
  ...
  { key: 'tab-transfer', label: 'Điều chuyển' },
  { key: 'tab-inventory', label: 'Kiểm kê' },
  { key: 'tab-tools', label: 'Công cụ dụng cụ (CCDC)' },
      ...
      : activeTabKey === 'tab-depreciation'
        ...
        : activeTabKey === 'tab-revaluation'
          ...
          : activeTabKey === 'tab-reduction'
            ...
            : activeTabKey === 'tab-transfer'
              ? 'Lập biên bản điều chuyển'
              : activeTabKey === 'tab-inventory'
                ? 'Lập biên bản kiểm kê'
                : activeTabKey === 'tab-tools'
                  ? 'Thêm CCDC'
            : 'Xem phạm vi báo cáo';

  const handleToolbarAction = () => {
    if (activeTabKey === 'tab-registrations') {
      window.dispatchEvent(new Event('open-fixed-asset-registration'));
    } else if (activeTabKey === 'tab-depreciation') {
      window.dispatchEvent(new Event('open-fixed-asset-depreciation'));
    } else if (activeTabKey === 'tab-revaluation') {
      window.dispatchEvent(new Event('open-fixed-asset-revaluation'));
    } else if (activeTabKey === 'tab-reduction') {
      window.dispatchEvent(new Event('open-fixed-asset-disposal'));
    } else if (activeTabKey === 'tab-tools') {
      window.dispatchEvent(new Event('open-tool-create'));
    } else {
      message.info(`${activeTabLabel}: ${activeTabAction}`);
    }
  };
    ...
    <div className="misa-ca-process-container">
      ...
      <div className="misa-ca-process-main">
        ...
        <div className="misa-ca-bottom-bar">
          ...
          <button type="button" className="misa-ca-category-btn" onClick={() => handleTabChange('tab-transfer')}>
            <SwapOutlined /> Điều chuyển TSCĐ
          </button>
          <button type="button" className="misa-ca-category-btn" onClick={() => handleTabChange('tab-inventory')}>
            <FileTextOutlined /> Kiểm kê TSCĐ
          </button>
          <button type="button" className="misa-ca-category-btn" onClick={() => handleTabChange('tab-tools')}>
            <ToolOutlined /> Công cụ dụng cụ (CCDC)
          </button>
        </div>
      </div>
  ...
  return (
    ...
    <MisaWorkspaceLayout
      ...
      toolbar={<PageToolbar
        ...
        actions={<Button type="primary" icon={<PlusOutlined />} className="misa-btn-primary" onClick={handleToolbarAction}>{activeTabAction}</Button>}
      />}
    >
      {/* Tab 1 – Quy trình */}
      ...
      <div className={activeTabKey === 'tab-transfer' ? 'misa-tab-pane' : 'misa-tab-pane-hidden'}>
        {isTabMounted('tab-transfer') && (
          <ErrorBoundary fallback={<div className="p-6 text-red-500 font-semibold">Lỗi khi tải Điều chuyển TSCĐ</div>}>
            <FixedAssetTransfers embedded />
          </ErrorBoundary>
        )}
      </div>

      <div className={activeTabKey === 'tab-inventory' ? 'misa-tab-pane' : 'misa-tab-pane-hidden'}>
        {isTabMounted('tab-inventory') && (
          <ErrorBoundary fallback={<div className="p-6 text-red-500 font-semibold">Lỗi khi tải Kiểm kê TSCĐ</div>}>
            <FixedAssetInventory embedded />
          </ErrorBoundary>
        )}
      </div>

      <div className={activeTabKey === 'tab-tools' ? 'misa-tab-pane' : 'misa-tab-pane-hidden'}>
        {isTabMounted('tab-tools') && (
          <ErrorBoundary fallback={<div className="p-6 text-red-500 font-semibold">Lỗi khi tải Công cụ dụng cụ (CCDC)</div>}>
            <Tools embedded />
          </ErrorBoundary>
        )}
      </div>
