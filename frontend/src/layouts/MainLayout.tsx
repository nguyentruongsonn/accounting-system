...
export const resolveBreadcrumb = (pathname: string): { section: string; page: string; tag?: string } => {
  ...
  if (pathname.startsWith('/settings/audit-trail')) return { section: 'Thiết lập', page: 'Nhật ký hoạt động' };
  if (pathname.startsWith('/settings/opening-balances')) return { section: 'Thiết lập', page: 'Số dư đầu kỳ' };
...
export const resolveActiveMenuKey = (pathname: string): string | undefined => {
  ...
  if (pathname.startsWith('/settings/audit-trail')) return '/settings/audit-trail';
  if (pathname.startsWith('/settings/opening-balances')) return '/settings/opening-balances';
...
const SEARCH_DIRECTORY = [
  ...
  { label: 'Nhật ký hoạt động', category: 'Thiết lập', route: '/settings/audit-trail' },
...
export const MainLayout: React.FC = () => {
  ...
  const menuItems = [
    ...
    {
      key: '/settings/opening-balances',
      icon: <BookOutlined style={{ fontSize: 16 }} />,
      label: 'Số dư đầu kỳ',
    },
    ...(authUser?.roles?.includes('admin') ? [
      {
        key: '/settings/audit-trail',
    ...
    <Layout className="app-shell" style={{ minHeight: '100vh', background: '#FFFFFF' }}>
      ...
      >
        ...
        <Menu
          ...
          defaultOpenKeys={[]}
          onClick={({ key }) => {
            if (key === 'purchase-group') {
              navigate('/purchase');
            } else if (key === 'sales-group') {
              navigate('/sales');
            } else {
              navigate(key);
            }
      ...
      >
        ...
        >
          ...
          <div className="app-header__leading" style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <Button
              type="text"
          ...
          <div className="app-header__trailing" style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <Button 
              type="text" 
