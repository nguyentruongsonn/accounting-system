...
export const CollectMultiCustomerModal: React.FC<CollectMultiCustomerModalProps> = ({ open, onClose, onSuccess }) => {
    ...
    const handleLoadInvoices = () => runManualDataLoad(async () => {
        const result = await invoicesQuery.refetch();
        if (!result.isError) setHasLoaded(true);
        return result;
    }, {
        success: 'Tải dữ liệu hóa đơn chưa thu thành công.',
        failure: 'Không thể tải dữ liệu hóa đơn chưa thu.',
    });
    ...
    return (
        ...
        >
            <ModalFrame className="misa-collect-multi-customer-modal__frame">
                {(invoicesQuery.isError || accountsQuery.isError) && (
                    <div className="misa-p-12 misa-mb-12" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div>
                            <div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Không thể tải hóa đơn chưa thu</div>
                            <div style={{ color: '#B91C1C', fontSize: 12 }}>Danh sách hóa đơn hoặc tài khoản chưa tải được. Dữ liệu cũ được giữ lại; hãy thử lại.</div>
                        </div>
                        <Button
                            size="small"
                            onClick={() => void runManualDataLoad(
                                () => Promise.all([invoicesQuery.refetch(), accountsQuery.refetch()]),
                                { success: 'Tải lại hóa đơn và tài khoản thành công.', failure: 'Không thể tải lại hóa đơn hoặc tài khoản.' },
                            )}
                        >Thử lại</Button>
                    </div>
                )}
                <div className="misa-filter-box"><div className="misa-flex-center-gap-12 flex-wrap">
                    ...
                    <Button type="primary" icon={<FilterOutlined />} className="misa-btn-modal-action" onClick={() => void handleLoadInvoices()} loading={invoicesQuery.isFetching}>Lấy dữ liệu</Button>
