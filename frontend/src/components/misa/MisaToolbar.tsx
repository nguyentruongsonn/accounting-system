...
export const MisaToolbar: React.FC<MisaToolbarProps> = ({
    ...
    onDatePresetChange,
    onRefresh,
    refreshSuccessMessage = 'Làm mới dữ liệu thành công!',
    onPrint,
    onExport,
    addButtonText = 'Thêm',
    onAddClick,
    ...
    const handleRefresh = async () => {
        if (!onRefresh) return;
        await runManualDataLoad(() => onRefresh(), {
            success: refreshSuccessMessage,
            failure: 'Không thể tải lại dữ liệu. Vui lòng thử lại.',
        });
    };
    ...
    return (
        <div className="misa-toolbar">
            ...
            <div className="misa-toolbar-actions">
                {onRefresh && (
                    <button type="button" className="misa-btn-tool" title="Làm mới (F5)" onClick={handleRefresh}>
                        <ReloadOutlined />
                    </button>
                )}
                {onPrint && (
