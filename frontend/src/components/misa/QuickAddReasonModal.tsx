...
import { Drawer, Form, Input, Tooltip, Spin } from 'antd';
                ...
                <Spin spinning={accountsLoading || voucherTypesLoading}>
                    {(voucherTypesError || voucherTypeOptions.length === 0) && !voucherTypesLoading && (
                        <div className="misa-p-12 misa-mb-16" style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8 }}>
                            <div style={{ fontWeight: 600, color: '#92400E', fontSize: 13 }}>Chưa có loại chứng từ từ máy chủ</div>
                            <div style={{ color: '#B45309', fontSize: 12 }}>Không dùng danh sách tĩnh thay thế. Hãy cấu hình catalogue loại chứng từ trên máy chủ trước khi lưu tài khoản ngầm định.</div>
                        </div>
                    )}
