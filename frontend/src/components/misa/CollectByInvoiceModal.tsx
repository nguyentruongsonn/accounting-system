...
import { Button, Input, InputNumber, DatePicker, Select, Space, Table } from 'antd';
...
import { createCustomerOutstandingInvoicesQuery } from './cashCollectionQueries';
...
export const CollectByInvoiceModal: React.FC<CollectByInvoiceModalProps> = ({ open, onClose, onSuccess, initialInvoice }) => {
    ...
    const outstandingQuery = useQuery({
        ...createCustomerOutstandingInvoicesQuery(selectedCustomerId, paymentDate.format('YYYY-MM-DD')),
        enabled: false,
    });
    ...
    return (
        ...
        >
            <ModalFrame className="misa-collect-by-invoice-modal__frame">
                {(customersQuery.isError || accountsQuery.isError) && (
                    <div className="misa-p-12 misa-mb-12" style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div>
                            <div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Không thể tải dữ liệu thu tiền theo hóa đơn</div>
                            <div style={{ color: '#B91C1C', fontSize: 12 }}>Danh mục khách hàng hoặc tài khoản chưa tải được. Dữ liệu form được giữ nguyên; hãy thử lại.</div>
                        </div>
                        <Button size="small" onClick={retryLookups}>Thử lại</Button>
                    </div>
                )}
