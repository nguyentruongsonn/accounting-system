...
import { Select, DatePicker, Button, Input, Table, Tooltip } from 'antd';
...
export const ReferenceVoucherModal: React.FC<ReferenceVoucherModalProps> = ({
    ...
    const { data: customers = [], refetch: refetchCustomers } = customerQuery;
    ...
    const { data: suppliers = [], refetch: refetchSuppliers } = supplierQuery;
    ...
    const { data: employees = [], refetch: refetchEmployees } = employeeQuery;
    ...
    const retryLookups = useCallback(async () => {
        if (searchBy === 'Đối tượng') {
            await Promise.all([refetch(), refetchCustomers(), refetchSuppliers(), refetchEmployees()]);
        } else {
            await refetch();
        }
    }, [searchBy, refetch, refetchCustomers, refetchSuppliers, refetchEmployees]);
    ...
    return (
        ...
        >
            <div className="misa-ref-body">
                ...
                {lookupError ? (
                    <div className="misa-p-12" style={{ marginBottom: 10, background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div>
                            <div style={{ fontWeight: 600, color: '#991B1B', fontSize: 13 }}>Không thể tải dữ liệu tham chiếu</div>
                            <div style={{ color: '#B91C1C', fontSize: 12 }}>Các lựa chọn và dòng đang hiển thị được giữ nguyên; máy chủ chưa trả về dữ liệu mới hợp lệ.</div>
                        </div>
                        <Button size="small" onClick={() => void retryLookups()}>Thử lại</Button>
                    </div>
                ) : null}
